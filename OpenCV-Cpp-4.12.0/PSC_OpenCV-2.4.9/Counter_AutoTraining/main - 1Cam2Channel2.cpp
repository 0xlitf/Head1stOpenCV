...
string g_fileName;
cv::Mat sourceImg, trainResult_1, trainResult_2;
...
{	
	...
	string root = "E:/VC++  Project/Counter_halcon/20240313";
	...
	for (auto& strProject : dirLists) {
		...
		for (auto& imagePath : projectLists) {
			...
			ptr_partsTrainer_01->setTrainTarget(2); // 设置单个物料训练超过3次才保存结果

			flag &= ptr_partsTrainer_02->HPDetection_start(g_strProject_2, roiPath_2);
			ptr_partsTrainer_02->setTrainTarget(2); // 设置单个物料训练超过3次才保存结果
#endif // OPENCV
...
void run() {
	...
	char resultName[255];
	sprintf_s(resultName, "%s/out_1.txt", path.c_str());
	ofstream out_1(resultName, ios::out | ios::binary);
	sprintf_s(resultName, "%s/out_2.txt", path.c_str());
	...
	for (int j = 0; j < files.size(); j++)
	{
...
#ifdef OPENCV
		ptr_partsTrainer_01->training(&sourceImg, &trainResult, 200);   //OPENCV
		ptr_partsTrainer_01->saveData();
		cv::imshow("trainResult", trainResult);
		cv::waitKey(0);

		ptr_partsTrainer_02->training(&sourceImg, &trainResult, 200);   //OPENCV
		...
		cv::waitKey(50);
		ptr_partsTrainer_01->HPDetection_train(sourceImg, trainResult_1);      //HALCON训练
		...
		ptr_partsTrainer_02->HPDetection_train(sourceImg, trainResult_2);      //HALCON训练
		...
		ptr_partsCounterHal_01->action(&sourceImg, showImg);
		ptr_partsCounterHal_02->action(&sourceImg, showImg);
...
#else
		tTime = (double)cv::getTickCount();
		ptr_partsCounterHal_01->actionHPDetectionObjectMat(&sourceImg, showImg);
		ptr_partsCounterHal_02->actionHPDetectionObjectMat(&sourceImg, showImg);
		tTime = ((double)cv::getTickCount() - tTime) / cv::getTickFrequency();
