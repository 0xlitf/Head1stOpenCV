...
HALCON_EXPORT bool dataToHobject(const uchar* pPixelBlock, int width, int height, int channels, bool isAlignment, bool isBgr, HObject* phImage)
{
    ...
    try
    {
        ...
        case 3:
        {
            GenImage3(phImage, "byte", width, height, (Hlong)pPixelBlock, (Hlong)pPixelBlock, (Hlong)pPixelBlock);

            uchar* pRed = NULL, * pGreen = NULL, * pBlue = NULL;
			GetImagePointer3(*phImage, (HTuple*)&pRed, (HTuple*)&pGreen, (HTuple*)&pBlue, NULL, NULL, NULL);
...
HALCON_EXPORT bool dataToHobject(const uchar* pRedBlock, const uchar* pGreenBlock, const uchar* pBlueBlock, int width, int height, bool isAlignment, HObject* phImage)
{
    ...
    try
    {
        GenImage3(phImage, "byte", width, height, (Hlong)pRedBlock, (Hlong)pGreenBlock, (Hlong)pBlueBlock);
...
HALCON_EXPORT bool dataOfHobject(const HObject *phImage, uchar*& pRed, uchar*& pGreen, uchar*& pBlue, int& width, int& height, int& channels, bool& isAlignment)
{
    ...
    try
    {
		...
		{
			GetImagePointer1(*phImage, (HTuple*)&pRed, (HTuple*)type, (HTuple*)&hWidth, (HTuple*)&hHeight);
		}
		else{
			GetImagePointer3(*phImage, (HTuple*)&pRed, (HTuple*)&pGreen, (HTuple*)&pBlue, (HTuple*)type, (HTuple*)&hWidth, (HTuple*)&hHeight);
        }
    }
    catch (...)
    {
...
bool hobjectToMat(HObject& Hobj, cv::Mat& rImage)
{
    ...
    if (htChannels[0].I() == 1)
    {
		...
		//// 检查图像类型
		//if (cType != "byte") {
		//	std::cerr << "错误: 图像不是8位类型，当前类型: " << cType.S() << std::endl;
		//	return -1;
		//}
		...
		//memcpy(rImage.data, (uchar*)ptr[0].I(), width[0].I() * height[0].I()); //挂在这一步报错
		//
		...
		//GetImagePointer1(Hobj, (HTuple*)&ptr, &cType, (HTuple*)&width, (HTuple*)&height);
		//GetImagePointer1(Hobj, (HTuple*)&ptr, &cType, &width, &height);
		//rImage.create(height, width, CV_8UC(1));

  //      for (int i = 0; i < height; i++)
  //      {
  //          memcpy(rImage.ptr() + rImage.step * i, (uchar*)ptr[0].I() + width * i, width.I());
  //      }
    }
    if (htChannels[0].I() == 3)
    {
		...
		//uchar* pHalconData = reinterpret_cast<uchar*>(ptrValue);  // 将整数值转换为指针
		//uchar* pOpencvData = rImage.data;
  ...
  //      ptrRed = ptrGreen = ptrBlue = NULL;
		...
		////GetImagePointer3(Hobj, (HTuple*)&ptrRed, (HTuple*)&ptrGreen, (HTuple*)&ptrBlue, &cType, &width, &height);   // 卡在这一步报错
  //      rImage.create(height, width, CV_8UC(3));
