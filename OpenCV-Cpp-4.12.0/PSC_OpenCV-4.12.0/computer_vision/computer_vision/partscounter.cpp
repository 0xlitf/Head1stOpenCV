...
PartsCounter::PartsCounter() : m_errorStatus(0) {
    ///////////////////////////////////////projectPath
    // 读取系统参数
    ...
    m_xCompensate =
        -20; // 物料允许原地向后滚动最大距离。 单位：pix像素。 一般不用修改。
    m_openElementSize =
        15; // 闭合物料之间间隙。
    // 值设置的太小，物料反光倒影容易作为杂料误计。值设置太大，挨得近的物料会算成一个大杂料。单位：pix像素

    m_matchMethod =
        2; // 检测方法。  0 比较轮廓相似度，1 比较尺寸（按误差百分比计算），2
    // 比较尺寸（按像素值计算）。 默认选2，一般不用修改。
    m_similarity = 0.95; // 物料轮廓和训练模型的相似度。取值范围0.0~1.0，值越大，检测精度越高。
    // 只在 m_matchMethod = 0 时起作用
    m_Lcoefficient = 0.8; // 物料长宽尺寸允许偏小百分比。取值范围0.0~1.0，值越大，检测精度越高。
    // 只在 m_matchMethod = 1 时起作用
    m_Hcoefficient = 1.2; // 物料长宽尺寸允许偏大百分比。取值范围1.0~2.0，值越小，检测精度越高。
    // 只在 m_matchMethod = 1 时起作用
    m_AdiffValue =
        0.2; // 面积大小误差百分比。  取值范围0.0~1.0，值越小，检测精度越高。
    // 不论哪种 m_matchMethod 方法都起作用。
    m_WdiffValue =
        8; // 物料长度尺寸允许误差像素范围。  单位：pix像素。
    // 取值范围>0，值越小，检测精度越高。 只在 m_matchMethod = 2 时起作用
    m_HdiffValue =
        8; // 物料宽度尺寸允许误差像素范围。  单位：pix像素。
    // 取值范围>0，值越小，检测精度越高。 只在 m_matchMethod = 2 时起作用
    m_roiLineLimitC =
        0.98; // ROI
    // 左边界起到指定百分比区域（按整个ROI宽比例算），该区域内只跟踪不识别。
    m_allowContourArea0 =
        30000; // 轮廓有效面积上限。面积超过该值的轮廓无效。 一般不用修改。
    m_allowContourArea1 =
        50; // 轮廓有效面积下限。面积低于该值的轮廓无效。 一般不用修改。
    m_detectLineC =
        20 * mySysParam.imgWidth /
                    320; // ROI 前边界后移 m_detectLineC 个像素，该区域内只跟踪不识别。
    m_MinScore =
        0.55; // halcon模版匹配算法最小匹配分值。根据物料轮廓相似度要求设定。取值范围0.0~1.0，值越大，匹配度要求更高，检测精度越高。
    m_MinDefectInnerRadian =
        4; // halcon精细检测算法缺陷部分最小内圆半径。  单位：pix像素。
    // 取值范围>0，值越小，检测精度越高。如果值太小，一小点缺胶都会判断为不合格。一般不用修改。
...
bool PartsCounter::updateProjectHP(std::string strPath, int trainID) {
    ...
    m_trainArea.insert(m_trainArea.end(), trainData.trainArea.begin(),
                       trainData.trainArea.end());
    m_trainWidth.insert(m_trainWidth.end(), trainData.trainWidth.begin(),
                        trainData.trainWidth.end());
    m_trainHeight.insert(m_trainHeight.end(), trainData.trainHeight.begin(),
                         trainData.trainHeight.end());
    m_trainContours.insert(m_trainContours.end(), trainData.trainContours.begin(),
                           trainData.trainContours.end());
...
bool PartsCounter::updateRoi(std::string strPath) {
    ...
    if (!ptr_HPDetectionPartsCounter->updateROI(m_roi.roiLTX, m_roi.roiLTY,
                                                m_roi.roiWidth, m_roi.roiHeight))
        return false;
...
void PartsCounter::drawObjData(cv::Mat *image, bool showImg) {
    ...
    if (m_num && showImg) {
        ...
        m_RoiImage = m_image(
            Range(m_roi.roiLTY, min(m_image.rows, m_roi.roiLTY + m_roi.roiHeight)),
            Range(m_roi.roiLTX, min(m_image.cols, m_roi.roiLTX + m_roi.roiWidth)));
        ...
        while (itContours != m_objContour.end()) {
            ...
            // 获取ROI图像
            obj_data.objRoiImage =
                m_RoiImage(Range(ri.y, min(m_RoiImage.rows, ri.y + ri.height)),
                                              Range(ri.x, min(m_RoiImage.cols, ri.x + ri.width)));
        ...
        // 画轮廓
        drawContours(*image, m_objContour, -1, color, 2, 8, noArray(), INT_MAX,
                     Point(m_roi.roiLTX, m_roi.roiLTY));
        ...
        // 画轮廓
        drawContours(*image, m_impurityContour, -1, color, 2, 8, noArray(), INT_MAX,
                     Point(m_roi.roiLTX, m_roi.roiLTY));
...
bool PartsCounter::autoSetRoi(Mat *image, int &roiLTX, int &roiLTY,
...
void PartsCounter::segmentObj(Mat *image, bool showImg) {
    ...
    // m_openElementSize));    //MORPH_ELLIPSE
    Mat element1 = getStructuringElement(
        MORPH_RECT, Size(m_openElementSize, 2)); // MORPH_ELLIPSE
    // Mat element2 = getStructuringElement(MORPH_RECT, Size(1,
    // m_openElemesntSize*0.5)); 创建3*3的圆形结构元素 Mat element1 =
    // getStructuringElement(MORPH_RECT, Size(1, 8));
    ...
    findContours(outThreshold, m_contours, hierarchy, CV_RETR_EXTERNAL,
    ...
    while (itContours != m_contours.end()) {
        ...
        // 如果目标面积太小或太大，均予以剔除
        if (((conArea > m_roi.roiWidth * m_roi.roiHeight) ||
             (conArea < m_allowContourArea1))) {
            ...
            if ((ri.height < 4) &&
                ((ri.y < 5) || (ri.y + ri.height > m_roi.roiHeight - 5))) {
                itContours = m_contours.erase(itContours);
                itHierarchy = hierarchy.erase(itHierarchy);
            } else {
                ++itContours;
    ...
    for (int j = 0; j < m_contours[i].size(); j++)
    {
            if (m_contours[i][j].x >= _rightPoint.x){
                    _rightPoint = m_contours[i][j];        //取最右轮廓点作为
    m_center if (m_contours[i][j].y < _rightPoint.y)  _rightPoint =
    m_contours[i][j];        //取最右上轮廓点作为 m_center
            }
    }
    m_center[i] = Point(_rightPoint.x + m_roi.roiLTX, _rightPoint.y);
    //将ROI裁剪部分补偿回来，获得在原图上的x坐标
    */
...
void PartsCounter::estimateObj() {
    ...
    for (int i = 0; i < num; ++i) {
        if ((m_objWidth[i] > 1.3 * maxTrainWidth) ||
            (m_objHeight[i] > maxTrainHeight + 10) ||
            (m_objWidth[i] > 0.9 * m_roi.roiWidth)) {
            // 物料过大，设置为连体杂料
            m_halcon_flag[i] = 3;
        } else if ((m_useHardTriggerMode == false) &&
                   ((((m_center[i].x - m_tempWidth[i]) <
                                                         m_roi.roiLTX + m_detectLineC) &&
                                                        (m_tempWidth[i] + m_detectLineC < 0.9 * m_roi.roiWidth)) ||
            ...
            for (int j = 0; j < m_trainContours.size(); ++j) {
                ...
                if ((COMP_METHOD_MATHCONTOURS == m_matchMethod)) {
                    double matchScore = matchShapes(m_contours[i], m_trainContours[j],
                                                    CV_CONTOURS_MATCH_I3, 0);
                    if ((((1 - matchScore) >= m_similarity) && ((1 - matchScore) < 1) ||
                         ((matchScore > 1))) &&
                        (abs(m_trainHeight[j] - m_objHeight[i]) < m_HdiffValue) &&
                        (abs(m_trainWidth[j] - m_objWidth[i]) < m_WdiffValue)) {
                        float tempArea =
                            m_trainArea[j] +
                                         ((m_objHeight[i] - m_trainHeight[j] - 1) / 2 *
                                                           m_trainWidth[j]) +
                                         ((m_objWidth[i] - m_trainWidth[j] - 1) / 2 * m_trainHeight[j]);
                        // 如果目标面积范围在
                        // 模板面积+/-公差面积（现为0.15）以内，则视为合格料 if ((((tempArea
                        // - m_objArea[i]< m_allowContourArea1*2)) && ((tempArea +
                ...
                if ((COMP_METHOD_MATHAREA1 == m_matchMethod)
                    //			&& ((m_Lcoefficient * m_trainArea[j] <
                    //m_objArea[i]) && (m_Hcoefficient * m_trainArea[j] > m_objArea[i]))
                    && ((m_Lcoefficient * m_trainHeight[j] < m_objHeight[i]) &&
                        (m_Hcoefficient * m_trainHeight[j] > m_objHeight[i])) &&
                    ((m_Lcoefficient * m_trainWidth[j] < m_objWidth[i]) &&
                                                                                                                                          (m_Hcoefficient * m_trainWidth[j] > m_objWidth[i]) &&
                                                                                                                                          (m_center[i].x - (m_tempWidth[i] / 2) >= 5))) {
                    float tempArea =
                        m_trainArea[j] +
                                     ((m_objHeight[i] - m_trainHeight[j] - 1) / 2 * m_trainWidth[j]) +
                                     ((m_objWidth[i] - m_trainWidth[j] - 1) / 2 * m_trainHeight[j]);
                    // 如果目标面积范围在
                    // 模板面积+/-公差面积（现为0.15）以内，则视为合格料 if ((((tempArea -
                    // m_objArea[i]< m_allowContourArea1*3)) && ((tempArea + m_objArea[i])
                    // > m_allowContourArea1*3)))
                ...
                if (COMP_METHOD_MATHAREA2 == m_matchMethod) {
                    if ((abs(m_trainHeight[j] - m_objHeight[i]) < m_HdiffValue) &&
                        (abs(m_trainWidth[j] - m_objWidth[i]) < m_WdiffValue)) {
                        // 计算模板面积=
            ...
            if ((m_halcon_flag[i] > 0) && (min_aDiffValue < 0.9))
                std::cout << "aDiffValue should less than ：" << min_aDiffValue
                          << std::endl;
        }
    }
...
void PartsCounter::hPDetectionEstimateObj() {
    ...
    if (m_lastallcontoursNum - m_allcontoursNum > 0) {
        for (int i = 0; i < m_allcontoursNum; i++) {
            ...
            if ((is_merged == -1) &&
                ((m_center[i].x - m_tempWidth[i] > m_roi.roiLTX + 5) ||
                                      (m_halcon_flag[i] == 3))) {
                for (unsigned int j = 1; j < m_lastallcontoursNum; j++) {
                    if (((fabs(m_objArea[i] - m_lastObjArea[j] - m_lastObjArea[j - 1]) <
                          100) ||
                         (m_halcon_flag[i] == 3)) // 合并前后面积之和相近是判断依据
                        && (m_objArea[i] - m_lastObjArea[j] >
                            100) // 合并后，料的面积必须有所增加
                        && (m_preCenter[j].x - m_center[i].x > -5) &&
                        ((m_preCenter[j - 1].x - m_lastTempWidth[j - 1] >
                                                                        m_roi.roiLTX + 5) ||
                                                                       (m_halcon_flag[i] == 3)) // 合并前，前一个料不能是正在出视野
                        && (m_preCenter[j].x - m_preCenter[j - 1].x <
                            m_lastTempWidth[j] + 20)) // 合并前两个料距离不能太远
                    {
                        is_merged = i;
                        merge_match = j - 1;
                        break;
    ...
    if (m_lastallcontoursNum - m_allcontoursNum < 0) {
        for (int i = 0; i < m_lastallcontoursNum; i++) {
            if ((is_departed == -1) &&
                (m_preCenter[i].x - m_lastTempWidth[i] > m_roi.roiLTX + 5)) {
                for (unsigned int j = m_allcontoursNum - 1; j > 0; j--) {
                    if ((fabs(m_lastObjArea[i] - m_objArea[j] - m_objArea[j - 1]) <
                         100) &&
                        (m_preCenter[i].x - m_center[j].x > -5) &&
                        (m_center[j - 1].x - m_tempWidth[j - 1] >
                                                                                                                                        m_roi.roiLTX + 5) // 分裂后，前一个料不能是正在出视野
                        && (m_center[j].x - m_center[j - 1].x <
                            m_tempWidth[j] + 20)) // 分开后两个料距离不能太远
                    {
                        is_departed = i;
                        departed_match = j - 1;
    ...
    for (int i = m_allcontoursNum - 1; i >= 0; i--) {
        ...
        for (int j = m_lastallcontoursNum - 1; j >= 0; j--) {
            if ((is_departed >= 0) && (i == departed_match)) // 物料分离
            {
                ...
                g[i][j] = -1;
            } else if ((abs(m_preCenter[j].y - m_center[i].y) < m_yRange + 5) &&
                       (abs(m_preCenter[j].x - m_center[i].x) < m_width * 0.3) &&
                       (m_preCenter[j].x - m_center[i].x > -10) &&
                       (m_preCenter[j].x < m_roi.roiLTX + m_roi.roiWidth - 5) &&
                       ((fabs(m_lastObjArea[j] - m_objArea[i]) <
                                                                                                                                                                                                                                                    max(m_lastObjArea[j], m_objArea[i]) * 0.5) &&
                                                                                                                                                                                                                                                   (m_center[i].x - m_tempWidth[i] >
                                                                                                                                                                                                                                                                                                                                           m_roi.roiLTX + 5)) // 面积差别不能太大，防止0,1位置互换
                       && (m_center[i].x < m_roi.roiLTX + m_roi.roiWidth - 5)) {
                g[i][j] = 1;
            } else if ((m_center[i].x - m_preCenter[m_lastallcontoursNum - 1].x >
                        10) // 在上一帧最后一个料之后新进视野的
                       || (abs(m_preCenter[j].x - m_center[i].x) >=
                           m_width * 0.3)                         // 距离相差过远的
                       || (m_center[i].x - m_preCenter[j].x > 10) // 料不会往后退
                       || ((fabs(m_lastObjArea[j] - m_objArea[i]) >
                            max(m_lastObjArea[j], m_objArea[i]) * 0.7) &&
                           (m_center[i].x - m_tempWidth[i] >
                                                                                                                   m_roi.roiLTX + 5)) // 面积差别不能太大，防止0,1位置互换
                       || (m_center[i].x >
                           m_roi.roiLTX + m_roi.roiWidth - 5)) // 还未进入视野的
            {
                g[i][j] = -1; // 以上一律不匹配
            } else {
                g[i][j] = 0;
            }
            if ((min_y > abs(m_preCenter[j].y - m_center[i].y)) &&
                (m_preCenter[j].x - m_center[i].x >
                                                                    -10)) { // 不但要y坐标接近，还要是往前移动。
    ...
    for (int i = 0; i < num; ++i) {
        ...
        if (m_center[i].x < m_roi.roiLTX + m_roi.roiWidth - 5) {
            ...
            if ((m_halcon_flag[i] == 0) &&
                ((m_center[i].x - m_tempWidth[i]) >=
                                            m_roi.roiLTX)) // 要求目标轮廓的左端点未出图像左边界5像素
            {
                ...
                if (cx[i] >= 0) {
                    if (3 == m_preFlag[cx[i]] &&
                        is_departed <
                                                     0) { // 上一帧m_preFlag =3，且在出视野过程(或者)。保持计杂料。
                        NG3_OK = true;
            ...
            m_errorStatus = 17;
            // 如果目标轮廓数超过上一帧中轮廓数（主要包括“正在新进入视野右侧过程中的料”和“一分为二多出来的料”），超过部分均默认设置m_Flag
            // =2

            // 如果该料未到视野左边界，即正在跑出视野过程中，则进一步做判断
            if ((m_center[i].x - m_tempWidth[i] > m_roi.roiLTX + 5)) {
                if (m_lastallcontoursNum == 0) {
                    if ((m_halcon_flag[i] == 2)) // 如果算法检测为边界料，还是认为是杂料.
                    // 上一帧没有料，突然出现的算NGEnding
                    {
                        m_impurityContour.push_back(m_contours[i]);
                        m_Flag[i] = 4;
                        continue;
                    }
                    // 如果上帧中没有物料，则直接设置新物料为m_Flag = 2,
                    // 避免视野中突然出现东西被计数
                    // m_Flag[i] = 2;
                    // continue;
                }
            ...
            if (cx[i] >= 0) // 匹配成功的
            {
                if (0 == m_preFlag[cx[i]]) {
                    ...
                    m_impurityContour.push_back(m_contours[i]);
                    if ((m_center[i].x - m_tempWidth[i] >
                         m_roi.roiLTX +
                                                              m_detectLineC)) // 如果没到传送带尾端，则以m_halcon_flag为准，否则以m_preFlag为准
                        m_Flag[i] = m_halcon_flag[i];
                    else
                ...
                for (int j = m_lastallcontoursNum - 1; j >= 0; j--) {
                    if ((cy[j] < 0) && (j < lastMatchNo) &&
                        (m_preCenter[j].x - m_center[i].x - m_xRange > -15) &&
                        (m_xRange > 0) &&
                        (m_objArea[i] > 100)) { // 上次匹配过的，这次不再匹配
                        lastMatchNo = j;
                        ...
                        if (0 == m_preFlag[j]) {
                            ...
                            m_impurityContour.push_back(m_contours[i]);
                            if ((m_center[i].x - m_tempWidth[i] >
                                 m_roi.roiLTX + m_detectLineC)) // && !NG3_OK
                                m_Flag[i] = m_halcon_flag[i];
                            else
                                m_Flag[i] = m_preFlag[j];
                        }
                        break;
                    } else if ((is_departed >= 0) &&
                               (i == departed_match)) { // 如果是分离料，
                        // 未匹配上的又未能识别的打杂料
                        m_impurityContour.push_back(m_contours[i]);
                        m_Flag[i] = 1;
                        break;
            ...
            if ((m_halcon_flag[i] == 1) ||
                (m_halcon_flag[i] ==
                                            3)) // 如果算法检测为杂料，就认为是杂料. 上一帧本来就是3，现在也是3
            {
                m_impurityContour.push_back(m_contours[i]);
                m_Flag[i] = m_halcon_flag[i];
                continue;
            ...
            if ((m_halcon_flag[i] ==
                 2)) // 如果算法检测为边界料，还是认为是杂料.
            // 上一帧有被跟踪关联，则不会流转到该位置，未被跟踪上则作杂料处理
            {
                m_impurityContour.push_back(m_contours[i]);
                m_Flag[i] = 4;
                continue;
    ...
    // m_err_num = m_num - m_obj_num;   //
    // 视野内杂料数量，进出视野的物料暂定归属于为不合格料（20250215发现未进视野的料算不合格的话，会导致objData无数据）
    ...
    if (m_lastcontoursNum > 0) {
        ...
        if (m_contoursNum > 0) {
            ...
            if (m_lastcontoursNum > m_contoursNum) {
                // 如果上帧杂质数和合格料数 均未增加
                //  					m_targetNumber =
                //  static_cast<unsigned int>(m_lastObjContours.size()) -
                //  static_cast<unsigned int>(m_objContour.size());
                // 					m_errorNumber =
                // static_cast<unsigned int>(m_lastImpurityContours.size()) -
                // static_cast<unsigned int>(m_impurityContour.size());
                for (int i = 0; i < m_lastallcontoursNum; i++) {
                    if ((i < merge_match) || is_merged == -1) {
                        // 则上一帧中，在当前帧最左料之前的料，视为已出视野并计数（增加限制条件：且该料必须从左视野边界处消失才计数）
                        if ((m_preCenter[i].x - m_lastTempWidth[i] < m_roi.roiLTX + 50) &&
                            (m_preCenter[i].x < m_roi.roiLTX + m_width * 0.3)) {
                            int _tmpXCompensate = m_xRange + 30;
                            for (int j = 0; j < m_center.size(); j++) {
                                if (((m_preCenter[i].x - m_center[j].x) > -10) && cy[i] >= 0) {
                                    _tmpXCompensate =
                                        m_xCompensate /
                                                      4; // 如果是物料y坐标接近，且x坐标相近，则认为可能是同一物料
                                    break;
                                } else if (
                                    (abs(m_preCenter[i].x - m_center[j].x - m_xRange) <
                                     15)) // 如果y坐标不相近，仅x坐标相近，原则上认为是不同物料。
                                {
                                    if (m_targetNumber ==
                                        m_lastcontoursNum -
                                                              m_contoursNum) // 如果计数已达到物料消失个数，则计数不再增加
                                    {
            ...
            m_lastImpurityContours.assign(m_impurityContour.begin(),
                                          m_impurityContour.end());
        ...
        ////////////////////上帧有料，当前帧无料，全部计数//////////////////////
            ...
            for (int a = 0; a < m_lastallcontoursNum; a++) {
                ...
                if ((m_preCenter[a].x - m_lastTempWidth[a] <
                     m_roi.roiLTX + m_detectLineC) &&
                    (m_preCenter[a].x < m_roi.roiLTX + m_width * 0.3)) {
                    // 若当前帧还是存在料（即m_Flag=2的那一类料）,如果该料不是位于视野左边界（即正在出视野过程中）则不计数
                    if ((2 == m_preFlag[a]) &&
                        (m_preCenter[a].x - m_lastTempWidth[a] > m_roi.roiLTX + 5)) {
                        continue;
                    }
                    // 剩下的正常计数
                    if (0 == m_preFlag[a])
...
void PartsCounter::actionHPDetectionObjectMat(cv::Mat *image, bool showImg) {
    ...
    for (unsigned int i = 0; i < m_num; i++) {
        ...
        if (m_halconDLModel == DL_ANOMALY_DETECTION) {
            ...
            if (data[i].isOK_halcon >
                0) // 分类和检测，modelID＞0都认为匹配到物料了，判断为OK，未匹配到判断为NG,modelID为0
    ...
    // 计算合格料和杂质料总数目
    m_err_num =
        m_num - m_obj_num; // 视野内杂料数量，进出视野的物料暂定归属于为不合格料
