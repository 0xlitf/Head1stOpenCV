...
public:
    ...
    bool autoSetRoi(cv::Mat *image, int &roiLTX, int &roiLTY, int &roiWidth,
                    int &roiHeight); // 自动更新ROI    Edit by pmh 20170516
    ...
    std::pair<unsigned int, unsigned int> getObjectNumber() {
        return make_pair(m_obj_num, m_err_num);
    } // 获取当前帧检测到的合格物料和杂料个数（无论是否合格）
    void action(cv::Mat *image, bool showImg = false); // 主函数
    void actionTransObject(double data[][3], int num); // 透明料
    void actionHPDetectionObjectMat(cv::Mat *image,
                                    bool showImg = false); // halcon精细缺陷检测
    ...
    void setUseInsideDetection(
        bool flag = true); // 设置内轮廓检测开关，默认false 不打开
...
private:
    ...
    std::vector<unsigned int>
        m_Flag; // 物料识别标志  0-合格，1-杂质，2-其他（不作判断）
    ...
    int m_contoursNum, m_lastcontoursNum, m_allcontoursNum,
        m_lastallcontoursNum; // 当前帧目标轮廓数， 上一帧目标轮廓数
...
#define MAXN 20      // MAXN表示X集合和Y集合顶点个数的最大值
    ...
    int cx[MAXN],
        cy[MAXN]; // cx[i],表示最终求得的最大匹配中，与x集合中元素Xi匹配的集合Y中顶点的索引
    // cy[i],表示最终求得的最大匹配中，与y集合中元素Yi匹配的集合X中顶点的索引
    int mk
        [MAXN]; // DFS算法中记录顶点访问状态的数据mk[i]=0表示未访问过，为1表示访问过
};
