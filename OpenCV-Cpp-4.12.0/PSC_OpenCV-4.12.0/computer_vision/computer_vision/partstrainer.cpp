...
bool PartsTrainer::training(cv::Mat* trainImg, Mat* trainResult, int thresh)
{
	...
	line(*trainResult, Point(m_roiLTX + m_roiWidth / 2, 0), Point(m_roiLTX + m_roiWidth / 2, mySysParam.imgHeight), Scalar(0, 0, 255));        // ROI中轴线-竖线
	line(*trainResult, Point(0, m_roiLTY + (m_roiHeight / 2)), Point(mySysParam.imgWidth, m_roiLTY + (m_roiHeight / 2)), Scalar(0, 0, 255));   // ROI中轴线-横线
...
bool PartsTrainer::writeTrainDataToXml()
{
	...
	for (int i = 0; i < resultList.size(); ++i)
	{
		...
		if ((m_isDynamic == false) || (resultList[i].train_count >= m_trainNum))      // 训练次数超过3次的就保存结果
		{
