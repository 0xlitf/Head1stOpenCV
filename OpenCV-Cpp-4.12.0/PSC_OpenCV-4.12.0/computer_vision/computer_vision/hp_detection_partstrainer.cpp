...
void HPDetectionPartsTrainer::init() {
    ...
    try {
        ...
        // pDevProcedure =
        // std::make_shared<HDevProcedure>("generate_shape_model_gray");
        pDevProcedure =
            std::make_shared<HDevProcedure>("generate_shape_model_dynamic");
...
void HPDetectionPartsTrainer::setTrainTarget(int num) { m_trainNumHD = num; }

void HPDetectionPartsTrainer::uninit() { CloseWindow(m_pData->WindowHandle); }

std::string HPDetectionPartsTrainer::project() { return m_pData->strProject; }

void HPDetectionPartsTrainer::setProject(std::string strProject) {
    m_pData->strProject = strProject;
}

bool HPDetectionPartsTrainer::start(std::string strProject,
                                    std::string roiPath) {
#ifndef HALCONCPlus
    ...
    GenRectangle1(&m_pData->RegionROI, m_roiLTY, m_roiLTX, m_roiLTY + m_roiHeight,
                  m_roiLTX + m_roiWidth);
...
void HPDetectionPartsTrainer::stop() { m_pData->bTraining = false; }

bool HPDetectionPartsTrainer::isTraining() { return m_pData->bTraining; }
...
int HPDetectionPartsTrainer::getTrainedObjNum() {
    ...
    if (m_pData->bTraining) {
        ...
        for (int i = 0; i < m_pData->resultList.size(); i++) {
            if ((m_isDynamicHD == 0) ||
                (m_pData->resultList[i].train_count >= m_trainNumHD)) {
                cnt++;
            }
        }
        return cnt;
...
std::string HPDetectionPartsTrainer::errorString() { return m_pData->strError; }
...
bool HPDetectionPartsTrainer::train(cv::Mat &image, cv::Mat &trainResult) {
    ...
    HTuple hModelID, hModelWidth, hModelHeight, hModelArea, hModelGrayVal,
        isTrained;
    HTuple Channels;
    ...
    try {
        ...
        if (m_halconDLModel == DL_MODE_CLOSED) {
            /*generate_shape_model(hImage, m_pData->RegionROI, &hModelRegionsOrigin,
         m_pData->WindowHandle, m_pData->ModelWidthAll, m_pData->ModelHeightAll,
         m_pData->hThreshold, &hModelID, &hModelWidth, &hModelHeight,
         &hModelArea, &isTrained);*/

            generate_shape_model_dynamic(
                hImage, m_pData->RegionROI, m_pData->hModelRegions,
                &hModelRegionsOrigin, m_pData->WindowHandle, m_pData->ModelIDs,
                m_pData->ModelWidthAll, m_pData->ModelHeightAll,
                m_pData->ModelGrayAll, m_pData->hThreshold, m_pData->hWdiffValue,
                m_isDynamicHD, &hModelID, &hModelWidth, &hModelHeight, &hModelArea,
                &hModelGrayVal, &isTrained);
        }
#else
        pDevProcedureCall->SetInputIconicParamObject("ImageModel",
                                                     hImage); // 输入图像 image 类型
        pDevProcedureCall->SetInputIconicParamObject(
            "RegionROI", m_pData->RegionROI); // ROI检测区域 region 类型
        pDevProcedureCall->SetInputIconicParamObject(
            "ModelRegions", m_pData->hModelRegions); // 已训练模型 region 类型
        pDevProcedureCall->SetInputCtrlParamTuple(
            "WindowHandle", m_pData->WindowHandle); // 窗口句柄 WindowHandle 类型
        pDevProcedureCall->SetInputCtrlParamTuple(
            "ModelIDs", m_pData->ModelIDs); // 已训练模型ID tuple 类型
        pDevProcedureCall->SetInputCtrlParamTuple(
            "ModelGrays", m_pData->ModelGrayAll); // 已训练模型平均灰度 tuple 类型

        pDevProcedureCall->SetInputCtrlParamTuple(
            "ModelWidthAll",
            m_pData->ModelWidthAll); // 已训练模型宽度尺寸 tuple 类型
        pDevProcedureCall->SetInputCtrlParamTuple(
            "ModelHeightAll",
            m_pData->ModelHeightAll); // 已训练模型高度尺寸 tuple 类型
        pDevProcedureCall->SetInputCtrlParamTuple(
            "Threshold", m_pData->hThreshold); // 图像分割阈值参数 tuple 类型
        pDevProcedureCall->SetInputCtrlParamTuple(
            "WdiffValue", m_pData->hWdiffValue); // 图像分割阈值参数 tuple 类型
        pDevProcedureCall->SetInputCtrlParamTuple(
            "IsDynamic", m_isDynamicHD); // 是否采用动态训练模式 tuple 类型
        pDevProcedureCall->Execute();    // 执行算法

        hModelRegionsOrigin = pDevProcedureCall->GetOutputIconicParamObject(
            "ModelRegionsOrigin"); // 获取算子的输出变量 ModelRegionsOrigin
        hModelID = pDevProcedureCall->GetOutputCtrlParamTuple(
            "ModelID"); // 获取算子的输出变量 ModelID
        hModelWidth = pDevProcedureCall->GetOutputCtrlParamTuple(
            "ModelWidth"); // 获取算子的输出变量 ModelWidth
        hModelHeight = pDevProcedureCall->GetOutputCtrlParamTuple(
            "ModelHeight"); // 获取算子的输出变量 ModelHeight
        hModelArea = pDevProcedureCall->GetOutputCtrlParamTuple(
            "ModelArea"); // 获取算子的输出变量 ModelHeight
        hModelGrayVal = pDevProcedureCall->GetOutputCtrlParamTuple(
            "ModelGrayVal"); // 获取算子的输出变量 ModelGrayVal
        isTrained = pDevProcedureCall->GetOutputCtrlParamTuple(
            "isTrained"); // 获取算子的输出变量 ModelID
#endif
        ...
        DumpWindowImage(
            &hImageResult,
            m_pData->WindowHandle); // 把算子输出图像保存下来，转成QImage outImage
        ...
        if (isTrained[0].I() == 1) {
            HTuple Area, CenterRow, CenterColumn, CenterRow2, CenterColumn2,
                AngleCheck, Score, Model, OriginalClipRegion;
            ...
            FindShapeModels(hImage, hModelID, 0, HTuple(360).TupleRad(), 0.01, 1, 0.2,
                            "least_squares", 0, 0.9, &CenterRow, &CenterColumn,
                            &AngleCheck, &Score, &Model);
            MoveRegion(hModelRegionsOrigin, &hModelRegionsOrigin,
                       mySysParam.imgHeight / 2 - CenterRow,
                       mySysParam.imgWidth / 2 - CenterColumn);
            ConcatObj(m_pData->hModelRegions, hModelRegionsOrigin,
                      &m_pData->hModelRegions);
            ...
            TupleConcat(m_pData->ModelHeightAll, hModelHeight,
                        &m_pData->ModelHeightAll);
            ...
            m_pData->resultData.train_count =
                0; // 重置光纤计数器，掉下才计数，刚创建不计数
            HP_matchID = hModelID;
            saveTrainData();
            return true;
    ...
    if (m_isOutBoundryHD == 0) {
        for (int i = 0; i < m_pData->resultList.size(); ++i) {
            if (0 != (int(HTuple(m_pData->resultList[i].modelID) == HP_matchID))) {
                ...
                std::cout << "HP_matchID: " << HP_matchID
                          << "  train_count:" << m_pData->resultList[i].train_count
                          << std::endl;
            }
        }
    }
...
bool HPDetectionPartsTrainer::finish() {
    ...
    try {
        ...
        for (int i = 0; i < m_pData->resultList.size(); ++i) {
            std::cout << "trainingID: " << i << "         accumulate times:"
                      << m_pData->resultList[i].train_count << std::endl;
            if ((m_isDynamicHD == 0) || (m_pData->resultList[i].train_count >=
                                         m_trainNumHD)) // 训练次数超过3次的就保存结果
            {
                string writePath =
                    strPath + "/train_out_" + to_string(indexID) + ".jpg";
                cv::imwrite(writePath.c_str(),
                            m_pData->resultList[i].image); // 保存训练图片 todo
                WriteRegion(m_pData->resultList[i].region,
                            (strPath + "/region_" + to_string(indexID) + ".reg")
                                .data()); // 保存所有物料区域 region
                WriteShapeModel(m_pData->resultList[i].modelID,
                                (strPath + "/model_" + to_string(indexID) + ".shm")
                                    .data()); // 保存所有匹配模版 modelID
                cv::Point dummyPoint;
                dummyPoint.x = 1;
                dummyPoint.y = 1;
                ...
                train_result.trainArea.push_back(
                    int(m_pData->resultList[i].modelArea[0].D()));
                train_result.trainWidth.push_back(
                    int(m_pData->resultList[i].modelWidth[0].D() * 2));
                train_result.trainHeight.push_back(
                    int(m_pData->resultList[i].modelHeight[0].D() * 2));
...
// extern "C" TRANSPARENT_PARTSHARED_EXPORT CTransparentPartsTrainer *
// createTransparentPartsTrainer()
//{
