...
void add_colormap_to_image(HObject ho_GrayValueImage, HObject ho_Image,
                           HObject *ho_ColoredImage,
    ...
    //
    // This procedure adds a gray-value image to a RGB image with a chosen color
    // map.
    //
    ...
    if (0 != (int(hv_Type == HTuple("real")))) {
        ...
        throw HException(
                    HTuple("For this transformation, a byte or real image is needed!"));
    }
    //
    // Apply the chosen color scheme on the gray value.
    apply_colorscheme_on_gray_value_image(ho_GrayValueImage, &ho_RGBValueImage,
    ...
    GenEmptyObj(&ho_ChannelsScaled);
    {
        ...
        HTuple step_val19 = 1;
        for (hv_ChannelIndex = 1; hv_ChannelIndex.Continue(end_val19, step_val19);
             hv_ChannelIndex += step_val19) {
            SelectObj(ho_Channels, &ho_Channel, hv_ChannelIndex);
            MinMaxGray(ho_Channel, ho_Channel, 0, &hv_ChannelMin, &hv_ChannelMax,
                       &hv__);
            scale_image_range(ho_Channel, &ho_ChannelScaled, hv_ChannelMin,
                              hv_ChannelMax);
...
void apply_colorscheme_on_gray_value_image(HObject ho_InputImage,
    ...
    if (0 != (int(hv_Schema == HTuple("jet")))) {
        ...
        hv_A1 = -4.0;
        hv_A0 = 255.0 + 128.0;
        hv_R = (((((hv_X - hv_OffR).TupleAbs()) * hv_A1) + hv_A0).TupleMax2(hv_Low))
                .TupleMin2(hv_High);
        hv_G = (((((hv_X - hv_OffG).TupleAbs()) * hv_A1) + hv_A0).TupleMax2(hv_Low))
                .TupleMin2(hv_High);
        hv_B = (((((hv_X - hv_OffB).TupleAbs()) * hv_A1) + hv_A0).TupleMax2(hv_Low))
                .TupleMin2(hv_High);
        //
...
void calculate_dl_image_zoom_factors(
        HTuple hv_ImageWidth, HTuple hv_ImageHeight, HTuple hv_TargetWidth,
        HTuple hv_TargetHeight, HTuple hv_DLPreprocessParam,
        HTuple *hv_ZoomFactorWidth, HTuple *hv_ZoomFactorHeight) {
    ...
    //
    // Aspect-ratio preserving zoom is supported for model type 'ocr_detection'
    // only.
    CreateDict(&hv___Tmp_Ctrl_Dict_Init_0);
    SetDictTuple(hv___Tmp_Ctrl_Dict_Init_0, "comp", "ocr_detection");
    hv_PreserveAspectRatio =
            (hv_DLPreprocessParam.TupleConcat(hv___Tmp_Ctrl_Dict_Init_0))
            .TupleTestEqualDictItem("model_type", "comp");
...
// Chapter: Deep Learning / Model
// Short Description: Check the content of the parameter dictionary
// DLPreprocessParam.
void check_dl_preprocess_param(HTuple hv_DLPreprocessParam) {
    ...
    GetDictParam(hv_DLPreprocessParam, "key_exists", "check_params",
                 &hv_KeyExists);
    if (0 != hv_KeyExists) {
    ...
    // catch (Exception)
        ...
        throw HException(HTuple(HTuple("DLPreprocessParam needs the parameter: '") +
                                "model_type") +
                         "'");
    }
    //
    ...
    if (0 != (HTuple(int(hv_Index == -1)).TupleOr(int(hv_Index == HTuple())))) {
        throw HException(HTuple(
                             "Only models of type '3d_gripping_point_detection', "
                             "'anomaly_detection', 'classification', 'detection', "
                             "'gc_anomaly_detection', 'multi_label_classification', "
                             "'ocr_recognition', 'ocr_detection' or 'segmentation' are supported"));
        return;
    }
    //
    ...
    if (0 != (HTuple(int(hv_DLModelType == HTuple("segmentation")))
              .TupleOr(int(hv_DLModelType ==
                           HTuple("3d_gripping_point_detection"))))) {
    ...
    //
    GetDictParam(hv_DLPreprocessParam, "key_exists", hv_ParamNames,
                 &hv_KeysExists);
    if (0 != (int(((hv_KeysExists.TupleEqualElem(0)).TupleSum()) > 0))) {
        {
            ...
            for (hv_I = 0; hv_I.Continue(end_val54, step_val54); hv_I += step_val54) {
                ...
                if (0 != (hv_Exists.TupleNot())) {
                    throw HException(("DLPreprocessParam needs the parameter: '" +
                                      HTuple(hv_ParamNames[hv_I])) +
                                     "'");
                }
            }
        }
    ...
    GetDictParam(hv_DLPreprocessParam, "keys", HTuple(), &hv_InputKeys);
    {
        ...
        for (hv_I = 0; hv_I.Continue(end_val64, step_val64); hv_I += step_val64) {
            ...
            if (0 != (int(hv_Indices == -1))) {
                throw HException(("Unknown key for DLPreprocessParam: '" +
                                  HTuple(hv_InputKeys[hv_I])) +
                                 "'");
                return;
            }
            ...
            if (0 != (int((hv_ValidTypes.TupleLength()) > 0))) {
                {
                    ...
                    HTuple step_val97 = 1;
                    for (hv_V = 0; hv_V.Continue(end_val97, step_val97);
                         hv_V += step_val97) {
                        ...
                        if (0 != (int(hv_T == HTuple("int")))) {
                            ...
                            if (0 != (hv_IsInt.TupleNot())) {
                                ...
                                if (0 != (int((hv_ValidTypes.TupleLength()) < 2))) {
                                    ...
                                    hv_ValidTypesListing =
                                            (((hv_ValidTypes.TupleSelectRange(
                                                   0, HTuple(0).TupleMax2(
                                                       (hv_ValidTypes.TupleLength()) - 2))) +
                                              HTuple(", ")) +
                                             HTuple(hv_ValidTypes[(hv_ValidTypes.TupleLength()) - 1]))
                                            .TupleSum();
                                }
                                throw HException(((((("The value given in the key '" + hv_Key) +
                                                     "' of DLPreprocessParam is invalid. Valid "
                                                     "types are: ") +
                                                    hv_ValidTypesListing) +
                                                   ". The given value was '") +
                                                  hv_Value) +
            ...
            if (0 != (int((hv_ValidValues.TupleLength()) > 0))) {
                ...
                if (0 != (int(hv_Index == -1))) {
                    ...
                    if (0 != (int((hv_ValidValues.TupleLength()) < 2))) {
                        ...
                        hv_ValidValueListing =
                                (((hv_ValidValues.TupleSelectRange(
                                       0, HTuple(0).TupleMax2((hv_ValidValues.TupleLength()) -
                                                              2))) +
                                  HTuple(", ")) +
                                 (hv_EmptyStrings.TupleConcat(HTuple(
                                                                  hv_ValidValues[(hv_ValidValues.TupleLength()) - 1]))))
                                .TupleSum();
                    }
                    throw HException(
                                ((((("The value given in the key '" + hv_Key) +
                                    "' of DLPreprocessParam is invalid. Valid values are: ") +
                                   hv_ValidValueListing) +
                                  ". The given value was '") +
                                 hv_Value) +
                                "'.");
    ...
    if (0 != (HTuple(HTuple(int(hv_DLModelType == HTuple("classification")))
                     .TupleOr(int(hv_DLModelType ==
                                  HTuple("multi_label_classification"))))
              .TupleOr(int(hv_DLModelType == HTuple("detection"))))) {
        // Check ImageRangeMin and ImageRangeMax.
        GetDictParam(hv_DLPreprocessParam, "key_exists", "image_range_min",
                     &hv_ImageRangeMinExists);
        GetDictParam(hv_DLPreprocessParam, "key_exists", "image_range_max",
                     &hv_ImageRangeMaxExists);
        ...
        if (0 != hv_ImageRangeMinExists) {
            ...
            if (0 != (int(hv_ImageRangeMin != -127))) {
                throw HException(("For model type " + hv_DLModelType) +
                                 " ImageRangeMin has to be -127.");
            }
        }
    ...
    if (0 != (HTuple(int(hv_DLModelType == HTuple("segmentation")))
        ...
        GetDictParam(hv_DLPreprocessParam, "key_exists",
                     hv_ParamNamesDetectionOptional, &hv_KeysExists);
        // If they are present, check that they are [].
        {
            ...
            for (hv_IndexParam = 0; hv_IndexParam.Continue(end_val157, step_val157);
                 hv_IndexParam += step_val157) {
                if (0 != (HTuple(hv_KeysExists[hv_IndexParam]))) {
                    GetDictTuple(hv_DLPreprocessParam,
                                 HTuple(hv_ParamNamesDetectionOptional[hv_IndexParam]),
                                 &hv_Value);
                    if (0 != (int(hv_Value != HTuple()))) {
                        throw HException(
                                    ((("The preprocessing parameter '" +
                                      ...
                                      "' was set to ") +
                                     hv_Value) +
                                    HTuple(" but for segmentation it should be set to [], as it is "
        ...
        GetDictTuple(hv_DLPreprocessParam, "set_background_id",
                     &hv_SetBackgroundID);
        if (0 != (HTuple(int(hv_SetBackgroundID != HTuple()))
                  .TupleAnd(int(hv_DLModelType ==
                                HTuple("3d_gripping_point_detection"))))) {
            throw HException(
                        HTuple(HTuple("The preprocessing parameter '") +
                               "set_background_id") +
                        HTuple("' should be set to [] for 3d_gripping_point_detection, as it "
                               "is not used for this method."));
        }
        ...
        GetDictTuple(hv_DLPreprocessParam, "class_ids_background",
                     &hv_ClassIDsBackground);
        if (0 != (HTuple(int(hv_ClassIDsBackground != HTuple()))
                  .TupleAnd(int(hv_DLModelType ==
                                HTuple("3d_gripping_point_detection"))))) {
            throw HException(
                        HTuple(HTuple("The preprocessing parameter '") +
        ...
        if (0 !=
                (HTuple(HTuple(int((hv_SetBackgroundID.TupleLength()) > 0))
                        .TupleAnd(
                            HTuple(int((hv_ClassIDsBackground.TupleLength()) > 0))
                            .TupleNot()))
                 .TupleOr(
                     HTuple(int((hv_ClassIDsBackground.TupleLength()) > 0))
                     .TupleAnd(
                         HTuple(int((hv_SetBackgroundID.TupleLength()) > 0))
                         .TupleNot())))) {
            throw HException("Both keys 'set_background_id' and "
                             "'class_ids_background' are required.");
        }
        // Check that 'class_ids_background' and 'set_background_id' are disjoint.
        if (0 != (int((hv_SetBackgroundID.TupleLength()) > 0))) {
            TupleIntersection(hv_SetBackgroundID, hv_ClassIDsBackground,
                              &hv_Intersection);
            if (0 != (hv_Intersection.TupleLength())) {
                throw HException("Class IDs in 'set_background_id' and "
                                 "'class_ids_background' need to be disjoint.");
            }
        }
        // Check 'ignore_class_ids'.
        GetDictTuple(hv_DLPreprocessParam, "ignore_class_ids", &hv_IgnoreClassIDs);
        if (0 != (HTuple(int(hv_IgnoreClassIDs != HTuple()))
                  .TupleAnd(int(hv_DLModelType ==
                                HTuple("3d_gripping_point_detection"))))) {
            throw HException(
                        HTuple(HTuple("The preprocessing parameter '") + "ignore_class_ids") +
                        HTuple("' should be set to [] for 3d_gripping_point_detection, as it "
                               "is not used for this method."));
        }
        ...
        hv_KnownClasses.Append(hv_ClassIDsBackground);
        {
            ...
            for (hv_I = 0; hv_I.Continue(end_val194, step_val194);
                 hv_I += step_val194) {
                hv_IgnoreClassID = HTuple(hv_IgnoreClassIDs[hv_I]);
                TupleFindFirst(hv_KnownClasses, hv_IgnoreClassID, &hv_Index);
                if (0 != (HTuple(int((hv_Index.TupleLength()) > 0))
                          .TupleAnd(int(hv_Index != -1)))) {
                    throw HException(
                                "The given 'ignore_class_ids' must not be included in the "
                                "'class_ids_background' or 'set_background_id'.");
                }
            }
        }
    } else if (0 != (int(hv_DLModelType == HTuple("detection")))) {
        // Check if segmentation specific parameters are set.
        GetDictParam(hv_DLPreprocessParam, "key_exists", hv_ParamNamesSegmentation,
                     &hv_KeysExists);
        // If they are present, check that they are [].
        {
            HTuple end_val205 = (hv_ParamNamesSegmentation.TupleLength()) - 1;
            HTuple step_val205 = 1;
            for (hv_IndexParam = 0; hv_IndexParam.Continue(end_val205, step_val205);
                 hv_IndexParam += step_val205) {
                if (0 != (HTuple(hv_KeysExists[hv_IndexParam]))) {
                    GetDictTuple(hv_DLPreprocessParam,
                                 HTuple(hv_ParamNamesSegmentation[hv_IndexParam]),
                                 &hv_Value);
                    if (0 != (int(hv_Value != HTuple()))) {
                        throw HException(
                                    ((("The preprocessing parameter '" +
                                      ...
                                      "' was set to ") +
                                     hv_Value) +
                                    HTuple(" but for detection it should be set to [], as it is "
                                           "not used for this method."));
                    }
                }
            }
        ...
        GetDictParam(hv_DLPreprocessParam, "key_exists",
                     hv_ParamNamesDetectionOptional, &hv_OptionalKeysExist);
        if (0 != (HTuple(hv_OptionalKeysExist[0]))) {
            // Check 'instance_type'.
            GetDictTuple(hv_DLPreprocessParam,
                         HTuple(hv_ParamNamesDetectionOptional[0]), &hv_InstanceType);
            if (0 !=
                    (int((((HTuple("rectangle1").Append("rectangle2")).Append("mask"))
                          .TupleFind(hv_InstanceType)) == -1))) {
                throw HException(
                            ("Invalid generic parameter for 'instance_type': " +
                             hv_InstanceType) +
                            HTuple(", only 'rectangle1' and 'rectangle2' are allowed"));
            }
        }
        // If instance_segmentation is set we might overwrite the instance_type for
        // the preprocessing.
        if (0 != (HTuple(hv_OptionalKeysExist[3]))) {
            GetDictTuple(hv_DLPreprocessParam,
                         HTuple(hv_ParamNamesDetectionOptional[3]),
                         &hv_IsInstanceSegmentation);
            if (0 != (int(((((HTuple(1).Append(0)).Append("true")).Append("false"))
                           .TupleFind(hv_IsInstanceSegmentation)) == -1))) {
                throw HException(
                            ("Invalid generic parameter for 'instance_segmentation': " +
                             hv_IsInstanceSegmentation) +
                            HTuple(", only true, false, 'true' and 'false' are allowed"));
            }
        }
        if (0 != (HTuple(hv_OptionalKeysExist[1]))) {
            // Check 'ignore_direction'.
            GetDictTuple(hv_DLPreprocessParam,
                         HTuple(hv_ParamNamesDetectionOptional[1]),
                         &hv_IgnoreDirection);
            if (0 !=
                    (int(((HTuple(1).Append(0)).TupleFind(hv_IgnoreDirection)) == -1))) {
                throw HException(("Invalid generic parameter for 'ignore_direction': " +
                                  hv_IgnoreDirection) +
                                 HTuple(", only true and false are allowed"));
            }
        }
        if (0 != (HTuple(hv_OptionalKeysExist[2]))) {
            // Check 'class_ids_no_orientation'.
            GetDictTuple(hv_DLPreprocessParam,
                         HTuple(hv_ParamNamesDetectionOptional[2]),
                         &hv_ClassIDsNoOrientation);
            TupleSemTypeElem(hv_ClassIDsNoOrientation, &hv_SemTypes);
            if (0 != (HTuple(int(hv_ClassIDsNoOrientation != HTuple()))
                      .TupleAnd(int(
                                    ((hv_SemTypes.TupleEqualElem("integer")).TupleSum()) !=
                                    (hv_ClassIDsNoOrientation.TupleLength()))))) {
                throw HException(
                            ("Invalid generic parameter for 'class_ids_no_orientation': " +
                             hv_ClassIDsNoOrientation) +
                            HTuple(", only integers are allowed"));
            } else {
                if (0 != (HTuple(int(hv_ClassIDsNoOrientation != HTuple()))
                          .TupleAnd(int(
                                        ((hv_ClassIDsNoOrientation.TupleGreaterEqualElem(0))
                                         .TupleSum()) !=
                                        (hv_ClassIDsNoOrientation.TupleLength()))))) {
                    throw HException(
                                ("Invalid generic parameter for 'class_ids_no_orientation': " +
                                 hv_ClassIDsNoOrientation) +
                                HTuple(", only non-negative integers are allowed"));
                }
            }
        }
    }
...
void compute_normals_xyz(HObject ho_x, HObject ho_y, HObject ho_z,
                         HObject *ho_NXImage, HObject *ho_NYImage,
                         HObject *ho_NZImage, HTuple hv_Smoothing) {
    ...
    // Smooth
    //-> 5 is used as it is used in surface_normals_object_model_3d -
    //'xyz_mapping'
    if (0 != hv_Smoothing) {
...
// Short Description: This procedure converts Deep OCR Detection results to an
// Object Detection results.
void convert_ocr_detection_result_to_object_detection(
        HTuple hv_OcrResults, HTuple *hv_DetectionResults) {
    ...
    TupleGenConst(hv_OcrResults.TupleLength(), HTuple::TupleConstant("HNULL"),
                  &(*hv_DetectionResults));
    {
        HTuple end_val6 = (hv_OcrResults.TupleLength()) - 1;
        HTuple step_val6 = 1;
        for (hv_Index = 0; hv_Index.Continue(end_val6, step_val6);
             hv_Index += step_val6) {
            hv_OcrResult = HTuple(hv_OcrResults[hv_Index]);
            // Check if input is valid
            GetDictParam(hv_OcrResult, "key_exists", "words", &hv_RequiredKeysExist);
            if (0 != (int((hv_RequiredKeysExist.TupleSum()) !=
                          (hv_RequiredKeysExist.TupleLength())))) {
                throw HException(("The item at Index " + hv_Index) +
                                 " is not a valid Deep OCR Detection Result");
            }
            GetDictParam(
                        hv_OcrResult.TupleGetDictTuple("words"), "key_exists",
                        ((((HTuple("row").Append("col")).Append("phi")).Append("length1"))
                         .Append("length2")),
            ...
            GetDictParam(hv_OcrResult.TupleGetDictTuple("words"), "key_data_type",
                         "row", &hv___Tmp_Ctrl_Type);
            if (0 != (int(hv___Tmp_Ctrl_Type == HTuple("object")))) {
                SetDictObject(
                            (hv_OcrResult.TupleGetDictTuple("words")).TupleGetDictObject("row"),
                            hv_DetectionResult, "bbox_row");
            } else {
                SetDictTuple(
                            hv_DetectionResult, "bbox_row",
                            (hv_OcrResult.TupleGetDictTuple("words")).TupleGetDictTuple("row"));
            }
            GetDictParam(hv_OcrResult.TupleGetDictTuple("words"), "key_data_type",
                         "col", &hv___Tmp_Ctrl_Type);
            if (0 != (int(hv___Tmp_Ctrl_Type == HTuple("object")))) {
                SetDictObject(
                            (hv_OcrResult.TupleGetDictTuple("words")).TupleGetDictObject("col"),
                            hv_DetectionResult, "bbox_col");
            } else {
                SetDictTuple(
                            hv_DetectionResult, "bbox_col",
                            (hv_OcrResult.TupleGetDictTuple("words")).TupleGetDictTuple("col"));
            }
            GetDictParam(hv_OcrResult.TupleGetDictTuple("words"), "key_data_type",
                         "phi", &hv___Tmp_Ctrl_Type);
            if (0 != (int(hv___Tmp_Ctrl_Type == HTuple("object")))) {
                SetDictObject(
                            (hv_OcrResult.TupleGetDictTuple("words")).TupleGetDictObject("phi"),
                            hv_DetectionResult, "bbox_phi");
            } else {
                SetDictTuple(
                            hv_DetectionResult, "bbox_phi",
                            (hv_OcrResult.TupleGetDictTuple("words")).TupleGetDictTuple("phi"));
            }
            GetDictParam(hv_OcrResult.TupleGetDictTuple("words"), "key_data_type",
                         "length1", &hv___Tmp_Ctrl_Type);
            if (0 != (int(hv___Tmp_Ctrl_Type == HTuple("object")))) {
                SetDictObject((hv_OcrResult.TupleGetDictTuple("words"))
                              .TupleGetDictObject("length1"),
                              hv_DetectionResult, "bbox_length1");
            } else {
                SetDictTuple(hv_DetectionResult, "bbox_length1",
                             (hv_OcrResult.TupleGetDictTuple("words"))
                             .TupleGetDictTuple("length1"));
            }
            GetDictParam(hv_OcrResult.TupleGetDictTuple("words"), "key_data_type",
                         "length2", &hv___Tmp_Ctrl_Type);
            if (0 != (int(hv___Tmp_Ctrl_Type == HTuple("object")))) {
                SetDictObject((hv_OcrResult.TupleGetDictTuple("words"))
                              .TupleGetDictObject("length2"),
                              hv_DetectionResult, "bbox_length2");
            } else {
                SetDictTuple(hv_DetectionResult, "bbox_length2",
                             (hv_OcrResult.TupleGetDictTuple("words"))
                             .TupleGetDictTuple("length2"));
            }
            SetDictTuple(
                        hv_DetectionResult, "bbox_confidence",
                        HTuple(
                            (hv_DetectionResult.TupleGetDictTuple("bbox_row")).TupleLength(),
                            1.0));
            SetDictTuple(
                        hv_DetectionResult, "bbox_class_id",
                        HTuple(
                            (hv_DetectionResult.TupleGetDictTuple("bbox_row")).TupleLength(),
                            0));
...
// Short Description: Convert the parameters of rectangles with format
// rectangle2 to the coordinates of its 4 corner-points.
void convert_rect2_5to8param(HTuple hv_Row, HTuple hv_Col, HTuple hv_Length1,
                             HTuple hv_Length2, HTuple hv_Phi, HTuple *hv_Row1,
                             HTuple *hv_Col1, HTuple *hv_Row2, HTuple *hv_Col2,
                             HTuple *hv_Row3, HTuple *hv_Col3, HTuple *hv_Row4,
                             HTuple *hv_Col4) {
...
// Chapter: Tools / Geometry
// Short Description: Convert for four-sided figures the coordinates of the 4
// corner-points to the parameters of format rectangle2.
void convert_rect2_8to5param(HTuple hv_Row1, HTuple hv_Col1, HTuple hv_Row2,
                             HTuple hv_Col2, HTuple hv_Row3, HTuple hv_Col3,
                             HTuple hv_Row4, HTuple hv_Col4,
                             HTuple hv_ForceL1LargerL2, HTuple *hv_Row,
                             HTuple *hv_Col, HTuple *hv_Length1,
    ...
    // Length1 and Length2.
    (*hv_Length1) = ((((hv_Row1 - hv_Row2) * (hv_Row1 - hv_Row2)) +
                      ((hv_Col1 - hv_Col2) * (hv_Col1 - hv_Col2)))
                     .TupleSqrt()) /
            2.0;
    (*hv_Length2) = ((((hv_Row2 - hv_Row3) * (hv_Row2 - hv_Row3)) +
                      ((hv_Col2 - hv_Col3) * (hv_Col2 - hv_Col3)))
                     .TupleSqrt()) /
            2.0;
    ...
    if (0 != hv_ForceL1LargerL2) {
        ...
        hv_IdxSwap =
                (((*hv_Length2) - (*hv_Length1)).TupleGreaterElem(1e-9)).TupleFind(1);
        if (0 != (int(hv_IdxSwap != -1))) {
            ...
            hv_Hor[hv_IdxSwap] =
                    HTuple(hv_Col2[hv_IdxSwap]) - HTuple(hv_Col3[hv_IdxSwap]);
            hv_Vert[hv_IdxSwap] =
                    HTuple(hv_Row3[hv_IdxSwap]) - HTuple(hv_Row2[hv_IdxSwap]);
        }
    }
...
void create_dl_preprocess_param(
        HTuple hv_DLModelType, HTuple hv_ImageWidth, HTuple hv_ImageHeight,
        HTuple hv_ImageNumChannels, HTuple hv_ImageRangeMin,
        HTuple hv_ImageRangeMax, HTuple hv_NormalizationType,
        HTuple hv_DomainHandling, HTuple hv_IgnoreClassIDs,
    ...
    //
    // This procedure creates a dictionary with all parameters needed for
    // preprocessing.
    //
    ...
    SetDictTuple((*hv_DLPreprocessParam), "image_num_channels",
                 hv_ImageNumChannels);
    ...
    SetDictTuple((*hv_DLPreprocessParam), "normalization_type",
                 hv_NormalizationType);
    ...
    if (0 != (HTuple(int(hv_DLModelType == HTuple("segmentation")))
              .TupleOr(int(hv_DLModelType ==
                           HTuple("3d_gripping_point_detection"))))) {
        SetDictTuple((*hv_DLPreprocessParam), "ignore_class_ids",
                     hv_IgnoreClassIDs);
        SetDictTuple((*hv_DLPreprocessParam), "set_background_id",
                     hv_SetBackgroundID);
        SetDictTuple((*hv_DLPreprocessParam), "class_ids_background",
                     hv_ClassIDsBackground);
    }
    //
    ...
    if (0 != (int(hv_GenParam != HTuple()))) {
        GetDictParam(hv_GenParam, "keys", HTuple(), &hv_GenParamNames);
        {
            ...
            HTuple step_val36 = 1;
            for (hv_GenParamIndex = 0;
                 hv_GenParamIndex.Continue(end_val36, step_val36);
                 hv_GenParamIndex += step_val36) {
                GetDictTuple(hv_GenParam, HTuple(hv_GenParamNames[hv_GenParamIndex]),
                             &hv_GenParamValue);
                SetDictTuple((*hv_DLPreprocessParam),
                             HTuple(hv_GenParamNames[hv_GenParamIndex]),
                             hv_GenParamValue);
            }
        }
    }
    ...
    if (0 != (int(hv_DLModelType == HTuple("detection")))) {
        GetDictParam((*hv_DLPreprocessParam), "key_exists",
                     ((HTuple("instance_type").Append("ignore_direction"))
                      .Append("instance_segmentation")),
                     &hv_KeysExist);
        ...
        if (0 != (HTuple(int(hv_InstanceType == HTuple("rectangle2")))
                  .TupleAnd(HTuple(hv_KeysExist[1]).TupleNot()))) {
        ...
        if (0 != (HTuple(hv_KeysExist[2]))) {
            GetDictTuple((*hv_DLPreprocessParam), "instance_segmentation",
                         &hv_IsInstanceSegmentation);
            if (0 != (int(((((HTuple(1).Append(0)).Append("true")).Append("false"))
                           .TupleFind(hv_IsInstanceSegmentation)) == -1))) {
                throw HException(
            ...
            if (0 != (HTuple(int(hv_IsInstanceSegmentation == HTuple("true")))
                      .TupleOr(int(hv_IsInstanceSegmentation == 1)))) {
...
// Chapter: Deep Learning / Model
// Short Description: Create a dictionary with the preprocessing parameters
// based on a given DL model.
void create_dl_preprocess_param_from_model(HTuple hv_DLModelHandle,
                                           HTuple hv_NormalizationType,
                                           HTuple hv_DomainHandling,
                                           HTuple hv_SetBackgroundID,
                                           HTuple hv_ClassIDsBackground,
                                           HTuple hv_GenParam,
                                           HTuple *hv_DLPreprocessParam) {
    ...
    //
    // This procedure creates a dictionary with all parameters needed for
    // preprocessing according to a model provided through DLModelHandle.
    //
    ...
    if (0 !=
            (HTuple(int(hv_ModelType == HTuple("anomaly_detection")))
             .TupleOr(int(hv_ModelType == HTuple("gc_anomaly_detection"))))) {
        // No specific parameters for both anomaly detection
        // and Global Context Anomaly Detection model types.
    } else if (0 != (HTuple(int(hv_ModelType == HTuple("classification")))
                     .TupleOr(int(hv_ModelType ==
                                  HTuple("multi_label_classification"))))) {
        ...
        // If GenParam has not been created yet, create it to add new generic
        // parameters.
        ...
        // If the model can do instance segmentation, the preprocessing
        // instance_type needs to be 'mask'.
        GetDlModelParam(hv_DLModelHandle, "instance_segmentation",
                        &hv_IsInstanceSegmentation);
        ...
        // For instance_type 'rectangle2', add the boolean ignore_direction and
        // class IDs without orientation.
        if (0 != (int(hv_InstanceType == HTuple("rectangle2")))) {
            GetDlModelParam(hv_DLModelHandle, "ignore_direction",
                            &hv_IgnoreDirection);
            ...
            GetDlModelParam(hv_DLModelHandle, "class_ids_no_orientation",
                            &hv_ClassIDsNoOrientation);
            SetDictTuple(hv_GenParam, "class_ids_no_orientation",
                         hv_ClassIDsNoOrientation);
        }
    } else if (0 !=
               (HTuple(int(hv_ModelType == HTuple("ocr_detection")))
                .TupleOr(int(hv_ModelType == HTuple("ocr_recognition"))))) {
        ...
        GetDlModelParam(hv_DLModelHandle, "ignore_class_ids", &hv_IgnoreClassIDs);
               ...
               (int(hv_ModelType == HTuple("3d_gripping_point_detection")))) {
        // The input image is expected to be a single channel image.
        hv_ImageNumChannels = 1;
    } else {
        throw HException(
                    ("Current model type is not supported: \"" + hv_ModelType) + "\"");
    }
    //
    // Create the dictionary with the preprocessing parameters returned by this
    // procedure.
    create_dl_preprocess_param(
                hv_ModelType, hv_ImageWidth, hv_ImageHeight, hv_ImageNumChannels,
                hv_ImageRangeMin, hv_ImageRangeMax, hv_NormalizationType,
                hv_DomainHandling, hv_IgnoreClassIDs, hv_SetBackgroundID,
                hv_ClassIDsBackground, hv_GenParam, &(*hv_DLPreprocessParam));
    //
    return;
}

// Chapter: Deep Learning / Model
// Short Description: Crops a given image object based on the given domain
// handling.
void crop_dl_sample_image(HObject ho_Domain, HTuple hv_DLSample, HTuple hv_Key,
    ...
    if (0 != hv_KeyExists) {
        ...
        if (0 != ((hv_DLPreprocessParam.TupleConcat(hv___Tmp_Ctrl_Dict_Init_0))
                  .TupleTestEqualDictItem("domain_handling", "comp"))) {
            SmallestRectangle1(ho_Domain, &hv_Row1, &hv_Column1, &hv_Row2,
                               &hv_Column2);
            CropPart(hv_DLSample.TupleGetDictObject(hv_Key), &ho___Tmp_Obj_0, hv_Row1,
                     hv_Column1, (hv_Column2 - hv_Column1) + 1,
                     (hv_Row2 - hv_Row1) + 1);
...
void dev_close_window_dict(HTuple hv_WindowHandleDict) {
    ...
    hv_WindowHandleDicts = hv_WindowHandleDict;
    {
        ...
        HTuple step_val5 = 1;
        for (hv_WindowDictIdx = 0; hv_WindowDictIdx.Continue(end_val5, step_val5);
             hv_WindowDictIdx += step_val5) {
            ...
            GetDictParam(hv_WindowHandleDict, "keys", HTuple(), &hv_WindowHandleKeys);
            {
                ...
                HTuple step_val8 = 1;
                for (hv_Index = 0; hv_Index.Continue(end_val8, step_val8);
                     hv_Index += step_val8) {
                    try {
                        GetDictTuple(hv_WindowHandleDict,
                                     HTuple(hv_WindowHandleKeys[hv_Index]),
                                     &hv_WindowHandles);
                    }
                    // catch (Exception)
                    catch (HException &HDevExpDefaultException) {
                    ...
                    hv_RemovedWindowIndices = HTuple();
                    {
                        ...
                        HTuple step_val15 = 1;
                        for (hv_WindowHandleIndex = 0;
                             hv_WindowHandleIndex.Continue(end_val15, step_val15);
                             hv_WindowHandleIndex += step_val15) {
                            // Not every entry has to be a window handle, therefore use
                            // try-catch.
                            try {
                                // Call set_window_param to check if the handle is a window
                                // handle.
                                SetWindowParam(HTuple(hv_WindowHandles[hv_WindowHandleIndex]),
                                               "flush", "true");
                                HDevWindowStack::SetActive(
                                            HTuple(hv_WindowHandles[hv_WindowHandleIndex]));
                                if (HDevWindowStack::IsOpen())
                                    CloseWindow(HDevWindowStack::Pop());
                                hv_RemovedWindowIndices =
                                        hv_RemovedWindowIndices.TupleConcat(hv_WindowHandleIndex);
                            }
                            // catch (Exception)
                            catch (HException &HDevExpDefaultException) {
                                HDevExpDefaultException.ToHTuple(&hv_Exception);
                    ...
                    TupleRemove(hv_WindowHandles, hv_RemovedWindowIndices,
                                &hv_WindowHandles);
                    // If some entries remained, set reduced tuple. Otherwise, remove
                    // whole key entry.
                    if (0 != (int((hv_WindowHandles.TupleLength()) > 0))) {
                        SetDictTuple(hv_WindowHandleDict,
                                     HTuple(hv_WindowHandleKeys[hv_Index]),
                                     hv_WindowHandles);
                    } else {
                        RemoveDictKey(hv_WindowHandleDict,
                                      HTuple(hv_WindowHandleKeys[hv_Index]));
                    }
                }
            }
...
void dev_display_confidence_regions(HObject ho_ImageConfidence,
                                    HTuple hv_DrawTransparency,
                                    HTuple *hv_Colors) {
    ...
    // display resulting regions.
    {
        ...
        HTuple step_val15 = 1;
        for (hv_ColorIndex = 0; hv_ColorIndex.Continue(end_val15, step_val15);
             hv_ColorIndex += step_val15) {
            hv_Threshold = hv_ColorIndex * (1.0 / hv_NumColors);
            hv_MinGray = hv_Threshold;
            hv_MaxGray = hv_Threshold + (1 / hv_NumColors);
            Threshold(ho_ImageConfidence, &ho_Region, hv_Threshold,
                      hv_Threshold + (1.0 / hv_NumColors));
            if (HDevWindowStack::IsOpen())
                SetColor(HDevWindowStack::GetActive(),
                         HTuple(hv_WeightsColorsAlpha[hv_ColorIndex]));
            if (HDevWindowStack::IsOpen())
...
// Chapter: Deep Learning / Model
// Short Description: Visualize different images, annotations and inference
// results for a sample.
void dev_display_dl_data(HTuple hv_DLSample, HTuple hv_DLResult,
                         HTuple hv_DLDatasetInfo, HTuple hv_KeysForDisplay,
    ...
    HTuple hv_AddDisplayKey, hv_AnomalyLabelGroundTruth,
    ...
    HTuple hv__, hv_MaxHeight, hv_PluralPostfix,
    ...
    //
    // This procedure displays the content of the provided DLSample and/or
    // DLResult depending on the input string KeysForDisplay. DLDatasetInfo is a
    // dictionary containing the information about the dataset. The visualization
    // can be adapted with GenParam.
    //
    ...
    //
    // By default, show class IDs and color frames for classification ground
    // truth/results.
    ...
    //
    // By default, show direction of the ground truth/results instances for
    // detection with instance_type 'rectangle2'.
    ...
    if (0 != (int(hv_GenParam != HTuple()))) {
        GetDictParam(hv_GenParam, "keys", HTuple(), &hv_GenParamNames);
        {
            ...
            HTuple step_val74 = 1;
            for (hv_ParamIndex = 0; hv_ParamIndex.Continue(end_val74, step_val74);
                 hv_ParamIndex += step_val74) {
                ...
                GetDictParam(hv_GenParam, "key_data_type", hv_GenParamName,
                             &hv___Tmp_Ctrl_Type);
                if (0 != (int(hv___Tmp_Ctrl_Type == HTuple("object")))) {
                    SetDictObject(hv_GenParam.TupleGetDictObject(hv_GenParamName),
                                  hv_Params, hv_GenParamName);
                } else {
                    SetDictTuple(hv_Params, hv_GenParamName,
                                 hv_GenParam.TupleGetDictTuple(hv_GenParamName));
                }
            }
        }
    }
    //
    if (0 != (HTuple(int((hv_DLSample.TupleLength()) > 1))
              .TupleOr(int((hv_DLResult.TupleLength()) > 1)))) {
        throw HException(
                    "Only a single dictionary for DLSample and DLResult is allowed");
    }
    //
    ...
    //
    //** Convert a Deep OCR Detection result to an Object Detection result if
    //necessary ***
    //
    hv_IsOCRDetection = 0;
    ...
    hv_KeyIndex = 0;
    {
        ...
        for (hv_KeyIndex = 0; hv_KeyIndex.Continue(end_val117, step_val117);
             hv_KeyIndex += step_val117) {
            // Check if Deep OCR Detection case
            TupleStrstr(HTuple(hv_KeysForDisplay[hv_KeyIndex]), "ocr_detection",
                        &hv_IsOCRDetection);
            ...
            if (0 != hv_IsOCRDetection) {
                ...
                if (0 != (int(hv_DLResult != HTuple()))) {
                    ...
                    convert_ocr_detection_result_to_object_detection(hv_OcrResult,
                                                                     &hv_DLResult);
                    GetDictParam(hv_DLResult, "keys", HTuple(), &hv_ResultKeys);
                    break;
                }
    ...
    if (0 != (int(hv_DLDatasetInfo == HTuple()))) {
        dev_display_dl_data_get_max_class_id(hv_DLSample, &hv_MaxClassIdSample,
                                             &hv_EmptySample);
        dev_display_dl_data_get_max_class_id(hv_DLResult, &hv_MaxClassIdResult,
                                             &hv_EmptyResult);
        ...
        if (0 != (int(hv_DLResult != HTuple()))) {
            ...
            if (0 != (HTuple(int((hv_ClassNameKey.TupleLength()) == 1))
                      .TupleAnd(int((hv_ClassIdKey.TupleLength()) == 1)))) {
                ...
                TupleUniq(HTuple(hv_ResultClassIds[hv_SortIndices]),
                          &hv_UniqueClassIds);
                TupleUniq(HTuple(hv_ResultClassNames[hv_SortIndices]),
                          &hv_UniqueClassNames);
                hv_ClassNames[hv_UniqueClassIds] = hv_UniqueClassNames;
            }
        }
        get_dl_class_colors(hv_ClassNames, hv_AdditionalGreenClassNames,
                            &hv_Colors);
        ...
        GetHandleParam(hv_DLDatasetInfo, "key_exists", hv_ClassKeys,
                       &hv_ClassKeysExist);
        if (0 != (int((hv_ClassKeysExist.TupleMin()) == 0))) {
            ...
            GetHandleParam(hv_DLDatasetInfo, "keys", HTuple(), &hv_DLDatasetInfoKeys);
            {
                ...
                for (hv_Index = 0; hv_Index.Continue(end_val180, step_val180);
                     hv_Index += step_val180) {
                    if (0 != (int((hv_DLDatasetInfoKeys.TupleFindFirst(
                                       HTuple(hv_ClassKeys[hv_Index]))) == -1))) {
                        throw HException(("Key " + HTuple(hv_ClassKeys[hv_Index])) +
                                         " is missing in DLDatasetInfo.");
                    }
                }
            }
            //
            ...
            CreateDict(&hv_ClassIDstoIndex);
            {
                ...
                for (hv_I = 0; hv_I.Continue(end_val196, step_val196);
                     hv_I += step_val196) {
    ...
    // catch (Exception)
        ...
        if (0 != (int(HTuple(hv_Exception[0]) == 1401))) {
            throw HException("WindowHandleDict has to be of type dictionary. Use "
                             "create_dict to create an empty dictionary.");
        } else {
            throw HException(hv_Exception);
        }
    }
    // For better usage, add meta information about the window handles in
    // WindowHandleDict.
    GetDictParam(hv_WindowHandleDict, "key_exists", "meta_information",
                 &hv_MetaInfoExists);
    if (0 != (hv_MetaInfoExists.TupleNot())) {
        CreateDict(&hv___Tmp_Ctrl_Dict_Init_2);
        SetDictTuple(hv_WindowHandleDict, "meta_information",
                     hv___Tmp_Ctrl_Dict_Init_2);
    ...
    GetDictParam(hv_WindowHandleDict, "keys", HTuple(), &hv_WindowHandleKeys);
    {
        ...
        for (hv_Index = 0; hv_Index.Continue(end_val228, step_val228);
             hv_Index += step_val228) {
            // Only consider the WindowHandleKeys that are needed for the current
            // visualization.
            hv_KeyIndex =
                    hv_KeysForDisplay.TupleFind(HTuple(hv_WindowHandleKeys[hv_Index]));
            if (0 != (HTuple(int(hv_KeyIndex != -1))
                      .TupleAnd(int(hv_KeyIndex != HTuple())))) {
                             ...
                             &hv_WindowHandles);
                {
                    ...
                    for (hv_WindowIndex = 0;
                         hv_WindowIndex.Continue(end_val233, step_val233);
                         hv_WindowIndex += step_val233) {
                        GetWindowParam(HTuple(hv_WindowHandles[hv_WindowIndex]), "flush",
                                       &hv_FlushValue);
                        hv_FlushValues = hv_FlushValues.TupleConcat(hv_FlushValue);
                        SetWindowParam(HTuple(hv_WindowHandles[hv_WindowIndex]), "flush",
                                       "false");
                    }
                }
            }
    ...
    while (0 != (int(hv_KeyIndex < (hv_KeysForDisplay.TupleLength())))) {
        ...
        if (0 != (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) == HTuple("image")))) {
            //
            // Image.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample,
                                HTuple(hv_KeysForDisplay[hv_KeyIndex]));
            //
            // Get or open next window.
            GetImageSize(ho_Image, &hv_WidthImage, &hv_HeightImage);
            get_next_window(
                        hv_Params.TupleGetDictTuple("font"),
                        hv_Params.TupleGetDictTuple("font_size"),
                        hv_Params.TupleGetDictTuple("display_bottom_desc"), hv_WidthImage,
                        hv_HeightImage, 0, hv_Params.TupleGetDictTuple("scale_windows"),
                        hv_Params.TupleGetDictTuple("threshold_width"),
                        hv_PrevWindowCoordinates, hv_WindowHandleDict,
                        HTuple(hv_KeysForDisplay[hv_KeyIndex]), &hv_CurrentWindowHandle,
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_ImageIDStringCapital,
                             "window", "bottom", "left", "white", "box", "false");
            }
        } else if (0 != (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                             HTuple("image_with_domain")))) {
            //
            // Image.
            ...
            if (HDevWindowStack::IsOpen())
                SetColor(HDevWindowStack::GetActive(),
                         hv_Params.TupleGetDictTuple("region_color"));
            if (HDevWindowStack::IsOpen())
                DispObj(ho_Image, HDevWindowStack::GetActive());
            if (HDevWindowStack::IsOpen())
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_ImageIDStringCapital,
                             ...
                             HTuple("gripping_confidence")))) {
            //
            // Confidence image.
            if (0 != (int(hv_DLResult == HTuple()))) {
                throw HException("DLResult dict is empty.");
            }
            if (0 != (int((hv_ResultKeys.TupleFind(
                               HTuple(hv_KeysForDisplay[hv_KeyIndex]))) != -1))) {
                GetDictObject(&ho_Image, hv_DLResult,
                              HTuple(hv_KeysForDisplay[hv_KeyIndex]));
            } else {
                throw HException(
                            ("Image with key '" + HTuple(hv_KeysForDisplay[hv_KeyIndex])) +
                            "' could not be found in DLResult.");
            }
            //
            // Get or open next window.
            GetImageSize(ho_Image, &hv_WidthImage, &hv_HeightImage);
            ...
            if (HDevWindowStack::IsOpen())
                DispText(HDevWindowStack::GetActive(), "Gripping confidence", "window",
            ...
            if (0 != (int((hv_ResultKeys.TupleFind("gripping_map")) != -1))) {
                ...
                throw HException(HTuple(HTuple("Image with key '") + "gripping_map") +
                                 "' could not be found in DLResult.");
            }
            //
            // Get or open next window.
            ...
            hv_ColorsGrippingGroundTruth.Append(
                        hv_Params.TupleGetDictTuple("gripping_point_map_color"));
            hv_ColorsGrippingGroundTruth.Append(
                        hv_Params.TupleGetDictTuple("gripping_point_background_color"));
            dev_display_segmentation_regions(
                        ho_GrippingMapImageResult, (HTuple(1).Append(0)),
                        hv_ColorsGrippingGroundTruth, HTuple(), &hv_ImageClassIDs);
            // Display gripping points.
            GetDictParam(hv_DLResult, "key_exists", "gripping_points",
                         &hv_GrippingPointsExists);
            if (0 != hv_GrippingPointsExists) {
                get_gripping_points_from_dict(hv_DLResult, &hv_Rows, &hv_Columns);
                GenCrossContourXld(&ho_Cross, hv_Rows, hv_Columns,
                                   hv_Params.TupleGetDictTuple("gripping_point_size"),
                                   0.785398);
                if (HDevWindowStack::IsOpen())
                    SetLineWidth(HDevWindowStack::GetActive(),
                                 hv_Params.TupleGetDictTuple("line_width"));
                if (HDevWindowStack::IsOpen())
                    SetColor(HDevWindowStack::GetActive(),
                             hv_Params.TupleGetDictTuple("gripping_point_color"));
                if (HDevWindowStack::IsOpen())
            ...
            //
                             ...
                             HTuple("gripping_map_ground_truth")))) {
            //
            // Image.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample, "image");
            //
            ...
            if (0 != (int((hv_SampleKeys.TupleFind("segmentation_image")) != -1))) {
                GetDictObject(&ho_GrippingMapGroundTruth, hv_DLSample,
                              "segmentation_image");
            } else {
                throw HException(
                            HTuple(HTuple("Image with key '") + "segmentation_image") +
                            "' could not be found in DLSample.");
            }
            //
            // Get or open next window.
            GetImageSize(ho_Image, &hv_WidthImage, &hv_HeightImage);
            ...
            dev_display_segmentation_regions(ho_GrippingMapGroundTruth, hv_ClassIDs,
                                             hv_ColorsGrippingGroundTruth, HTuple(),
                                             &hv_ImageClassIDs);
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_ImageIDStringCapital,
            ...
            if (HDevWindowStack::IsOpen())
                DispText(HDevWindowStack::GetActive(), "Gripping map ground truth",
                         "window", "top", "left", "black", "box", "true");
            //
        } else if (0 !=
                   (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) == HTuple("x")))) {
            //
            // X.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample,
                                HTuple(hv_KeysForDisplay[hv_KeyIndex]));
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                   ...
                   (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) == HTuple("xyz")))) {
            //
            // XYZ.
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                   ...
                   (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) == HTuple("y")))) {
            //
            // Y.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample,
                                HTuple(hv_KeysForDisplay[hv_KeyIndex]));
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                   ...
                   (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) == HTuple("z")))) {
            //
            // Z.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample,
                                HTuple(hv_KeysForDisplay[hv_KeyIndex]));
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_ImageIDStringCapital,
                             ...
                             HTuple("gripping_point_cloud")))) {
            //
            // In case of 'gripping_point_cloud' provide a window that can be used by
            // the procedure dev_display_dl_3d_data. No actual drawing happens  in
            // this procedure to keep it free from operators not belonging either to
            // the Foundation or Deep Learning license modules.
            GetDictParam(hv_DLSample, "keys", HTuple(), &hv_SampleKeys);
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample, "image");
            //
            // Get or open next window.
            ...
            //
        } else if (0 != (HTuple(HTuple(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                           HTuple("anomaly_both")))
                                .TupleOr(int(
                                             HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                             HTuple("anomaly_both_local"))))
                         .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                      HTuple("anomaly_both_global"))))) {
            //
            ...
            if (0 != (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                          HTuple("anomaly_both")))) {
                TupleRegexpReplace(hv_AnomalyImages, "image", "both",
                                   &hv_PossibleKeysForDisplay);
                hv_AddDisplayKey = 0;
                {
                    HTuple end_val469 = (hv_PossibleKeysForDisplay.TupleLength()) - 1;
                    HTuple step_val469 = 1;
                    for (hv_Index = 0; hv_Index.Continue(end_val469, step_val469);
                         hv_Index += step_val469) {
                        if (0 != (int((hv_KeysForDisplay.TupleFindFirst(HTuple(
                                                                            hv_PossibleKeysForDisplay[hv_Index]))) == -1))) {
                            hv_KeysForDisplay = hv_KeysForDisplay.TupleConcat(
                                        HTuple(hv_PossibleKeysForDisplay[hv_Index]));
                            hv_AddDisplayKey = 1;
                        }
                    }
                }
                //
                // Display not possible for 'anomaly_both' if key 'anomaly_image' is
                // missing.
                if (0 != (HTuple(int((hv_PossibleKeysForDisplay.TupleFindFirst(HTuple(
                                                                                   hv_KeysForDisplay[hv_KeyIndex]))) == -1))
                          .TupleAnd(hv_AddDisplayKey))) {
                    hv_KeyIndex += 1;
                    continue;
                }
            }
            ...
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample, "image");
            get_anomaly_ground_truth_label(hv_SampleKeys, hv_DLSample,
                                           &hv_AnomalyLabelGroundTruth,
                                           &hv_AnomalyLabelIDGroundTruth);
            //
            // Get the anomaly results either by applying the specified thresholds or
            // out of DLResult.
            TupleRegexpMatch(HTuple(hv_KeysForDisplay[hv_KeyIndex]),
                             "anomaly_both(.*)", &hv_AnomalyResultPostfix);
            get_anomaly_result(
                        &ho_AnomalyImage, &ho_AnomalyRegion, hv_DLResult,
                        hv_Params.TupleGetDictTuple("anomaly_classification_threshold"),
                        hv_Params.TupleGetDictTuple("anomaly_region_threshold"),
                        hv_AnomalyResultPostfix, &hv_AnomalyScore, &hv_AnomalyClassID,
            ...
            get_next_window(hv_Params.TupleGetDictTuple("font"),
                            hv_Params.TupleGetDictTuple("font_size"),
                            hv_Params.TupleGetDictTuple("display_bottom_desc"),
                            hv_WidthImage, hv_HeightImage, 0,
                            hv_Params.TupleGetDictTuple("scale_windows"),
                            hv_Params.TupleGetDictTuple("threshold_width"),
                            hv_PrevWindowCoordinates, hv_WindowHandleDict,
                            hv_WindowHandleName, &hv_CurrentWindowHandle,
                            &hv_WindowImageRatio, &hv_PrevWindowCoordinates);
            //
            ...
            if (0 != (hv_Params.TupleGetDictTuple(
                          "display_ground_truth_anomaly_regions"))) {
                dev_display_ground_truth_anomaly_regions(
                            hv_SampleKeys, hv_DLSample, hv_CurrentWindowHandle,
                            hv_Params.TupleGetDictTuple("line_width"),
                            hv_Params.TupleGetDictTuple("anomaly_region_label_color"),
                            hv_Params.TupleGetDictTuple("anomaly_color_transparency"),
                            &hv_AnomalyRegionGroundTruthExists);
            }
            //
            // Display anomaly regions defined by the specified threshold or out of
            // DLResult.
            CreateDict(&hv___Tmp_Ctrl_Dict_Init_3);
            SetDictTuple(hv___Tmp_Ctrl_Dict_Init_3, "comp", -1);
            if (0 !=
                    (HTuple(
                         ((hv_Params.TupleConcat(hv___Tmp_Ctrl_Dict_Init_3))
                          .TupleTestEqualDictItem("anomaly_region_threshold", "comp"))
                         .TupleNot())
                     .TupleOr(int((hv_ResultKeys.TupleFind(
                                       "anomaly_region" + hv_AnomalyResultPostfix)) !=
                                  -1)))) {
                dev_display_result_anomaly_regions(
                            ho_AnomalyRegion, hv_CurrentWindowHandle,
                            hv_Params.TupleGetDictTuple("line_width"),
                            hv_Params.TupleGetDictTuple("anomaly_region_result_color"));
            }
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "bottom",
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                hv_Text[hv_Text.TupleLength()] =
                        ((hv_AnomalyLabelIDGroundTruth + " : '") +
                         hv_AnomalyLabelGroundTruth) +
                        "'";
                if (0 != (HTuple(HTuple(int(hv_AnomalyRegionGroundTruthExists ==
                                            HTuple("false")))
                                 .TupleAnd(int(hv_AnomalyLabelIDGroundTruth == 1)))
                          .TupleAnd(hv_Params.TupleGetDictTuple(
                                        "display_ground_truth_anomaly_regions")))) {
                    hv_Text[hv_Text.TupleLength()] = "";
                ...
                if (0 !=
                        (((hv_Params.TupleConcat(hv___Tmp_Ctrl_Dict_Init_4))
                          .TupleTestEqualDictItem("anomaly_region_threshold", "comp"))
                         .TupleAnd(int((hv_ResultKeys.TupleFind(
                                            "anomaly_region" +
                                            hv_AnomalyResultPostfix)) == -1)))) {
                    hv_Text[hv_Text.TupleLength()] = "";
                ...
                hv_Text[hv_Text.TupleLength()] = "";
                hv_Text[hv_Text.TupleLength()] =
                        (("anomaly_score" + hv_AnomalyResultPostfix) + ": ") +
                        (hv_AnomalyScore.TupleString(".3f"));
                hv_Text[hv_Text.TupleLength()] = "";
                if (0 != (HTuple(int(hv_AnomalyClassThresholdDisplay != -1))
                          .TupleOr(int(hv_AnomalyRegionThresholdDisplay != -1)))) {
                    hv_Text[hv_Text.TupleLength()] = "---------------";
                ...
                if (0 != (int(hv_AnomalyClassThresholdDisplay != -1))) {
                    hv_Text[hv_Text.TupleLength()] =
                            "Classification: " +
                            (hv_AnomalyClassThresholdDisplay.TupleString(".3f"));
                    hv_Text[hv_Text.TupleLength()] = "";
                }
                if (0 != (int(hv_AnomalyRegionThresholdDisplay != -1))) {
                    hv_Text[hv_Text.TupleLength()] =
                            "Segmentation: " +
                            (hv_AnomalyRegionThresholdDisplay.TupleString(".3f"));
                    hv_Text[hv_Text.TupleLength()] = "";
                }
                // Get or open next child window
                get_child_window(hv_HeightImage, hv_Params.TupleGetDictTuple("font"),
                                 hv_Params.TupleGetDictTuple("font_size"), hv_Text,
                                 hv_PrevWindowCoordinates, hv_WindowHandleDict,
                                 hv_WindowHandleName, &hv_WindowImageRatio,
                ...
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             "left", hv_LineColors, "box", "false");
            }
            //
        } else if (0 != (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                             HTuple("anomaly_ground_truth")))) {
            // Image.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample, "image");
            get_anomaly_ground_truth_label(hv_SampleKeys, hv_DLSample,
                                           &hv_AnomalyLabelGroundTruth,
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                hv_Text[hv_Text.TupleLength()] =
                        ((hv_AnomalyLabelIDGroundTruth + " : '") +
                         hv_AnomalyLabelGroundTruth) +
                        "'";
                if (0 != (HTuple(HTuple(int(hv_AnomalyRegionExists == HTuple("false")))
                                 .TupleAnd(int(hv_AnomalyLabelIDGroundTruth == 1)))
                          .TupleAnd(hv_Params.TupleGetDictTuple(
                                        "display_ground_truth_anomaly_regions")))) {
                    hv_Text[hv_Text.TupleLength()] = "";
                ...
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             "left", "white", "box", "false");
                                           ...
                                           HTuple("anomaly_image")))
                                .TupleOr(int(
                                             HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                             HTuple("anomaly_image_local"))))
                         .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                      HTuple("anomaly_image_global"))))) {
            //
            // Image.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample, "image");
            //
            ...
            TupleRegexpMatch(hv_AnomalyImageKey, "anomaly_image(.*)",
            ...
            add_colormap_to_image(
                        ho_AnomalyImage, ho_Image, &ho_AnomalyImage,
            ...
            //
            TupleRegexpReplace(hv_AnomalyImageKey, "image", "score",
                               &hv_AnomalyScoreKey);
            //
            // Display the legend.
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                hv_Text[hv_Text.TupleLength()] = "";
                hv_Text[hv_Text.TupleLength()] =
                        (hv_AnomalyScoreKey + ": ") + (hv_AnomalyScore.TupleString(".3f"));
                // Get or open next child window
                get_child_window(hv_HeightImage, hv_Params.TupleGetDictTuple("font"),
                                 hv_Params.TupleGetDictTuple("font_size"), hv_Text,
                                 hv_PrevWindowCoordinates, hv_WindowHandleDict,
                ...
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             "left", "white", "box", "false");
                                           ...
                                           HTuple("anomaly_result")))
                                .TupleOr(int(
                                             HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                             HTuple("anomaly_result_local"))))
                         .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                      HTuple("anomaly_result_global"))))) {
            //
            // Get image.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample, "image");
            //
            ...
            TupleRegexpMatch(hv_AnomalyResultKey, "anomaly_result(.*)",
                             &hv_AnomalyResultPostfix);
            get_anomaly_result(
                        &ho_AnomalyImage, &ho_AnomalyRegion, hv_DLResult,
                        hv_Params.TupleGetDictTuple("anomaly_classification_threshold"),
            ...
            // Display anomaly regions defined by the specified threshold or out of
            // DLResult.
            TupleRegexpReplace(hv_AnomalyResultKey, "result", "region",
                               &hv_AnomalyRegionKey);
            CreateDict(&hv___Tmp_Ctrl_Dict_Init_5);
            SetDictTuple(hv___Tmp_Ctrl_Dict_Init_5, "comp", -1);
            if (0 !=
                    (HTuple(
                         ((hv_Params.TupleConcat(hv___Tmp_Ctrl_Dict_Init_5))
                          .TupleTestEqualDictItem("anomaly_region_threshold", "comp"))
                         .TupleNot())
                     .TupleOr(int((hv_ResultKeys.TupleFind(hv_AnomalyRegionKey)) !=
                                  -1)))) {
                dev_display_result_anomaly_regions(
                            ho_AnomalyRegion, hv_CurrentWindowHandle,
                            hv_Params.TupleGetDictTuple("line_width"),
            ...
            //
            TupleRegexpReplace(hv_AnomalyResultKey, "result", "score",
                               &hv_AnomalyScoreKey);
            //
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                if (0 !=
                        (((hv_Params.TupleConcat(hv___Tmp_Ctrl_Dict_Init_6))
                          .TupleTestEqualDictItem("anomaly_region_threshold", "comp"))
                         .TupleAnd(int((hv_ResultKeys.TupleFind(hv_AnomalyRegionKey)) ==
                                       -1)))) {
                    hv_Text[hv_Text.TupleLength()] = "";
                    hv_Text[hv_Text.TupleLength()] = "No segmentation result found";
                }
            ...
            if (0 != (int((hv_ResultKeys.TupleFind("gripping_confidence")) != -1))) {
                ...
                throw HException("Image with key 'gripping_confidence' could not be "
                                 "found in DLResult.");
            }
            add_colormap_to_image(
                        ho_ConfidenceImage, ho_Image, &ho_HeatmapScene,
                        hv_Params.TupleGetDictTuple("heatmap_color_scheme"));
            //
            // Get or open next window.
            GetImageSize(ho_Image, &hv_WidthImage, &hv_HeightImage);
            ...
            if (HDevWindowStack::IsOpen())
                DispText(HDevWindowStack::GetActive(), "Gripping heatmap", "window",
                         "top", "left", "black", "box", "true");
            //
            GenCrossContourXld(&ho_Cross, hv_Rows, hv_Columns,
                               hv_Params.TupleGetDictTuple("gripping_point_size"),
            ...
            if (HDevWindowStack::IsOpen())
                DispText(HDevWindowStack::GetActive(), "Gripping points", "window",
                         "top", "left", "black", "box", "true");
            //
            GenCrossContourXld(&ho_Cross, hv_Rows, hv_Columns,
                               hv_Params.TupleGetDictTuple("gripping_point_size"),
            ...
            if (HDevWindowStack::IsOpen())
                DispObj(ho_Cross, HDevWindowStack::GetActive());
            //
        } else if (0 != (HTuple(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                    HTuple("bbox_both")))
                         .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                      HTuple("ocr_detection_both"))))) {
            //
            ...
            // Visualization.
            //
            hv_DisplayDirectionTemp =
                    hv_Params.TupleGetDictTuple("display_direction");
            if (0 != ((hv_Params.TupleGetDictTuple("display_direction"))
                      .TupleAnd(hv_IsOCRDetection))) {
                hv_DisplayDirectionTemp = -1;
            }
            dev_display_ground_truth_detection(
                        hv_DLSample, hv_SampleKeys, hv_Params.TupleGetDictTuple("line_width"),
                        hv_ClassIDs, hv_Colors,
                        hv_Params.TupleGetDictTuple("bbox_label_color"), hv_WindowImageRatio,
                        hv_Params.TupleGetDictTuple("bbox_text_color"),
                        hv_Params.TupleGetDictTuple("display_labels"),
            ...
            dev_display_result_detection(
                        hv_DLResult, hv_ResultKeys, hv_Params.TupleGetDictTuple("line_width"),
                        hv_ClassIDs, hv_TextConf, hv_Colors,
                        hv_Params.TupleGetDictTuple("bbox_label_color"), hv_WindowImageRatio,
                        "bottom", hv_Params.TupleGetDictTuple("bbox_text_color"),
                        hv_Params.TupleGetDictTuple("display_labels"),
                        hv_DisplayDirectionTemp, hv_CurrentWindowHandle, &hv_BboxClassIndex);
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                if (0 != (int(((hv_BboxClassIndex.TupleLength()) +
                               (hv_BboxLabelIndex.TupleLength())) > 0))) {
                    hv_BboxClassLabelIndexUniq =
                            ((hv_BboxClassIndex.TupleConcat(hv_BboxLabelIndex)).TupleSort())
                            .TupleUniq();
                    hv_Text = hv_Text.TupleConcat(
                                HTuple(hv_ClassesLegend[hv_BboxClassLabelIndexUniq]));
                    hv_BboxColorsBoth = hv_BboxColorsBoth.TupleConcat(
                                HTuple(hv_Colors[hv_BboxClassLabelIndexUniq]));
                } else {
                ...
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             "left", hv_BboxColorsBoth, "box", "false");
                                    ...
                                    HTuple("bbox_ground_truth")))
                         .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                      HTuple("ocr_detection_ground_truth"))))) {
            //
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                if (0 != (hv_BboxIDs.TupleLength())) {
                    ...
                    hv_Text =
                            hv_Text.TupleConcat(HTuple(hv_ClassesLegend[hv_BboxIDsUniq]));
                    hv_BboxColors =
                            hv_BboxColors.TupleConcat(HTuple(hv_Colors[hv_BboxIDsUniq]));
                } else {
            ...
            dev_display_result_detection(
                        ...
                        hv_ClassIDs, hv_TextConf, hv_Colors,
                        hv_Params.TupleGetDictTuple("bbox_label_color"), hv_WindowImageRatio,
                        "top", hv_Params.TupleGetDictTuple("bbox_text_color"),
                        hv_Params.TupleGetDictTuple("display_labels"),
                        hv_DisplayDirectionTemp, hv_CurrentWindowHandle, &hv_BboxClassIndex);
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                if (0 != (int((hv_BboxClassIndex.TupleLength()) > 0))) {
                    ...
                    hv_Text = hv_Text.TupleConcat(
                                HTuple(hv_ClassesLegend[hv_BboxClassIndexUniq]));
                    hv_BboxColorsResults = hv_BboxColorsResults.TupleConcat(
                                HTuple(hv_Colors[hv_BboxClassIndexUniq]));
                } else {
                ...
                if (HDevWindowStack::IsOpen())
                   ...
                   (HTuple(
                        HTuple(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                   HTuple("ocr_detection_score_map_character")))
                        .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                     HTuple("ocr_detection_score_map_link"))))
                    .TupleOr(
                        int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                            HTuple("ocr_detection_score_map_orientation"))))) {
            //
            // Extract score maps
            ho_ScoreMaps = hv_OcrResult.TupleGetDictObject("score_maps");
            Decompose4(ho_ScoreMaps, &ho_CharacterScoreMap, &ho_LinkScoreMap,
                       &ho_OrientationSinScoreMap, &ho_OrientationCosScoreMap);
            //
            ...
            if (0 != (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                ...
                hv_Text = "Link score " + hv_ImageIDStringBraces;
                                 ...
                                 HTuple("ocr_detection_score_map_orientation")))) {
                Compose2(ho_OrientationSinScoreMap, ho_OrientationCosScoreMap,
                         &ho_OrientationScoreMap);
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                   ...
                   (HTuple(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                               HTuple("classification_both")))
                    .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                 HTuple("multi_label_classification_both"))))) {
            //
            // Ground truth and result classification image.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample, "image");
            //
            get_classification_ground_truth(hv_SampleKeys, hv_DLSample,
                                            &hv_ClassificationLabelIDGroundTruth);
            get_classification_result(hv_ResultKeys, hv_DLResult,
            ...
            // Get prediction color.
            hv_PredictionColor = "white";
            if (0 != (int((hv_ClassificationLabelIDGroundTruth.TupleSort()) ==
                          (hv_ClassificationLabelIDResult.TupleSort())))) {
            ...
            if (0 !=
                    (hv_Params.TupleGetDictTuple("display_classification_color_frame"))) {
                // Create a frame with line width 7 that is completely displayed in the
                // window.
                hv_BoarderOffset = 7 / 2.;
                GetDictTuple(hv_WindowHandleDict, "meta_information", &hv_MetaInfo);
                GetDictTuple(hv_MetaInfo,
                             HTuple(hv_KeysForDisplay[hv_KeyIndex]) +
                             "_window_image_ratio_height",
                             &hv_WindowImageRatioHeight);
                GetDictTuple(hv_MetaInfo,
                             HTuple(hv_KeysForDisplay[hv_KeyIndex]) +
                             "_window_image_ratio_width",
                ...
                GenContourPolygonXld(
                            &ho_PredictionColorFrame,
                            ((((hv_BoarderOffsetRow - 0.5)
                               .TupleConcat(hv_BoarderOffsetRow - 0.5))
                              .TupleConcat((hv_HeightImage + 0.5) - hv_BoarderOffsetRow))
                             .TupleConcat((hv_HeightImage + 0.5) - hv_BoarderOffsetRow))
                            .TupleConcat(hv_BoarderOffsetRow - 0.5),
                            ((((hv_BoarderOffsetCol - 0.5)
                               .TupleConcat((hv_WidthImage + 0.5) - hv_BoarderOffsetCol))
                              .TupleConcat((hv_WidthImage + 0.5) - hv_BoarderOffsetCol))
                             .TupleConcat(hv_BoarderOffsetCol - 0.5))
                            .TupleConcat(hv_BoarderOffsetCol - 0.5));
                if (HDevWindowStack::IsOpen())
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_classification_ids"))) {
                GetDictTuple(hv_WindowHandleDict, "meta_information", &hv_MetaInfo);
                GetDictTuple(hv_MetaInfo,
                             HTuple(hv_KeysForDisplay[hv_KeyIndex]) + "_margin_bottom",
                             &hv_MarginBottom);
                GetDictTuple(hv_MetaInfo,
                             HTuple(hv_KeysForDisplay[hv_KeyIndex]) +
                             "_window_coordinates",
                             &hv_WindowCoordinates);
                hv_CurrentWindowHeight =
                        HTuple(hv_WindowCoordinates[3]) - HTuple(hv_WindowCoordinates[0]);
                GetFontExtents(hv_CurrentWindowHandle, &hv__, &hv__, &hv__,
                               &hv_MaxHeight);
                if (0 !=
                        (int((hv_ClassificationLabelIDGroundTruth.TupleLength()) > 1))) {
                    hv_PluralPostfix = "s";
                    hv_ClassificationLabelIDGroundTruth =
                            hv_ClassificationLabelIDGroundTruth.TupleSort();
                } else {
                    hv_PluralPostfix = "";
                }
                hv_ClassificationLabelIDGroundTruthList = "";
                {
                    HTuple end_val1023 =
                            (hv_ClassificationLabelIDGroundTruth.TupleLength()) - 1;
                    HTuple step_val1023 = 1;
                    for (hv_C = 0; hv_C.Continue(end_val1023, step_val1023);
                         hv_C += step_val1023) {
                        hv_ClassificationLabelIDGroundTruthList +=
                                HTuple(hv_ClassificationLabelIDGroundTruth[hv_C]);
                        if (0 != (int(hv_C <
                                      ((hv_ClassificationLabelIDGroundTruth.TupleLength()) -
                                       1)))) {
                ...
                hv_Text = (("GT label ID" + hv_PluralPostfix) + ": ") +
                        hv_ClassificationLabelIDGroundTruthList;
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             "left", "white", "box", "false");
                ...
                {
                    HTuple end_val1040 =
                            (hv_ClassificationLabelIDResult.TupleLength()) - 1;
                    HTuple step_val1040 = 1;
                    for (hv_C = 0; hv_C.Continue(end_val1040, step_val1040);
                         hv_C += step_val1040) {
                        hv_ClassificationLabelIDResultList +=
                                HTuple(hv_ClassificationLabelIDResult[hv_C]);
                        if (0 !=
                                (int(hv_C <
                                     ((hv_ClassificationLabelIDResult.TupleLength()) - 1)))) {
                ...
                hv_Text = (("Result class ID" + hv_PluralPostfix) + ": ") +
                        hv_ClassificationLabelIDResultList;
                if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                    if (HDevWindowStack::IsOpen())
                        DispText(HDevWindowStack::GetActive(), hv_Text, "window",
                                 hv_CurrentWindowHeight -
                                 ((hv_MarginBottom + hv_MaxHeight) + 10),
                                 "left", "white", "box", "false");
                } else {
                    if (HDevWindowStack::IsOpen())
                        DispText(HDevWindowStack::GetActive(), hv_Text, "window", "bottom",
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                hv_Text = (("Ground truth class ID" + hv_PluralPostfix) + " ") +
                        hv_ImageIDStringBraces;
                hv_Text = hv_Text.TupleConcat(
                            HTuple(hv_ClassesLegend[hv_ClassIDstoIndex.TupleGetDictTuple(
                                       hv_ClassificationLabelIDGroundTruth)]));
                hv_Text[hv_Text.TupleLength()] = "";
                hv_Text[hv_Text.TupleLength()] = "";
                if (0 != (int((hv_ClassificationLabelIDResult.TupleLength()) > 1))) {
                ...
                GetDictParam(hv_DLResult, "key_exists", "selected_confidences",
                             &hv_HasConfidences);
                if (0 != hv_HasConfidences) {
                    hv_TPIndices = HTuple();
                    {
                        HTuple end_val1082 =
                                ((hv_DLResult.TupleGetDictTuple("selected_class_ids"))
                                 .TupleLength()) -
                                1;
                        HTuple step_val1082 = 1;
                        for (hv_RIdx = 0; hv_RIdx.Continue(end_val1082, step_val1082);
                             hv_RIdx += step_val1082) {
                            hv_ResultLine = HTuple((hv_DLResult.TupleGetDictTuple(
                                                        "selected_class_ids"))[hv_RIdx]);
                            hv_ResultLine = ((hv_ResultLine + " (") +
                                             (HTuple((hv_DLResult.TupleGetDictTuple(
                                                          "selected_confidences"))[hv_RIdx])
                                              .TupleString(".2f"))) +
                                    ") : ";
                            hv_ResultLine += HTuple((hv_DLResult.TupleGetDictTuple(
                                                         "selected_class_names"))[hv_RIdx]);
                            hv_Text = hv_Text.TupleConcat(hv_ResultLine);
                            if (0 !=
                                    (int(-1 != (hv_ClassificationLabelIDGroundTruth.TupleFind(
                                                    HTuple((hv_DLResult.TupleGetDictTuple(
                                                                "selected_class_ids"))[hv_RIdx])))))) {
                    ...
                    // Check for false negatives and add those to the legend (only
                    // multi-label classification)
                    hv_HasFalseNegatives =
                            int(((hv_ClassificationLabelIDGroundTruth.TupleDifference(
                                      hv_DLResult.TupleGetDictTuple("selected_class_ids")))
                                 .TupleLength()) > 0);
                    if (0 != hv_HasFalseNegatives) {
                        hv_FalseNegativeClassIDs =
                                hv_ClassificationLabelIDGroundTruth.TupleDifference(
                                    hv_DLResult.TupleGetDictTuple("selected_class_ids"));
                        ...
                        hv_Text = hv_Text.TupleConcat("");
                        hv_Text = hv_Text.TupleConcat("False negative class ID" +
                                                      hv_PluralPostfix);
                        {
                            HTuple end_val1102 = (hv_FalseNegativeClassIDs.TupleLength()) - 1;
                            HTuple step_val1102 = 1;
                            for (hv_FNIdx = 0; hv_FNIdx.Continue(end_val1102, step_val1102);
                                 hv_FNIdx += step_val1102) {
                                hv_FNClassIdx = hv_ClassIDstoIndex.TupleGetDictTuple(
                                            HTuple(hv_FalseNegativeClassIDs[hv_FNIdx]));
                                hv_FNLine = HTuple((
                                                       hv_DLResult.TupleGetDictTuple("class_ids"))[hv_FNClassIdx]);
                                hv_FNLine = ((hv_FNLine + " (") +
                                             ((HTuple((hv_DLResult.TupleGetDictTuple(
                                                           "confidences"))[hv_FNClassIdx])
                                               .TupleString("f"))
                                              .TupleSubstr(0, 3))) +
                                        ") : ";
                                hv_FNLine += HTuple((hv_DLResult.TupleGetDictTuple(
                                                         "class_names"))[hv_FNClassIdx]);
                ...
                if (0 != hv_IsMLC) {
                    ...
                    TupleGenConst((hv_DLResult.TupleGetDictTuple("selected_class_ids"))
                                  .TupleLength(),
                                  "red", &hv_ResultColors);
                }
                hv_LineColors[HTuple::TupleGenSequence(
                            1, hv_ClassificationLabelIDGroundTruth.TupleLength(), 1)] =
                        HTuple(hv_Colors[hv_ClassIDstoIndex.TupleGetDictTuple(
                                   hv_ClassificationLabelIDGroundTruth)]);
                if (0 != (int(hv_ClassificationLabelIDResult != HTuple()))) {
                    if (0 != hv_IsMLC) {
                        hv_ResultColors[hv_TPIndices] = "green";
                        hv_LineColors[HTuple::TupleGenSequence(
                                    (5 + (hv_ClassificationLabelIDGroundTruth.TupleLength())) - 1,
                                    (((5 + (hv_ClassificationLabelIDGroundTruth.TupleLength())) -
                                      1) +
                                     (hv_ClassificationLabelIDResult.TupleLength())) -
                                    1,
                                    1)] = hv_ResultColors;
                    } else {
                        hv_LineColors[HTuple::TupleGenSequence(
                ...
                if (0 != hv_HasConfidences) {
                    ...
                    if (0 != hv_HasFalseNegatives) {
                        hv_LineColors[HTuple::TupleGenSequence(
                                    ((hv_LineColors.TupleLength()) - 4) - hv_NumFNLines,
                                    (hv_LineColors.TupleLength()) - 4, 1)] = "red";
                    }
                }
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                   ...
                   (HTuple(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                               HTuple("classification_ground_truth")))
                    .TupleOr(int(
                                 HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                 HTuple("multi_label_classification_ground_truth"))))) {
            //
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_classification_ids"))) {
                hv_ClassificationLabelIDGroundTruthList = "";
                {
                    ...
                    for (hv_C = 0; hv_C.Continue(end_val1171, step_val1171);
                         hv_C += step_val1171) {
                        hv_ClassificationLabelIDGroundTruthList +=
                                HTuple(hv_ClassificationLabelIDGroundTruth[hv_C]);
                        if (0 != (int(hv_C <
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             "left",
                             HTuple("white").TupleConcat(
                                 HTuple(hv_Colors[hv_ClassIDstoIndex.TupleGetDictTuple(
                                            hv_ClassificationLabelIDGroundTruth)])),
                             "box", "false");
            }
        } else if (0 != (HTuple(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                    HTuple("classification_result")))
                         .TupleOr(int(
                                      HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                      HTuple("multi_label_classification_result"))))) {
            //
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_classification_ids"))) {
                GetDictTuple(hv_WindowHandleDict, "meta_information", &hv_MetaInfo);
                GetDictTuple(hv_MetaInfo, "classification_result_margin_bottom",
                             &hv_MarginBottom);
                GetDictTuple(hv_MetaInfo, "classification_result_window_coordinates",
                             &hv_WindowCoordinates);
                hv_CurrentWindowHeight =
                        HTuple(hv_WindowCoordinates[3]) - HTuple(hv_WindowCoordinates[0]);
                GetFontExtents(hv_CurrentWindowHandle, &hv__, &hv__, &hv__,
                ...
                hv_ClassificationLabelIDResultList = "";
                {
                    ...
                    for (hv_C = 0; hv_C.Continue(end_val1228, step_val1228);
                         hv_C += step_val1228) {
                        hv_ClassificationLabelIDResultList +=
                                HTuple(hv_ClassificationLabelIDResult[hv_C]);
                        if (0 !=
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                if (0 != (int(hv_ClassificationLabelIDResult == HTuple()))) {
                    ...
                    GetDictParam(hv_DLResult, "key_exists", "selected_class_ids",
                                 &hv_IsMLC);
                    if (0 != hv_IsMLC) {
                        TupleGenConst(hv_Colors.TupleLength(), "light gray", &hv_Colors);
                    }
                ...
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             ...
                             "box", "false");
                                           ...
                                           HTuple("heatmap_grad_cam")))
                                .TupleOr(int(
                                             HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                             HTuple("heatmap_guided_grad_cam"))))
                         .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                      HTuple("heatmap_confidence_based"))))) {
            //
            // Display the heatmap image (method 'heatmap_grad_cam',
            // 'heatmap_guided_grad_cam', or 'heatmap_confidence_based') in the
            // selected color scheme. Retrieve heatmap image, inferred image, and
            // inference results.
            hv_SelectedHeatmapMethod = HTuple(hv_KeysForDisplay[hv_KeyIndex]);
            if (0 !=
                    (HTuple(
                         HTuple(int((hv_ResultKeys.TupleFind("heatmap_grad_cam")) != -1))
                         .TupleOr(int((hv_ResultKeys.TupleFind(
                                           "heatmap_guided_grad_cam")) != -1)))
                     .TupleOr(int((hv_ResultKeys.TupleFind(
                                       "heatmap_confidence_based")) != -1)))) {
                if (0 !=
                        (int(hv_SelectedHeatmapMethod == HTuple("heatmap_grad_cam")))) {
                    GetDictTuple(hv_DLResult, "heatmap_grad_cam", &hv_DictHeatmap);
                    hv_MethodName = "Grad-CAM";
                } else if (0 != (int(hv_SelectedHeatmapMethod ==
                                     HTuple("heatmap_guided_grad_cam")))) {
                    GetDictTuple(hv_DLResult, "heatmap_guided_grad_cam", &hv_DictHeatmap);
                    hv_MethodName = "Guided-Grad-CAM";
                } else {
                    GetDictTuple(hv_DLResult, "heatmap_confidence_based",
                                 &hv_DictHeatmap);
                    hv_MethodName = "Confidence based";
                }
                GetDictParam(hv_DictHeatmap, "keys", HTuple(), &hv_HeatmapKeys);
                //
                if (0 != (int(hv_SelectedHeatmapMethod ==
                              HTuple("heatmap_guided_grad_cam")))) {
                    ...
                    hv_HeatmapImageName =
                            hv_HeatmapKeys.TupleRegexpSelect("heatmap_image_class_[0-9]*");
                    hv_TargetClassID = hv_HeatmapImageName.TupleRegexpMatch(
            ...
            if (0 != (int(hv_SelectedHeatmapMethod == HTuple("heatmap_grad_cam")))) {
                ...
                add_colormap_to_image(
                            ho_ImageHeatmap, ho_Image, &ho_ImageHeatmap,
                            hv_Params.TupleGetDictTuple("heatmap_color_scheme"));
            }
            //
            // Get or open next window.
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                ...
                GetDictTuple(hv_DLResult, "classification_confidences",
                             &hv_Confidences);
                if (0 != (int(hv_SelectedHeatmapMethod ==
                              HTuple("heatmap_confidence_based")))) {
                    GetDictTuple(hv_DictHeatmap, "classification_heatmap_maxdeviation",
                                 &hv_MaxDeviation);
                }
                hv_ClassificationLabelNameResult = ((const HTuple &)hv_ClassNames)[0];
                hv_ClassificationLabelIDResult = ((const HTuple &)hv_ClassIDs)[0];
                hv_TargetClassConfidence = HTuple(hv_Confidences[hv_ClassIDs.TupleFind(
                                                      hv_TargetClassID.TupleNumber())]);
                hv_Text = "--------- ";
                hv_Text[hv_Text.TupleLength()] = "Image ";
                hv_Text[hv_Text.TupleLength()] = "--------- ";
                hv_Text[hv_Text.TupleLength()] = "";
                if (0 != (int((hv_SampleKeys.TupleFind("image_label_id")) != -1))) {
                    GetDictTuple(hv_DLSample, "image_label_id",
                                 &hv_ClassificationLabelIDGroundTruth);
                    GetDictTuple(hv_DLDatasetInfo, "class_names",
                                 &hv_ClassificationLabelNamesGroundTruth);
                    // Get prediction color.
                    if (0 != (int(hv_ClassificationLabelIDGroundTruth ==
                                  hv_ClassificationLabelIDResult))) {
                        hv_PredictionColor = "green";
                    } else {
                        hv_PredictionColor = "red";
                    ...
                    hv_Text[hv_Text.TupleLength()] =
                            HTuple(hv_ClassificationLabelNamesGroundTruth
                                   [hv_ClassificationLabelIDGroundTruth]);
                    hv_Text[hv_Text.TupleLength()] = "";
                }
                ...
                hv_Text[hv_Text.TupleLength()] = "";
                hv_Text[hv_Text.TupleLength()] =
                        "Confidence: " + (HTuple(hv_Confidences[0]).TupleString(".2f"));
                hv_Text[hv_Text.TupleLength()] = "";
                hv_Text[hv_Text.TupleLength()] = "--------- ";
                ...
                hv_Text[hv_Text.TupleLength()] =
                        ...
                        (hv_TargetClassConfidence.TupleString(".2f"));
                if (0 != (int(hv_SelectedHeatmapMethod ==
                              HTuple("heatmap_confidence_based")))) {
                    hv_Text[hv_Text.TupleLength()] =
                            "Maximum deviation: " + (hv_MaxDeviation.TupleString(".2f"));
                }
                //
                // Get or open next child window
                get_child_window(hv_HeightImage, hv_Params.TupleGetDictTuple("font"),
                ...
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             "left", hv_LineColors, "box", "false");
                                           ...
                                           HTuple("ocr_recognition_ground_truth")))
                                .TupleOr(int(
                                             HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                             HTuple("ocr_recognition_result"))))
                         .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                      HTuple("ocr_recognition_both"))))) {
            //
            // OCR Recognition.
            hv_ShowGT = HTuple(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                   HTuple("ocr_recognition_both")))
                    .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                 HTuple("ocr_recognition_ground_truth")));
            hv_ShowResult = HTuple(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                       HTuple("ocr_recognition_both")))
                    .TupleOr(int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                                 HTuple("ocr_recognition_result")));
            ...
            get_next_window(
                        hv_Params.TupleGetDictTuple("font"),
                        hv_Params.TupleGetDictTuple("font_size"),
                        hv_NumLines * (hv_Params.TupleGetDictTuple("display_bottom_desc")),
                        hv_WidthImage, hv_HeightImage, 0,
                        hv_Params.TupleGetDictTuple("scale_windows"),
                        hv_Params.TupleGetDictTuple("threshold_width"),
                        hv_PrevWindowCoordinates, hv_WindowHandleDict,
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                ...
                if (0 != hv_ShowGT) {
                    hv_Text[hv_Text.TupleLength()] =
                            (("Ground truth: \"" + (hv_DLSample.TupleGetDictTuple("word"))) +
                             "\" ") +
                            hv_ImageIDStringBraces;
                }
                // Result.
                if (0 != hv_ShowResult) {
                    ...
                    if (0 != hv_GTWordKeyExists) {
                        ...
                        hv_HeightMarginBottom = (hv_HeightImage * hv_MarginBottom) /
                                (hv_HeightWindow - hv_MarginBottom);
                        hv_Size = hv_HeightMarginBottom / 2;
                        hv_Length = hv_Size / 2;
                        hv_Row = hv_HeightImage + (hv_HeightMarginBottom / 2);
                        hv_Column = hv_WidthImage - (1.5 * hv_Size);
                        HomMat2dIdentity(&hv_HomMat2DIdentity);
                        HomMat2dRotate(hv_HomMat2DIdentity, HTuple(45).TupleRad(), 0, 0,
                                       &hv_HomMat2DRotate);
                        HomMat2dTranslate(hv_HomMat2DRotate, hv_Row, hv_Column,
                                          &hv_HomMat2DCompose);
                        if (0 != ((hv_DLResult.TupleConcat(hv_DLSample))
                                  .TupleTestEqualDictItem("word", "word"))) {
                            hv_PredictionForegroundColor = "green";
                            hv_PredictionBackgroundColor = "#008000";
                            GenContourPolygonXld(
                                        &ho_PredictionSymbol,
                                        (-0.75 * hv_Length).TupleConcat((HTuple(0).Append(0))),
                                        ((-hv_Length).TupleConcat(-hv_Length))
                                        .TupleConcat(hv_Length));
                        } else {
                            hv_PredictionForegroundColor = "red";
                            hv_PredictionBackgroundColor = "#800000";
                            GenContourPolygonXld(&ho_CrossLineH, (HTuple(0).Append(0)),
                                                 (-hv_Length).TupleConcat(hv_Length));
                            GenContourPolygonXld(&ho_CrossLineV,
                                                 (-hv_Length).TupleConcat(hv_Length),
                                                 (HTuple(0).Append(0)));
                            ConcatObj(ho_CrossLineH, ho_CrossLineV, &ho_PredictionSymbol);
                        }
                        AffineTransContourXld(ho_PredictionSymbol, &ho_PredictionSymbol,
                                              hv_HomMat2DCompose);
                        SetWindowParam(hv_CurrentWindowHandle, "background_color",
                    ...
                    hv_Text[hv_Text.TupleLength()] =
                            ((("Deep OCR:" + hv_Spaces) + " \"") +
                             (hv_DLResult.TupleGetDictTuple("word"))) +
                            "\"";
                }
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "bottom",
                             ...
                             HTuple("segmentation_confidence")))) {
            //
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(),
                             "Confidence image " + hv_ImageIDStringBraces, "window",
                             "bottom", "left", "white", "box", "false");
            }
        } else if (0 != (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                             HTuple("segmentation_confidence_map")))) {
            //
            ...
            if (HDevWindowStack::IsOpen())
                DispObj(ho_Image, HDevWindowStack::GetActive());
            dev_display_confidence_regions(
                        ho_ImageConfidence, hv_Params.TupleGetDictTuple("map_transparency"),
                        &hv_ConfidenceColors);
            dev_display_map_color_bar(
                        hv_WidthImage, hv_HeightImage,
                        hv_Params.TupleGetDictTuple("map_color_bar_width"),
                        hv_ConfidenceColors, 1.0, hv_WindowImageRatio,
                        hv_CurrentWindowHandle);
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(),
                             "Confidence map " + hv_ImageIDStringBraces, "window",
                             "bottom", "left", "white", "box", "false");
            }
            //
        } else if (0 != (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                             HTuple("segmentation_image_both")))) {
            //
            // Ground truth and result segmentation on image.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample, "image");
            get_segmentation_image_ground_truth(&ho_SegmentationImagGroundTruth,
            ...
            hv_ColorsResults =
                    hv_Colors +
                    (hv_Params.TupleGetDictTuple("segmentation_transparency"));
            if (HDevWindowStack::IsOpen())
                SetDraw(HDevWindowStack::GetActive(), "margin");
            if (HDevWindowStack::IsOpen())
                SetLineWidth(HDevWindowStack::GetActive(), 2);
            dev_display_segmentation_regions(
                        ho_SegmentationImagGroundTruth, hv_ClassIDs, hv_ColorsResults,
                        hv_Params.TupleGetDictTuple("segmentation_exclude_class_ids"),
                        &hv_GroundTruthIDs);
            if (HDevWindowStack::IsOpen())
                SetLineWidth(HDevWindowStack::GetActive(), 6);
            dev_display_segmentation_regions(
                        ho_SegmentationImageResult, hv_ClassIDs, hv_ColorsResults,
                        hv_Params.TupleGetDictTuple("segmentation_exclude_class_ids"),
                        &hv_ResultIDs);
            if (HDevWindowStack::IsOpen())
                SetDraw(HDevWindowStack::GetActive(), "fill");
            hv_Text =
                    "Ground truth and result segmentation " + hv_ImageIDStringBraces;
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "bottom",
                             "left", "white", "box", "false");
            ...
            // Display the legend.
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                hv_ImageClassIDsUniq =
                        ((hv_GroundTruthIDs.TupleConcat(hv_ResultIDs)).TupleSort())
                        .TupleUniq();
                // Get Indices according to image class IDs.
                TupleGenConst(hv_ImageClassIDsUniq.TupleLength(), 0,
                              &hv_ImageClassIDsIndices);
                {
                    HTuple end_val1504 = (hv_ImageClassIDsUniq.TupleLength()) - 1;
                    HTuple step_val1504 = 1;
                    for (hv_ImageClassIDsIndex = 0;
                         hv_ImageClassIDsIndex.Continue(end_val1504, step_val1504);
                         hv_ImageClassIDsIndex += step_val1504) {
                        hv_ImageClassIDsIndices[hv_ImageClassIDsIndex] =
                                hv_ClassIDs.TupleFindFirst(
                                    HTuple(hv_ImageClassIDsUniq[hv_ImageClassIDsIndex]));
                    }
                }
                hv_Text = hv_Text.TupleConcat(
                            HTuple(hv_ClassesLegend[hv_ImageClassIDsIndices]));
                hv_Text[(hv_Text.TupleLength()) + 1] =
                        HTuple("- thicker line: result, thinner lines: ground truth");
                hv_Text[hv_Text.TupleLength()] =
                        "  (you may have to zoom in for a more detailed view)";
                hv_StringSegExcludeClassIDs = "";
                {
                    ...
                    for (hv_StringIndex = 0;
                         hv_StringIndex.Continue(end_val1511, step_val1511);
                         hv_StringIndex += step_val1511) {
                        if (0 !=
                                (int(hv_StringIndex == (((hv_Params.TupleGetDictTuple(
                                                              "segmentation_exclude_class_ids"))
                                                         .TupleLength()) -
                                                        1)))) {
                            hv_StringSegExcludeClassIDs +=
                                    HTuple((hv_Params.TupleGetDictTuple(
                                                "segmentation_exclude_class_ids"))[hv_StringIndex]);
                        } else {
                            hv_StringSegExcludeClassIDs =
                                    (hv_StringSegExcludeClassIDs +
                                     HTuple((hv_Params.TupleGetDictTuple(
                ...
                if (0 != (((hv_Params.TupleConcat(hv___Tmp_Ctrl_Dict_Init_7))
                           .TupleTestEqualDictItem("segmentation_exclude_class_ids",
                                                   "comp"))
                          .TupleNot())) {
                    hv_Text[hv_Text.TupleLength()] =
                            ("- (excluded classID(s) " + hv_StringSegExcludeClassIDs) +
                            " from visualization)";
                }
                ...
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             "left",
                             (HTuple("white").TupleConcat(
                                  HTuple(hv_Colors[hv_ImageClassIDsIndices])))
                             .TupleConcat(
                                 ((HTuple("white").Append("white")).Append("white"))),
                             "box", "false");
            }
        } else if (0 != (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                             HTuple("segmentation_image_diff")))) {
            //
            ...
            if (HDevWindowStack::IsOpen())
                DispObj(ho_Image, HDevWindowStack::GetActive());
            AbsDiffImage(ho_SegmentationImagGroundTruth, ho_SegmentationImageResult,
                         &ho_ImageAbsDiff, 1);
            MinMaxGray(ho_SegmentationImageResult, ho_ImageAbsDiff, 0, &hv_Min,
                       &hv_Max, &hv_Range);
            if (0 != (int(hv_Min != hv_Max))) {
                Threshold(ho_ImageAbsDiff, &ho_DiffRegion, 0.00001, hv_Max);
                if (HDevWindowStack::IsOpen())
                    SetColor(HDevWindowStack::GetActive(),
                             "#ff0000" + (hv_Params.TupleGetDictTuple(
                                              "segmentation_transparency")));
                if (HDevWindowStack::IsOpen())
                    DispObj(ho_DiffRegion, HDevWindowStack::GetActive());
            } else {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), "No difference found.",
                             "window", "top", "left", "black", HTuple(), HTuple());
            }
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                hv_Text = "Difference of ground truth and result segmentation " +
                        hv_ImageIDStringBraces;
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "bottom",
                             "left", "white", "box", "false");
                             ...
                             HTuple("segmentation_image_ground_truth")))) {
            //
            ...
            hv_ColorsSegmentation =
                    hv_Colors +
                    (hv_Params.TupleGetDictTuple("segmentation_transparency"));
            GetDraw(hv_CurrentWindowHandle, &hv_DrawMode);
            if (HDevWindowStack::IsOpen())
                SetDraw(HDevWindowStack::GetActive(),
                        hv_Params.TupleGetDictTuple("segmentation_draw"));
            GetLineWidth(hv_CurrentWindowHandle, &hv_Width);
            if (HDevWindowStack::IsOpen())
                SetLineWidth(HDevWindowStack::GetActive(),
                             hv_Params.TupleGetDictTuple("line_width"));
            dev_display_segmentation_regions(
                        ho_SegmentationImagGroundTruth, hv_ClassIDs, hv_ColorsSegmentation,
                        hv_Params.TupleGetDictTuple("segmentation_exclude_class_ids"),
                        &hv_ImageClassIDs);
            if (HDevWindowStack::IsOpen())
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                              ...
                              &hv_ImageClassIDsIndices);
                {
                    ...
                    for (hv_ImageClassIDsIndex = 0;
                         hv_ImageClassIDsIndex.Continue(end_val1586, step_val1586);
                         hv_ImageClassIDsIndex += step_val1586) {
                        hv_ImageClassIDsIndices[hv_ImageClassIDsIndex] =
                                hv_ClassIDs.TupleFindFirst(
                                    HTuple(hv_ImageClassIDsUniq[hv_ImageClassIDsIndex]));
                    }
                ...
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(), hv_Text, "window", "top",
                             ...
                             HTuple("segmentation_image_result")))) {
            //
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_legend"))) {
                              ...
                              &hv_ImageClassIDsIndices);
                {
                    ...
                    for (hv_ImageClassIDsIndex = 0;
                         hv_ImageClassIDsIndex.Continue(end_val1627, step_val1627);
                         hv_ImageClassIDsIndex += step_val1627) {
                        hv_ImageClassIDsIndices[hv_ImageClassIDsIndex] =
                                hv_ClassIDs.TupleFindFirst(
                                    HTuple(hv_ImageClassIDsUniq[hv_ImageClassIDsIndex]));
                    }
            ...
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(),
                             "Weight image " + hv_ImageIDStringBraces, "window", "bottom",
                             "left", "white", "box", "false");
            }
        } else if (0 != (int(HTuple(hv_KeysForDisplay[hv_KeyIndex]) ==
                             HTuple("segmentation_weight_map")))) {
            //
            // Weight map on image.
            get_dl_sample_image(&ho_Image, hv_SampleKeys, hv_DLSample, "image");
            get_weight_image(&ho_ImageWeight, hv_SampleKeys, hv_DLSample);
            ...
            if (0 !=
                    ((hv_Params.TupleConcat(hv___Tmp_Ctrl_Dict_Init_8))
                     .TupleTestEqualDictItem("segmentation_max_weight", "comp"))) {
                // Calculate SegMaxWeight if not given in GenParam.
                MinMaxGray(ho_ImageWeight, ho_ImageWeight, 0, &hv_MinWeight,
            ...
            if (HDevWindowStack::IsOpen())
                DispObj(ho_Image, HDevWindowStack::GetActive());
            dev_display_weight_regions(
                        ho_ImageWeight, hv_Params.TupleGetDictTuple("map_transparency"),
                        hv_Params.TupleGetDictTuple("segmentation_max_weight"),
                        &hv_WeightsColors);
            dev_display_map_color_bar(
                        hv_WidthImage, hv_HeightImage,
                        hv_Params.TupleGetDictTuple("map_color_bar_width"), hv_WeightsColors,
                        hv_Params.TupleGetDictTuple("segmentation_max_weight"),
                        hv_WindowImageRatio, hv_CurrentWindowHandle);
            if (0 != (hv_Params.TupleGetDictTuple("display_bottom_desc"))) {
                if (HDevWindowStack::IsOpen())
                    DispText(HDevWindowStack::GetActive(),
                             "Weight map " + hv_ImageIDStringBraces, "window", "bottom",
                             "left", "white", "box", "false");
            }
        } else {
            ...
            GetDictParam(hv_WindowHandleDict, "keys", HTuple(), &hv_WindowHandleKeys);
            {
                ...
                for (hv_Index = 0; hv_Index.Continue(end_val1678, step_val1678);
                     hv_Index += step_val1678) {
                    // Only consider the WindowHandleKeys that are needed for the current
                    // visualization.
                    hv_Indices = hv_KeysForDisplay.TupleFind(
                                HTuple(hv_WindowHandleKeys[hv_Index]));
                    if (0 != (HTuple(int(hv_Indices != -1))
                              .TupleAnd(int(hv_Indices != HTuple())))) {
                        GetDictTuple(hv_WindowHandleDict,
                                     HTuple(hv_WindowHandleKeys[hv_Index]),
                                     &hv_WindowHandles);
                        {
                            ...
                            for (hv_WindowIndex = 0;
                                 hv_WindowIndex.Continue(end_val1683, step_val1683);
                                 hv_WindowIndex += step_val1683) {
                                // Reset values of windows that have been changed temporarily.
                                SetWindowParam(HTuple(hv_WindowHandles[hv_WindowIndex]),
                                               "flush", HTuple(hv_FlushValues[hv_Index]));
                            }
                        }
                    }
                }
            }
            throw HException("Key for display unknown: " +
                             HTuple(hv_KeysForDisplay[hv_KeyIndex]));
        }
        //
        hv_KeyIndex += 1;
    ...
    GetDictParam(hv_WindowHandleDict, "keys", HTuple(), &hv_WindowHandleKeysNew);
    {
        ...
        for (hv_Index = 0; hv_Index.Continue(end_val1697, step_val1697);
             hv_Index += step_val1697) {
            // Only consider the WindowHandleKeys that are needed for the current
            // visualization.
            hv_KeyIndex =
                    hv_KeysForDisplay.TupleFind(HTuple(hv_WindowHandleKeysNew[hv_Index]));
            if (0 != (HTuple(int(hv_KeyIndex != -1))
                      .TupleAnd(int(hv_KeyIndex != HTuple())))) {
                GetDictTuple(hv_WindowHandleDict,
                             HTuple(hv_WindowHandleKeysNew[hv_Index]),
                             &hv_WindowHandles);
                {
                    HTuple end_val1702 = (hv_WindowHandles.TupleLength()) - 1;
                    HTuple step_val1702 = 1;
                    for (hv_WindowIndex = 0;
                         hv_WindowIndex.Continue(end_val1702, step_val1702);
                         hv_WindowIndex += step_val1702) {
                        // Display content of window handle.
                        if (0 != (int((hv_WindowHandleKeys.TupleLength()) ==
                                      (hv_WindowHandleKeysNew.TupleLength())))) {
                            // Reset values of windows that have been changed temporarily.
                            if (0 != (int(HTuple(hv_FlushValues[hv_WindowIndex]) ==
                                          HTuple("true")))) {
                                FlushBuffer(HTuple(hv_WindowHandles[hv_WindowIndex]));
                            }
                            SetWindowParam(HTuple(hv_WindowHandles[hv_WindowIndex]), "flush",
                                           HTuple(hv_FlushValues[hv_WindowIndex]));
                        } else {
...
// Chapter: Deep Learning / Model
// Short Description: Try to guess the maximum class id based on the given
// sample/result.
void dev_display_dl_data_get_max_class_id(HTuple hv_DLSample,
                                          HTuple *hv_MaxClassId,
                                          HTuple *hv_Empty) {
    ...
    try {
        ...
        if (0 != (int(hv_Indices > -1))) {
            ...
            hv_LengthAll = 0;
            {
                ...
                HTuple step_val12 = 1;
                for (hv_KeyIndex = 0; hv_KeyIndex.Continue(end_val12, step_val12);
                     hv_KeyIndex += step_val12) {
...
// Chapter: Deep Learning / Anomaly Detection and Global Context Anomaly
// Detection Short Description: Display the ground truth anomaly regions of the
// given DLSample.
void dev_display_ground_truth_anomaly_regions(
        HTuple hv_SampleKeys, HTuple hv_DLSample, HTuple hv_CurrentWindowHandle,
        HTuple hv_LineWidth, HTuple hv_AnomalyRegionLabelColor,
    ...
    hv_InitialColor = HTuple();
    {
        ...
        HTuple step_val7 = 1;
        for (hv_IndexColor = 0; hv_IndexColor.Continue(end_val7, step_val7);
             hv_IndexColor += step_val7) {
            hv_Color_RGBA =
                    ((("#" + (HTuple(hv_Red[hv_IndexColor]).TupleString("2x"))) +
                      (HTuple(hv_Green[hv_IndexColor]).TupleString("2x"))) +
                     (HTuple(hv_Blue[hv_IndexColor]).TupleString("2x"))) +
                    (HTuple(hv_Alpha[hv_IndexColor]).TupleString("2x"));
            TupleRegexpReplace(hv_Color_RGBA, (HTuple(" ").Append("replace_all")),
                               "0", &hv_Color_RGBA);
    ...
    if (0 != (int((hv_SampleKeys.TupleFind("anomaly_ground_truth")) != -1))) {
        ...
        if (0 != (int(hv_Area > 0))) {
            if (HDevWindowStack::IsOpen())
                SetColor(HDevWindowStack::GetActive(),
                         hv_AnomalyRegionLabelColor + hv_AnomalyColorTransparency);
...
void dev_display_ground_truth_detection(
        HTuple hv_DLSample, HTuple hv_SampleKeys, HTuple hv_LineWidthBbox,
        HTuple hv_ClassIDs, HTuple hv_BboxColors, HTuple hv_BboxLabelColor,
        HTuple hv_WindowImageRatio, HTuple hv_TextColor, HTuple hv_ShowLabels,
    ...
    //
    // This procedure displays the ground truth bounding boxes and masks (if
    // present) of a DLSample.
    //
    hv_InstanceType = "";
    hv_MaskExists = 0;
    if (0 != (int((hv_SampleKeys.TupleFind("bbox_row1")) != -1))) {
        ...
        throw HException(
                    "Ground truth bounding box data could not be found in DLSample.");
    }
    if (0 != (int((hv_SampleKeys.TupleFind("mask")) != -1))) {
        GetDictObject(&ho_InstanceMask, hv_DLSample, "mask");
        hv_MaskExists = 1;
    }
    if (0 != (HTuple(HTuple(int(hv_InstanceType != HTuple("rectangle1")))
                     .TupleAnd(int(hv_InstanceType != HTuple("rectangle2"))))
              .TupleAnd(hv_MaskExists.TupleNot()))) {
        throw HException("Ground truth bounding box or mask data could not be "
                         "found in DLSample.");
    }
    ...
    if (0 != (int((hv_BboxLabels.TupleLength()) > 0))) {
        ...
        if (0 != (int(hv_InstanceType == HTuple("rectangle1")))) {
            ...
            GenRectangle2ContourXld(
                        &ho_BboxRectangle, 0.5 * (hv_BboxRow1 + hv_BboxRow2),
                        0.5 * (hv_BboxCol1 + hv_BboxCol2), hv_BboxPhi,
                        0.5 * (hv_BboxCol2 - hv_BboxCol1), 0.5 * (hv_BboxRow2 - hv_BboxRow1));
            if (0 != hv_ShowLabels) {
                hv_LabelRow = hv_BboxRow1;
                hv_LabelCol = hv_BboxCol1;
            }
        } else if (0 != (int(hv_InstanceType == HTuple("rectangle2")))) {
            GenRectangle2ContourXld(&ho_BboxRectangle, hv_BboxRow, hv_BboxCol,
            ...
            if (0 != hv_ShowDirection) {
                if (0 != (int(hv_ShowDirection == -1))) {
                    ...
                    hv_ArrowBaseRow =
                            hv_BboxRow -
                            ((hv_BboxLength1 - hv_HalfLengthArrow) * (hv_BboxPhi.TupleSin()));
                    hv_ArrowBaseCol =
                            hv_BboxCol +
                            ((hv_BboxLength1 - hv_HalfLengthArrow) * (hv_BboxPhi.TupleCos()));
                    hv_ArrowHeadRow =
                            hv_BboxRow -
                            ((hv_BboxLength1 + hv_HalfLengthArrow) * (hv_BboxPhi.TupleSin()));
                    hv_ArrowHeadCol =
                    ...
                    hv_ArrowHeadSize = (hv_MaxLengthArrow.TupleMin2(
                                            hv_BboxLength1.TupleMin2(hv_BboxLength2))) *
                            hv_ArrowSizeFactorHead;
                } else {
                    hv_ArrowHeadSize = 20.0;
                    hv_ArrowBaseRow = hv_BboxRow;
                    hv_ArrowBaseCol = hv_BboxCol;
                    hv_ArrowHeadRow = hv_BboxRow - ((hv_BboxLength1 + hv_ArrowHeadSize) *
                                                    (hv_BboxPhi.TupleSin()));
                    hv_ArrowHeadCol = hv_BboxCol + ((hv_BboxLength1 + hv_ArrowHeadSize) *
                                                    (hv_BboxPhi.TupleCos()));
                }
                gen_arrow_contour_xld(&ho_OrientationArrows, hv_ArrowBaseRow,
                                      hv_ArrowBaseCol, hv_ArrowHeadRow, hv_ArrowHeadCol,
                                      hv_ArrowHeadSize, hv_ArrowHeadSize);
            }
            ...
            SetLineWidth(HDevWindowStack::GetActive(), hv_LineWidthBbox);
        {
            ...
            HTuple step_val88 = 1;
            for (hv_IndexBbox = 0; hv_IndexBbox.Continue(end_val88, step_val88);
                 hv_IndexBbox += step_val88) {
                ...
                if (HDevWindowStack::IsOpen())
                    SetColor(HDevWindowStack::GetActive(),
                             HTuple(hv_BboxColors[hv_ClassID]) + "60");
                if (0 != hv_MaskExists) {
                    if (HDevWindowStack::IsOpen())
                        SetDraw(HDevWindowStack::GetActive(), "fill");
                ...
                if (0 != (int(hv_InstanceType != HTuple("")))) {
                    ...
                    if (0 != (HTuple(int(hv_InstanceType == HTuple("rectangle2")))
                              .TupleAnd(hv_ShowDirection))) {
                        SelectObj(ho_OrientationArrows, &ho_ArrowSelected,
                                  hv_IndexBbox + 1);
                        if (HDevWindowStack::IsOpen())
                            SetColor(HDevWindowStack::GetActive(),
                                     HTuple(hv_BboxColors[hv_ClassID]) + "FF");
                        if (HDevWindowStack::IsOpen())
        ...
        if (0 != hv_ShowLabels) {
            // For better visibility the text is displayed after all bounding boxes
            // are drawn. Select text color.
            if (0 != (int(hv_TextColor == HTuple("")))) {
                hv_TextColorClasses = HTuple(hv_BboxColors[(*hv_BboxIDs)]);
            } else {
                TupleGenConst((*hv_BboxIDs).TupleLength(), hv_TextColor,
                              &hv_TextColorClasses);
            }
            // Display text.
            if (HDevWindowStack::IsOpen())
                DispText(
                            HDevWindowStack::GetActive(), hv_BboxLabels, "image", hv_LabelRow,
                            hv_LabelCol, hv_TextColorClasses,
                            ((HTuple("box_color").Append("shadow")).Append("border_radius")),
...
void dev_display_map_color_bar(HTuple hv_ImageWidth, HTuple hv_ImageHeight,
                               HTuple hv_MapColorBarWidth, HTuple hv_Colors,
                               HTuple hv_MaxValue, HTuple hv_WindowImageRatio,
        ...
        SetDraw(HDevWindowStack::GetActive(), "fill");
    {
        ...
        for (hv_Row = hv_ImageHeight - 1; hv_Row.Continue(end_val13, step_val13);
             hv_Row += step_val13) {
            ...
            hv_Row2 = hv_Row;
            hv_Column2 =
                    (hv_ImageWidth + 20) + (hv_MapColorBarWidth / hv_WindowImageRatio);
            GenRectangle1(&ho_Rectangle, hv_Row1, hv_Column1, hv_Row2, hv_Column2);
            if (HDevWindowStack::IsOpen())
                SetColor(HDevWindowStack::GetActive(),
                         HTuple(hv_Colors[hv_ColorIndex]));
            if (HDevWindowStack::IsOpen())
    ...
    GetStringExtents(hv_WindowHandle, "0123456789", &hv__, &hv__, &hv__,
                     &hv_TextHeight);
    for (hv_Index = 0; hv_Index <= 1; hv_Index += 0.2) {
        hv_Text = (hv_MaxValue - (hv_Index * hv_MaxValue)).TupleString(".1f");
        if (HDevWindowStack::IsOpen())
            DispText(HDevWindowStack::GetActive(), hv_Text, "image",
                     hv_Index * (hv_ImageHeight -
                                 (2 * (hv_TextHeight / hv_WindowImageRatio))),
                     hv_ImageWidth + (40 / hv_WindowImageRatio), "black", "box",
                     "false");
    }
    //
...
// Chapter: Deep Learning / Anomaly Detection and Global Context Anomaly
// Detection Short Description: Display the detected anomaly regions.
void dev_display_result_anomaly_regions(HObject ho_AnomalyRegion,
                                        HTuple hv_CurrentWindowHandle,
                                        HTuple hv_LineWidth,
    ...
    hv_InitialColor = HTuple();
    {
        ...
        HTuple step_val6 = 1;
        for (hv_IndexColor = 0; hv_IndexColor.Continue(end_val6, step_val6);
             hv_IndexColor += step_val6) {
            hv_Color_RGBA =
                    ((("#" + (HTuple(hv_Red[hv_IndexColor]).TupleString("2x"))) +
                      (HTuple(hv_Green[hv_IndexColor]).TupleString("2x"))) +
                     (HTuple(hv_Blue[hv_IndexColor]).TupleString("2x"))) +
...
void dev_display_result_detection(
        HTuple hv_DLResult, HTuple hv_ResultKeys, HTuple hv_LineWidthBbox,
        HTuple hv_ClassIDs, HTuple hv_TextConf, HTuple hv_Colors,
        HTuple hv_BoxLabelColor, HTuple hv_WindowImageRatio,
        HTuple hv_TextPositionRow, HTuple hv_TextColor, HTuple hv_ShowLabels,
        HTuple hv_ShowDirection, HTuple hv_WindowHandle,
        HTuple *hv_BboxClassIndices) {
    ...
    //
    // This procedure displays the bounding boxes and masks (if present) defined
    // by a DLResult. The ClassIDs are necessary to display bounding boxes from
    // the same class always with the same color.
    //
    hv_InstanceType = "";
    hv_MaskExists = 0;
    if (0 != (int((hv_ResultKeys.TupleFind("bbox_row1")) != -1))) {
        ...
        throw HException(
                    "Result bounding box data could not be found in DLResult.");
    }
    ...
    if (0 != (HTuple(HTuple(int(hv_InstanceType != HTuple("rectangle1")))
              ...
              .TupleAnd(hv_MaskExists.TupleNot()))) {
        throw HException(
                    "Result bounding box or mask data could not be found in DLSample.");
    }
    GetDictTuple(hv_DLResult, "bbox_class_id", &hv_BboxClasses);
    if (0 != (int((hv_BboxClasses.TupleLength()) > 0))) {
        ...
        // Draw bounding boxes.
        {
            ...
            HTuple step_val96 = 1;
            for (hv_IndexBbox = 0; hv_IndexBbox.Continue(end_val96, step_val96);
                 hv_IndexBbox += step_val96) {
                hv_ClassID =
                        hv_ClassIDs.TupleFind(HTuple(hv_BboxClasses[hv_IndexBbox]));
                ...
                if (0 != hv_MaskExists) {
                    ...
                    if (HDevWindowStack::IsOpen())
                        SetColor(HDevWindowStack::GetActive(),
                                 HTuple(hv_Colors[hv_ClassID]) + "80");
                    if (HDevWindowStack::IsOpen())
                        DispObj(ho_MaskSelected, HDevWindowStack::GetActive());
                    if (HDevWindowStack::IsOpen())
                        SetDraw(HDevWindowStack::GetActive(), "margin");
                }
                {
                    ...
                    for (hv_IndexStyle = 0;
                         hv_IndexStyle.Continue(end_val108, step_val108);
                         hv_IndexStyle += step_val108) {
                        if (HDevWindowStack::IsOpen())
                            SetColor(HDevWindowStack::GetActive(),
                                     HTuple(hv_CurrentColors[hv_IndexStyle]));
                        if (HDevWindowStack::IsOpen())
                            SetLineWidth(HDevWindowStack::GetActive(),
                                         HTuple(hv_LineWidths[hv_IndexStyle]));
                        if (0 != (int(hv_InstanceType != HTuple("")))) {
                            SelectObj(ho_BboxRectangle, &ho_RectangleSelected,
                                      hv_IndexBbox + 1);
                            if (HDevWindowStack::IsOpen())
                                DispObj(ho_RectangleSelected, HDevWindowStack::GetActive());
                            if (0 != (HTuple(int(hv_InstanceType == HTuple("rectangle2")))
        ...
        if (0 != hv_ShowLabels) {
            // For better visibility the text is displayed after all bounding boxes
            // are drawn. Get text and text size for correct positioning of result
            // class IDs.
            hv_Text = hv_BboxClasses + hv_TextConf;
            // Select text color.
            if (0 != (int(hv_TextColor == HTuple("")))) {
                ...
                TupleGenConst((*hv_BboxClassIndices).TupleLength(), hv_TextColor,
                              &hv_TextColorClasses);
            }
            ...
            if (HDevWindowStack::IsOpen())
                DispText(
                            HDevWindowStack::GetActive(), hv_Text, "image", hv_LabelRow,
                            hv_LabelCol, hv_TextColorClasses,
                            ((HTuple("box_color").Append("shadow")).Append("border_radius")),
...
void dev_display_segmentation_regions(HObject ho_SegmentationImage,
                                      HTuple hv_ClassIDs,
                                      HTuple hv_ColorsSegmentation,
                                      HTuple hv_ExcludeClassIDs,
    ...
    Threshold(ho_SegmentationImage, &ho_Regions, hv_IncludedClassIDs,
              hv_IncludedClassIDs);
    //
    // Get classes with non-empty regions.
    RegionFeatures(ho_Regions, "area", &hv_Area);
    if (0 !=
            (int((hv_Area.TupleLength()) != (hv_IncludedClassIDs.TupleLength())))) {
        throw HException("No equal number of class IDs and segmentation regions.");
    }
    TupleSelectMask(hv_IncludedClassIDs, hv_Area.TupleGreaterElem(0),
                    &(*hv_ImageClassIDs));
    //
    // Display all non-empty class regions in distinct colors.
    {
        ...
        HTuple step_val22 = 1;
        for (hv_Index = 0; hv_Index.Continue(end_val22, step_val22);
             hv_Index += step_val22) {
            if (0 != (int(HTuple(hv_Area[hv_Index]) > 0))) {
                ...
                if (HDevWindowStack::IsOpen())
                    SetColor(HDevWindowStack::GetActive(),
                             HTuple(hv_ColorsSegmentation[hv_IndexColor]));
...
void dev_display_weight_regions(HObject ho_ImageWeight,
                                HTuple hv_DrawTransparency,
    ...
    if (0 != (int((hv_GrayVal.TupleMax()) > hv_SegMaxWeight))) {
        throw HException(
                    ((("The maximum weight (" + (hv_GrayVal.TupleMax())) +
                      ") in the weight image is greater than the given SegMaxWeight (") +
                     hv_SegMaxWeight) +
                    ").");
    }
    //
    while (0 != (int(hv_GrayVal != HTuple()))) {
        ...
        Threshold(ho_ImageWeight, &ho_WeightsRegion, hv_GrayValWeight,
                  hv_GrayValWeight);
        //
        // Visualize the respective group.
        hv_ColorIndex = (((hv_GrayValWeight / hv_SegMaxWeight) * (hv_NumColors - 1))
                         .TupleCeil())
                .TupleInt();
...
void dev_open_window_fit_size(HTuple hv_Row, HTuple hv_Column, HTuple hv_Width,
    ...
    if (0 != (HTuple(int((hv_WidthLimit.TupleLength()) == 0))
              .TupleOr(int(hv_WidthLimit < 0)))) {
        hv_MinWidth = 500;
        hv_MaxWidth = 800;
    ...
    if (0 != (HTuple(int((hv_HeightLimit.TupleLength()) == 0))
              .TupleOr(int(hv_HeightLimit < 0)))) {
        hv_MinHeight = 400;
        hv_MaxHeight = 600;
    ...
    if (0 != (HTuple(int(hv_MinWidth > hv_Width))
              .TupleOr(int(hv_MinHeight > hv_Height)))) {
        hv_ResizeFactor = (((hv_MinWidth.TupleReal()) / hv_Width)
                           .TupleConcat((hv_MinHeight.TupleReal()) / hv_Height))
                .TupleMax();
    }
    hv_TempWidth = hv_Width * hv_ResizeFactor;
    hv_TempHeight = hv_Height * hv_ResizeFactor;
    // Then, shrink window to maximum extents (if necessary).
    if (0 != (HTuple(int(hv_MaxWidth < hv_TempWidth))
              .TupleOr(int(hv_MaxHeight < hv_TempHeight)))) {
        hv_ResizeFactor =
                hv_ResizeFactor *
                ((((hv_MaxWidth.TupleReal()) / hv_TempWidth)
                  .TupleConcat((hv_MaxHeight.TupleReal()) / hv_TempHeight))
                 .TupleMin());
    }
    ...
    OpenWindow(hv_Row, hv_Column, hv_WindowWidth, hv_WindowHeight, 0, "visible",
               "", &(*hv_WindowHandle));
...
// Chapter: Develop
// Short Description: Switch dev_update_pc, dev_update_var, and
// dev_update_window to 'off'.
void dev_update_off() {
...
// Short Description: Filter the instance segmentation masks of a DL sample
// based on a given selection.
void filter_dl_sample_instance_segmentation_masks(HTuple hv_DLSample,
    ...
    if (0 != hv_MaskKeyExists) {
        ...
        if (0 != (int(hv_Indices == -1))) {
            ...
            // key 'mask'. However, this would be an unwanted big change in the
            // dictionary.
...
// Chapter: Deep Learning / Model
// Short Description: Retrieve the indices of Samples that contain KeyName
// matching KeyValue according to the Mode set.
void find_dl_samples(HTuple hv_Samples, HTuple hv_KeyName, HTuple hv_KeyValue,
    ...
    if (0 != (int((hv_KeyName.TupleLength()) != 1))) {
        throw HException(HTuple("Invalid KeyName size: ") +
                         (hv_KeyName.TupleLength()));
    }
    if (0 != (int((hv_Mode.TupleLength()) != 1))) {
        throw HException(HTuple("Invalid Mode size: ") + (hv_Mode.TupleLength()));
    }
    if (0 != (HTuple(HTuple(int(hv_Mode != HTuple("match")))
                     .TupleAnd(int(hv_Mode != HTuple("or"))))
              .TupleAnd(int(hv_Mode != HTuple("contain"))))) {
        throw HException("Invalid Mode value: " + hv_Mode);
    }
    hv_NumKeyValues = hv_KeyValue.TupleLength();
    if (0 != (HTuple(int(hv_Mode == HTuple("contain")))
              .TupleAnd(int(hv_NumKeyValues < 1)))) {
        throw HException("Invalid KeyValue size for contain Mode: " +
                         hv_NumKeyValues);
    }
    //
    // Find the indices.
    ...
    //
    {
        ...
        HTuple step_val24 = 1;
        for (hv_SampleIndex = 0; hv_SampleIndex.Continue(end_val24, step_val24);
             hv_SampleIndex += step_val24) {
            ...
            if (0 != hv_KeyExists) {
                ...
                if (0 != (int(hv_Mode == HTuple("match")))) {
                ...
                } else if (0 != (HTuple(int(hv_Mode == HTuple("or")))
                                 .TupleAnd(int((hv_Tuple.TupleLength()) == 1)))) {
                    // Mode 'or': Tuple must have only 1 element and it has to be equal to
                    // any of KeyValues elements.
                    ...
                    // Mode 'contain': Tuple must contain any of the elements in KeyValue.
                    {
                        ...
                        HTuple step_val37 = 1;
                        for (hv_ValueIndex = 0;
                             hv_ValueIndex.Continue(end_val37, step_val37);
                             hv_ValueIndex += step_val37) {
    ...
    (*hv_SampleIndices) =
            (*hv_SampleIndices).TupleSelectRange(0, hv_NumFound - 1);
    return;
}
...
void gen_arrow_contour_xld(HObject *ho_Arrow, HTuple hv_Row1, HTuple hv_Column1,
    ...
    hv_RowP1 = (hv_Row1 + ((hv_Length - hv_HeadLength) * hv_DR)) +
            (hv_HalfHeadWidth * hv_DC);
    hv_ColP1 = (hv_Column1 + ((hv_Length - hv_HeadLength) * hv_DC)) -
            (hv_HalfHeadWidth * hv_DR);
    hv_RowP2 = (hv_Row1 + ((hv_Length - hv_HeadLength) * hv_DR)) -
            (hv_HalfHeadWidth * hv_DC);
    hv_ColP2 = (hv_Column1 + ((hv_Length - hv_HeadLength) * hv_DC)) +
            (hv_HalfHeadWidth * hv_DR);
    //
    // Finally create output XLD contour for each input point pair
    {
        ...
        HTuple step_val45 = 1;
        for (hv_Index = 0; hv_Index.Continue(end_val45, step_val45);
             hv_Index += step_val45) {
            if (0 != (int(HTuple(hv_Length[hv_Index]) == -1))) {
                // Create_ single points for arrows with identical start and end point
                GenContourPolygonXld(&ho_TempArrow, HTuple(hv_Row1[hv_Index]),
                                     HTuple(hv_Column1[hv_Index]));
            } else {
                // Create arrow contour
                GenContourPolygonXld(&ho_TempArrow,
                                     ((((HTuple(hv_Row1[hv_Index])
                                         .TupleConcat(HTuple(hv_Row2[hv_Index])))
                                        .TupleConcat(HTuple(hv_RowP1[hv_Index])))
                                       .TupleConcat(HTuple(hv_Row2[hv_Index])))
                                      .TupleConcat(HTuple(hv_RowP2[hv_Index])))
                                     .TupleConcat(HTuple(hv_Row2[hv_Index])),
                                     ((((HTuple(hv_Column1[hv_Index])
                                         .TupleConcat(HTuple(hv_Column2[hv_Index])))
                                        .TupleConcat(HTuple(hv_ColP1[hv_Index])))
                                       .TupleConcat(HTuple(hv_Column2[hv_Index])))
                                      .TupleConcat(HTuple(hv_ColP2[hv_Index])))
                                     .TupleConcat(HTuple(hv_Column2[hv_Index])));
            }
...
// Chapter: OCR / Deep OCR
// Short Description: Generate ground truth characters if they don't exist and
// words to characters mapping.
void gen_dl_ocr_detection_gt_chars(
        HTuple hv_DLSampleTargets, HTuple hv_DLSample, HTuple hv_ScaleWidth,
        HTuple hv_ScaleHeight,
        HTupleVector /*{eTupleVector,Dim=1}*/ *hvec_WordsCharsMapping) {
    ...
    if (0 !=
            (int(((hv_DLSample.TupleGetDictTuple("bbox_label_id")).TupleLength()) >
                 0))) {
        // Check if chars GT exist otherwise generate them.
        TupleFindFirst(hv_DLSample.TupleGetDictTuple("bbox_label_id"), 1,
                       &hv_CharBoxIndex);
        if (0 != (int(hv_CharBoxIndex == -1))) {
            hv_WordLengths = (hv_DLSample.TupleGetDictTuple("word")).TupleStrlen();
            (*hvec_WordsCharsMapping)
                    [((hv_DLSample.TupleGetDictTuple("bbox_label_id")).TupleLength()) -
                    1] = HTupleVector(HTuple());
            {
                HTuple end_val7 =
                        ((hv_DLSample.TupleGetDictTuple("bbox_label_id")).TupleLength()) -
                        1;
                ...
                for (hv_J = 0; hv_J.Continue(end_val7, step_val7); hv_J += step_val7) {
                    // For each word box
                    if (0 != (int(HTuple((hv_DLSample.TupleGetDictTuple(
                                              "bbox_label_id"))[hv_J]) == 0))) {
                        if (0 != (int(HTuple(hv_WordLengths[hv_J]) != 0))) {
                            hv_Start = (hv_DLSampleTargets.TupleGetDictTuple("bbox_label_id"))
                                    .TupleLength();
                            hv_End = (((hv_DLSampleTargets.TupleGetDictTuple("bbox_label_id"))
                                       .TupleLength()) -
                                      1) +
                                    HTuple(hv_WordLengths[hv_J]);
                            (*hvec_WordsCharsMapping)[hv_J] =
                                    HTupleVector(HTuple::TupleGenSequence(hv_Start, hv_End, 1));
                            split_rectangle2(
                                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_row"))[hv_J]),
                                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_col"))[hv_J]),
                                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_phi"))[hv_J]),
                                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_length1"))[hv_J]),
                                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_length2"))[hv_J]),
                                        HTuple(hv_WordLengths[hv_J]), &hv_SplitRow, &hv_SplitColumn,
                                        &hv_SplitPhi, &hv_SplitLength1, &hv_SplitLength2);
                            TupleGenConst(HTuple(hv_WordLengths[hv_J]), 1, &hv_CharsIds);
                            TupleGenConst(HTuple(hv_WordLengths[hv_J]), "",
                                          &hv_EmptyWordStrings);
                            SetDictTuple(
                                        hv_DLSampleTargets, "bbox_label_id",
                                        (hv_DLSampleTargets.TupleGetDictTuple("bbox_label_id"))
                                        .TupleConcat(hv_CharsIds));
                            SetDictTuple(hv_DLSampleTargets, "bbox_row",
                                         (hv_DLSampleTargets.TupleGetDictTuple("bbox_row"))
                                         .TupleConcat(hv_SplitRow));
                            SetDictTuple(hv_DLSampleTargets, "bbox_col",
                                         (hv_DLSampleTargets.TupleGetDictTuple("bbox_col"))
                                         .TupleConcat(hv_SplitColumn));
                            SetDictTuple(hv_DLSampleTargets, "bbox_phi",
                                         (hv_DLSampleTargets.TupleGetDictTuple("bbox_phi"))
                                         .TupleConcat(hv_SplitPhi));
                            SetDictTuple(
                                        hv_DLSampleTargets, "bbox_length1",
                                        (hv_DLSampleTargets.TupleGetDictTuple("bbox_length1"))
                                        .TupleConcat(hv_SplitLength1 * hv_ScaleWidth));
                            SetDictTuple(
                                        hv_DLSampleTargets, "bbox_length2",
                                        (hv_DLSampleTargets.TupleGetDictTuple("bbox_length2"))
                                        .TupleConcat(hv_SplitLength2 * hv_ScaleHeight));
                            SetDictTuple(hv_DLSampleTargets, "word",
                                         (hv_DLSampleTargets.TupleGetDictTuple("word"))
                                         .TupleConcat(hv_EmptyWordStrings));
                        } else {
                            throw HException(
                                        ((("Sample with image id " +
                                           (hv_DLSample.TupleGetDictTuple("image_id"))) +
                                          " is not valid. The word bounding box at index ") +
                                         hv_J) +
                                        " has an empty string as the ground truth. This is not "
                                        "allowed. Please assign a word label to every word bounding "
                                        "box.");
                        }
                    }
                }
...
void gen_dl_ocr_detection_gt_link_map(
        HObject *ho_GtLinkMap, HTuple hv_ImageWidth, HTuple hv_ImageHeight,
        HTuple hv_DLSampleTargets,
        HTupleVector /*{eTupleVector,Dim=1}*/ hvec_WordToCharVec, HTuple hv_Alpha) {
    ...
    hv_DiameterC =
            2 *
            ((hv_DLSampleTargets.TupleGetDictTuple("bbox_length1"))
             .TupleHypot(hv_DLSampleTargets.TupleGetDictTuple("bbox_length2")));
    // Loop over word boxes.
    {
        HTuple end_val10 = ((hv_DLSampleTargets.TupleGetDictTuple("bbox_label_id"))
                            .TupleLength()) -
                1;
        HTuple step_val10 = 1;
        for (hv_IndexW = 0; hv_IndexW.Continue(end_val10, step_val10);
             hv_IndexW += step_val10) {
            // For each word box
            if (0 != (int(HTuple((hv_DLSampleTargets.TupleGetDictTuple(
                                      "bbox_label_id"))[hv_IndexW]) == 0))) {
                ...
                if (0 != (int((hv_CharBoxIndices.TupleLength()) == 0))) {
                    ...
                    GenCircle(
                                &ho_Lines, HTuple(hv_CRow[hv_CharBoxIndices]),
                                HTuple(hv_CCol[hv_CharBoxIndices]),
                                (((0.5 * hv_Alpha) * HTuple(hv_DiameterC[hv_CharBoxIndices]))
                                 .TupleRound()) +
                                0.5);
                } else {
                    ...
                    hv_CharDistToWordCenter =
                            (hv_CharCRows - HTuple(hv_CRow[hv_IndexW]))
                            .TupleHypot(hv_CharCCols - HTuple(hv_CCol[hv_IndexW]));
                    hv_ExtremeCharIndex =
                            ((const HTuple &)HTuple(hv_CharDistToWordCenter.TupleSortIndex()))
                            [(hv_CharDistToWordCenter.TupleLength()) - 1];
                    hv_DistToExtreme =
                            (hv_CharCRows - HTuple(hv_CharCRows[hv_ExtremeCharIndex]))
                            .TupleHypot(hv_CharCCols -
                                        HTuple(hv_CharCCols[hv_ExtremeCharIndex]));
                    ...
                    hv_Box1Idx = hv_CharIndexSorted.TupleSelectRange(
                                0, (hv_CharIndexSorted.TupleLength()) - 2);
                    hv_Box2Idx = hv_CharIndexSorted.TupleSelectRange(
                                1, (hv_CharIndexSorted.TupleLength()) - 1);
                    // Generate link lines between each pair of adjacent characters.
                    GenRegionLine(&ho_Lines, HTuple(hv_CharCRows[hv_Box1Idx]),
                                  HTuple(hv_CharCCols[hv_Box1Idx]),
                                  HTuple(hv_CharCRows[hv_Box2Idx]),
                                  HTuple(hv_CharCCols[hv_Box2Idx]));
                    // Dilate the lines by 0.5/1.5/2.5/... pixels, such that the line
                    // thickness is approximately Alpha*mean(D1, D2)
                    hv_Diameter1 =
                            HTuple(hv_DiameterC[HTuple(hv_CharBoxIndices[hv_Box1Idx])]);
                    hv_Diameter2 =
                            HTuple(hv_DiameterC[HTuple(hv_CharBoxIndices[hv_Box2Idx])]);
                    hv_DilationRadius =
                            (((0.25 * hv_Alpha) * (hv_Diameter1 + hv_Diameter2))
                             .TupleRound()) +
                            0.5;
                    // dilation_circle only accepts a single radius, so we need to loop
                    // over the lines.
                    CountObj(ho_Lines, &hv_NumLines);
                    {
                        HTuple end_val39 = hv_NumLines;
                        HTuple step_val39 = 1;
                        for (hv_Index = 1; hv_Index.Continue(end_val39, step_val39);
                             hv_Index += step_val39) {
                            SelectObj(ho_Lines, &ho_Line, hv_Index);
                            DilationCircle(ho_Line, &ho_LineDilated,
                                           HTuple(hv_DilationRadius[hv_Index - 1]));
...
// Chapter: OCR / Deep OCR
// Short Description: Generate target orientation score maps for ocr detection
// training.
void gen_dl_ocr_detection_gt_orientation_map(HObject *ho_GtOrientationMaps,
    ...
    if (0 !=
        ...
        if (0 != (int(hv_Indices != -1))) {
            ...
            GenRectangle2(
                        &ho_Region,
                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_row"))[hv_Indices]),
                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_col"))[hv_Indices]),
                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_phi"))[hv_Indices]),
                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_length1"))[hv_Indices]),
                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_length2"))[hv_Indices]));
            OverpaintRegion(ho_GtOrientationSin, ho_Region,
                            HTuple(hv_Phi[hv_Indices]).TupleSin(), "fill");
            OverpaintRegion(ho_GtOrientationCos, ho_Region,
                            HTuple(hv_Phi[hv_Indices]).TupleCos(), "fill");
        }
    }
...
void gen_dl_ocr_detection_gt_score_map(HObject *ho_TargetText,
                                       HTuple hv_DLSample, HTuple hv_BoxCutoff,
                                       HTuple hv_RenderCutoff,
                                       HTuple hv_ImageWidth,
                                       HTuple hv_ImageHeight) {
    ...
    {
        HTuple end_val5 =
                ((hv_DLSample.TupleGetDictTuple("bbox_label_id")).TupleLength()) - 1;
        HTuple step_val5 = 1;
        for (hv_Index = 0; hv_Index.Continue(end_val5, step_val5);
             hv_Index += step_val5) {
            // For each char box
            if (0 != (HTuple(int(HTuple((hv_DLSample.TupleGetDictTuple(
                                             "bbox_label_id"))[hv_Index]) == 1))
                      .TupleAnd(int(hv_BoxCutoff != 0)))) {
                // Compute the sigma of an unnormalized normal distribution, such that
                // a certain threshold value is reached at the interval of a certain
                // size.
                hv_Sigma1 =
                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_length1"))[hv_Index]) *
                        ((-0.5 / (hv_BoxCutoff.TupleLog())).TupleSqrt());
                hv_Sigma2 =
                        HTuple((hv_DLSample.TupleGetDictTuple("bbox_length2"))[hv_Index]) *
                        ((-0.5 / (hv_BoxCutoff.TupleLog())).TupleSqrt());
                if (0 !=
                        (HTuple(HTuple(int(hv_Sigma1 != 0)).TupleAnd(int(hv_Sigma2 != 0)))
                         .TupleAnd(int(hv_RenderCutoff != 0)))) {
                    // Compute the radius of an unnormalized normal distribution,
                    // where a certain threshold value is reached at the end.
                    hv_ExtendedLength1 =
                            hv_Sigma1 * ((-2 * (hv_RenderCutoff.TupleLog())).TupleSqrt());
                    hv_ExtendedLength2 =
                            hv_Sigma2 * ((-2 * (hv_RenderCutoff.TupleLog())).TupleSqrt());
                    GenRectangle2(
                                &ho_ExtendedRectangle,
                                HTuple((hv_DLSample.TupleGetDictTuple("bbox_row"))[hv_Index]),
                                HTuple((hv_DLSample.TupleGetDictTuple("bbox_col"))[hv_Index]),
                                HTuple((hv_DLSample.TupleGetDictTuple("bbox_phi"))[hv_Index]),
                                hv_ExtendedLength1, hv_ExtendedLength2);
                    ClipRegion(ho_ExtendedRectangle, &ho_ExtendedRectangle, 0, 0,
                               hv_ImageHeight - 1, hv_ImageWidth - 1);
                    ...
                    if (0 != (int(hv_Area > 1))) {
                        ...
                        HomMat2dRotate(
                                    ...
                                    -HTuple((hv_DLSample.TupleGetDictTuple("bbox_phi"))[hv_Index]),
                                    0, 0, &hv_HomMat2D);
                        AffineTransPoint2d(hv_HomMat2D, hv_Rows, hv_Columns, &hv_DistRow,
                                           &hv_DistCol);
                        hv_ScaledGaussian =
                                (-0.5 * (((hv_DistCol * hv_DistCol) / (hv_Sigma1 * hv_Sigma1)) +
                                         ((hv_DistRow * hv_DistRow) / (hv_Sigma2 * hv_Sigma2))))
                                .TupleExp();
                        GetGrayval((*ho_TargetText), hv_Rows, hv_Columns, &hv_Grayval);
                        SetGrayval((*ho_TargetText), hv_Rows, hv_Columns,
                                   hv_ScaledGaussian.TupleMax2(hv_Grayval));
                    }
                }
            }
        }
...
// Chapter: OCR / Deep OCR
// Short Description: Preprocess dl samples and generate targets and weights for
// ocr detection training.
void gen_dl_ocr_detection_targets(HTuple hv_DLSampleOriginal,
                                  HTuple hv_DLPreprocessParam) {
    ...
    // They make the the uniformly sized char boxes a bit smaller, as we can
    // expect a spacing between the characters.
    hv_ScaleHeight = 0.9;
    hv_ScaleWidth = 0.8;
    ...
    hv_Alpha = 0.1;
    // Parameter used to determine the dilation radius of word boxes in the weight
    // score map.
    hv_WSWeightRenderThreshold = 0.05;
    // Parameter represents the dilation radius of word lines in the weight link
    // map.
    hv_LinkZeroWeightRadius = 2.5;
    // Confidence is here only a place holder for the fallback weak supervision
    // case.
    hv_Confidence = 1.0;
    ...
    HomMat2dIdentity(&hv_HomMat2DIdentity);
    HomMat2dScale(hv_HomMat2DIdentity, 1.0 / hv_Stride, 1.0 / hv_Stride, 0, 0,
                  &hv_HomMat2DScale);
    AffineTransPoint2d(hv_HomMat2DScale,
                       hv_DLSample.TupleGetDictTuple("bbox_col"),
                       hv_DLSample.TupleGetDictTuple("bbox_row"),
                       &hv___Tmp_Ctrl_0, &hv___Tmp_Ctrl_1);
    SetDictTuple(hv_DLSample, "bbox_row", hv___Tmp_Ctrl_1);
    SetDictTuple(hv_DLSample, "bbox_col", hv___Tmp_Ctrl_0);
    SetDictTuple(hv_DLSample, "bbox_length1",
                 (hv_DLSample.TupleGetDictTuple("bbox_length1")) / hv_Stride);
    SetDictTuple(hv_DLSample, "bbox_length2",
                 (hv_DLSample.TupleGetDictTuple("bbox_length2")) / hv_Stride);
    ...
    gen_dl_ocr_detection_gt_score_map(&ho_TargetText, hv_DLSampleTargets,
                                      hv_BoxCutoff, hv_RenderCutoff,
                                      hv_ScoreMapsWidth, hv_ScoreMapsHeight);
    gen_dl_ocr_detection_gt_link_map(&ho_TargetLink, hv_ScoreMapsWidth,
                                     hv_ScoreMapsHeight, hv_DLSampleTargets,
                                     hvec_WordsCharsMapping, hv_Alpha);
    gen_dl_ocr_detection_gt_orientation_map(&ho_TargetOrientation,
                                            hv_ScoreMapsWidth, hv_ScoreMapsHeight,
                                            hv_DLSampleTargets);
    // Generate weight maps from WordRegions and CharBoxes.
    gen_dl_ocr_detection_weight_score_map(
                &ho_TargetWeightText, hv_ScoreMapsWidth, hv_ScoreMapsHeight,
                hv_DLSampleTargets, hv_BoxCutoff, hv_WSWeightRenderThreshold,
                hv_Confidence);
    gen_dl_ocr_detection_weight_link_map(ho_TargetLink, ho_TargetWeightText,
                                         &ho_TargetWeightLink,
                                         hv_LinkZeroWeightRadius);
    MultImage(ho_TargetText, ho_TargetWeightText, &ho_WeightedCharScore, 1, 0);
    gen_dl_ocr_detection_weight_orientation_map(
                ho_WeightedCharScore, &ho_TargetWeightOrientation, hv_DLSampleTargets);
    // Take account of the image domain in DLSampleOriginal.
    GetDomain(hv_DLSampleOriginal.TupleGetDictObject("image"),
              &ho_OriginalDomain);
    AreaCenter(ho_OriginalDomain, &hv_OriginalDomainArea, &hv__, &hv__);
    GetImageSize(hv_DLSampleOriginal.TupleGetDictObject("image"),
                 &hv_OriginalWidth, &hv_OriginalHeight);
    hv_IsOriginalDomainFull =
            int(hv_OriginalDomainArea == (hv_OriginalWidth * hv_OriginalHeight));
    if (0 != (hv_IsOriginalDomainFull.TupleNot())) {
        ...
        ZoomImageSize(ho_Image, &ho_DomainWeight, hv_ScoreMapsWidth,
        ...
        for (hv_ChannelIdx = 1; hv_ChannelIdx <= 2; hv_ChannelIdx += 1) {
            AccessChannel(ho_TargetOrientation, &ho_TargetOrientationChannel,
                          hv_ChannelIdx);
            AccessChannel(ho_TargetWeightOrientation,
                          &ho_TargetWeightOrientationChannel, hv_ChannelIdx);
            MultImage(ho_DomainWeight, ho_TargetOrientationChannel,
                      &ho_TargetOrientationChannel, 1, 0);
            MultImage(ho_DomainWeight, ho_TargetWeightOrientationChannel,
                      &ho_TargetWeightOrientationChannel, 1, 0);
            AppendChannel(ho_TargetOrientationOut, ho_TargetOrientationChannel,
                          &ho_TargetOrientationOut);
            AppendChannel(ho_TargetWeightOrientationOut,
                          ho_TargetWeightOrientationChannel,
    ...
    SetDictObject(ho_TargetOrientation, hv_DLSampleOriginal,
                  "target_orientation");
    ...
    SetDictObject(ho_TargetWeightOrientation, hv_DLSampleOriginal,
                  "target_weight_orientation");
}

// Chapter: OCR / Deep OCR
// Short Description: Generate link score map weight for ocr detection training.
void gen_dl_ocr_detection_weight_link_map(HObject ho_LinkMap,
                                          HObject ho_TargetWeight,
                                          HObject *ho_TargetWeightLink,
                                          HTuple hv_LinkZeroWeightRadius) {
    ...
    if (0 != (int(hv_LinkZeroWeightRadius > 0))) {
        ...
        ClipRegion(ho_RegionComplement, &ho_RegionComplement, 0, 0, hv_Height - 1,
                   hv_Width - 1);
        Union2(ho_LinkRegion, ho_RegionComplement, &ho_RegionUnion);
        Complement(ho_RegionUnion, &ho_RegionBorder);
        PaintRegion(ho_RegionBorder, ho_TargetWeight, &(*ho_TargetWeightLink), 0,
                    "fill");
    } else {
...
// Chapter: OCR / Deep OCR
// Short Description: Generate orientation score map weight for ocr detection
// training.
void gen_dl_ocr_detection_weight_orientation_map(
        HObject ho_InitialWeight, HObject *ho_OrientationTargetWeight,
        HTuple hv_DLSample) {
    ...
    if (0 !=
        ...
        if (0 != (int(hv_Indices != -1))) {
            ...
            OverpaintRegion((*ho_OrientationTargetWeight), ho_BackgroundRegion, 0,
                            "fill");
        }
    }
    // We need two channels: for Sin and Cos
    Compose2((*ho_OrientationTargetWeight), (*ho_OrientationTargetWeight),
             &(*ho_OrientationTargetWeight));
    return;
}

// Chapter: OCR / Deep OCR
// Short Description: Generate text score map weight for ocr detection training.
void gen_dl_ocr_detection_weight_score_map(
        HObject *ho_TargetWeightText, HTuple hv_ImageWidth, HTuple hv_ImageHeight,
        HTuple hv_DLSample, HTuple hv_BoxCutoff, HTuple hv_WSWeightRenderThreshold,
    ...
    if (0 !=
        ...
        }
        {
            HTuple end_val9 =
                    ((hv_DLSample.TupleGetDictTuple("bbox_label_id")).TupleLength()) - 1;
            HTuple step_val9 = 1;
            for (hv_WordIndex = 0; hv_WordIndex.Continue(end_val9, step_val9);
                 hv_WordIndex += step_val9) {
                // For each word box
                if (0 != (int(HTuple((hv_DLSample.TupleGetDictTuple(
                                          "bbox_label_id"))[hv_WordIndex]) == 0))) {
                    if (0 != (HTuple(HTuple(int(hv_BoxCutoff == 0))
                                     .TupleOr(int(hv_WSWeightRenderThreshold == 0)))
                              .TupleNot())) {
                        hv_SigmaL2 = HTuple((hv_DLSample.TupleGetDictTuple(
                                                 "bbox_length2"))[hv_WordIndex]) *
                                ((-0.5 / (hv_BoxCutoff.TupleLog())).TupleSqrt());
                        hv_WordLength2Ext =
                                hv_SigmaL2 *
                                ((-2 * (hv_WSWeightRenderThreshold.TupleLog())).TupleSqrt());
                        hv_DilationRadius =
                                hv_WordLength2Ext - HTuple((hv_DLSample.TupleGetDictTuple(
                                                                "bbox_length2"))[hv_WordIndex]);
                    } else {
                        hv_DilationRadius = 0;
                    }
                    GenRectangle2(
                                &ho_WordRegion,
                                HTuple((hv_DLSample.TupleGetDictTuple("bbox_row"))[hv_WordIndex]),
                                HTuple((hv_DLSample.TupleGetDictTuple("bbox_col"))[hv_WordIndex]),
                                HTuple((hv_DLSample.TupleGetDictTuple("bbox_phi"))[hv_WordIndex]),
                                HTuple((
                                           hv_DLSample.TupleGetDictTuple("bbox_length1"))[hv_WordIndex]),
                                HTuple((hv_DLSample.TupleGetDictTuple(
                                            "bbox_length2"))[hv_WordIndex]));
                    // Slightly enlarge the weight region to suppress halos at the box
                    // borders.
                    if (0 != (int(hv_DilationRadius >= 0.5))) {
                        DilationCircle(ho_WordRegion, &ho_WordRegionDilated,
                                       hv_DilationRadius);
                    } else {
                        ho_WordRegionDilated = ho_WordRegion;
                    }
                    // Set the confidence as weight for the word region.
                    OverpaintRegion((*ho_TargetWeightText), ho_WordRegionDilated,
                                    hv_Confidence, "fill");
                }
            }
        }
...
void gen_dl_samples_from_images(HObject ho_Images, HTuple *hv_DLSampleBatch) {
    ...
    // Loop through all given images.
    {
        ...
        HTuple step_val10 = 1;
        for (hv_ImageIndex = 0; hv_ImageIndex.Continue(end_val10, step_val10);
             hv_ImageIndex += step_val10) {
...
void gen_words_chars_mapping(
    ...
    if (0 !=
            (int(((hv_DLSample.TupleGetDictTuple("bbox_label_id")).TupleLength()) >
                 0))) {
        TupleFind(hv_DLSample.TupleGetDictTuple("bbox_label_id"), 0,
                  &hv_WordsIndices);
        TupleFind(hv_DLSample.TupleGetDictTuple("bbox_label_id"), 1,
                  &hv_CharsIndices);
        if (0 != (HTuple(int(hv_CharsIndices != -1))
                  .TupleAnd(int(hv_WordsIndices != -1)))) {
            hv_WordLengths =
                    HTuple((hv_DLSample.TupleGetDictTuple("word"))[hv_WordsIndices])
                    .TupleStrlen();
            // Init vector.
            (*hvec_WordsCharsMapping)
                    [((hv_DLSample.TupleGetDictTuple("bbox_label_id")).TupleLength()) -
                    1] = HTupleVector(HTuple());
            hv_WordArea =
                    (4 * HTuple((hv_DLSample.TupleGetDictTuple(
                                     "bbox_length1"))[hv_WordsIndices])) *
                    HTuple(
                        (hv_DLSample.TupleGetDictTuple("bbox_length2"))[hv_WordsIndices]);
            hv_CharArea =
                    (4 * HTuple((hv_DLSample.TupleGetDictTuple(
                                     "bbox_length1"))[hv_CharsIndices])) *
                    HTuple(
                        (hv_DLSample.TupleGetDictTuple("bbox_length2"))[hv_CharsIndices]);
            ...
            hv_CharAreaThreshold = hv_CharArea * 0.8;
            {
                ...
                HTuple step_val12 = 1;
                for (hv_WordIndex = 0; hv_WordIndex.Continue(end_val12, step_val12);
                     hv_WordIndex += step_val12) {
                    if (0 != (int(HTuple(hv_WordLengths[hv_WordIndex]) != 0))) {
                        AreaIntersectionRectangle2(
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_row"))[HTuple(hv_WordsIndices[hv_WordIndex])]),
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_col"))[HTuple(hv_WordsIndices[hv_WordIndex])]),
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_phi"))[HTuple(hv_WordsIndices[hv_WordIndex])]),
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_length1"))[HTuple(hv_WordsIndices[hv_WordIndex])]),
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_length2"))[HTuple(hv_WordsIndices[hv_WordIndex])]),
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_row"))[hv_CharsIndices]),
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_col"))[hv_CharsIndices]),
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_phi"))[hv_CharsIndices]),
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_length1"))[hv_CharsIndices]),
                                    HTuple((hv_DLSample.TupleGetDictTuple(
                                                "bbox_length2"))[hv_CharsIndices]),
                                    &hv_AreaIntersection);
                        hv_CIsInsideW =
                                hv_AreaIntersection.TupleGreaterElem(hv_CharAreaThreshold);
                        hv_CIndex = hv_CIsInsideW.TupleFind(1);
                        if (0 != (int(hv_CIndex != -1))) {
                            (*hvec_WordsCharsMapping)[HTuple(hv_WordsIndices[hv_WordIndex])] =
                                    HTupleVector(HTuple(hv_CharsIndices[hv_CIndex]));
                        }
                    } else {
                        throw HException(
                                    ((("Sample with image id " +
...
// Chapter: Deep Learning / Anomaly Detection and Global Context Anomaly
// Detection Short Description: Get the ground truth anomaly label and label ID.
void get_anomaly_ground_truth_label(HTuple hv_SampleKeys, HTuple hv_DLSample,
                                    HTuple *hv_AnomalyLabelGroundTruth,
    ...
    if (0 != (int((hv_SampleKeys.TupleFind("anomaly_label_id")) != -1))) {
        GetDictTuple(hv_DLSample, "anomaly_label_id",
                     &(*hv_AnomalyLabelIDGroundTruth));
    } else {
        throw HException(
                    "Ground truth class label id cannot be found in DLSample.");
    }
    //
    return;
}
...
// Chapter: Deep Learning / Anomaly Detection and Global Context Anomaly
// Detection Short Description: Get the anomaly results out of DLResult and
// apply thresholds (if specified).
void get_anomaly_result(HObject *ho_AnomalyImage, HObject *ho_AnomalyRegion,
                        HTuple hv_DLResult, HTuple hv_AnomalyClassThreshold,
                        HTuple hv_AnomalyRegionThreshold,
                        HTuple hv_AnomalyResultPostfix, HTuple *hv_AnomalyScore,
                        HTuple *hv_AnomalyClassID,
                        HTuple *hv_AnomalyClassThresholdDisplay,
    ...
    if (0 != (int(hv_ResultKeys == HTuple()))) {
        throw HException(("Result anomaly image" + hv_ErrorMsgPostfix) +
                         " could not be found in DLResult.");
    }
    //
    ...
    if (0 != (int((hv_ResultKeys.TupleFindFirst(hv_AnomalyScoreKey)) != -1))) {
        ...
        throw HException(("Result anomaly score" + hv_ErrorMsgPostfix) +
                         " could not be found in DLResult.");
    }
    //
    ...
    if (0 != (int(hv_AnomalyRegionThreshold != -1))) {
        ...
        if (0 != (int((hv_AnomalyRegionThreshold.TupleLength()) != 1))) {
            throw HException("Selected 'anomaly_region_threshold' must be specified "
                             "by exactly one value.");
        }
        Threshold((*ho_AnomalyImage), &(*ho_AnomalyRegion),
                  hv_AnomalyRegionThreshold, "max");
        (*hv_AnomalyRegionThresholdDisplay) = hv_AnomalyRegionThreshold;
    } else {
        // If no threshold is given, use the threshold and resulting anomaly region
        // out of DLResult.
        ...
        if (0 != (int((hv_ResultKeys.TupleFind("anomaly_segmentation_threshold")) !=
                      -1))) {
            GetDictTuple(hv_DLResult, "anomaly_segmentation_threshold",
    ...
    if (0 != (int(hv_AnomalyClassThreshold != -1))) {
        ...
        if (0 != (int((hv_AnomalyClassThreshold.TupleLength()) != 1))) {
            throw HException("Selected 'anomaly_classification_threshold' must be "
                             "specified by exactly one value.");
        }
        ...
        // If no threshold is given, use the threshold and resulting class id out of
        // DLResult.
        if (0 !=
                (int((hv_ResultKeys.TupleFindFirst(hv_AnomalyClassIdKey)) != -1))) {
            GetDictTuple(hv_DLResult, hv_AnomalyClassIdKey, &(*hv_AnomalyClassID));
        } else {
            (*hv_AnomalyClassID) = -1;
        }
        if (0 != (int((hv_ResultKeys.TupleFind(
                           "anomaly_classification_threshold")) != -1))) {
            GetDictTuple(hv_DLResult, "anomaly_classification_threshold",
                         &(*hv_AnomalyClassThresholdDisplay));
        }
    }
    //
    return;
...
// Chapter: Graphics / Window
// Short Description: Get the next child window that can be used for
// visualization.
void get_child_window(HTuple hv_HeightImage, HTuple hv_Font, HTuple hv_FontSize,
                      HTuple hv_Text, HTuple hv_PrevWindowCoordinates,
                      HTuple hv_WindowHandleDict, HTuple hv_WindowHandleKey,
                      HTuple *hv_WindowImageRatio,
    ...
    try {
        ...
        // catch (Exception)
            ...
            HDevExpDefaultException.ToHTuple(&hv_Exception);
            // Since there is something wrong with the current window, create a new
            // one.
            hv_OpenNewWindow = 1;
        }
    }
    ...
    if (0 != (hv_OpenNewWindow.TupleNot())) {
        ...
        if (HDevWindowStack::IsOpen())
            ClearWindow(HDevWindowStack::GetActive());
        set_display_font(hv_ChildWindowHandle, hv_FontSize, hv_Font, "true",
                         "false");
        //
        GetDictTuple(hv_WindowHandleDict, "meta_information", &hv_MetaInfo);
        //
        // Get previous window coordinates.
        GetWindowExtents(hv_ParentWindowHandle, &hv_WindowRow, &hv_WindowColumn,
        ...
        try {
            ...
            GetDictTuple(hv_MetaInfo,
                         hv_WindowHandleKey + "_window_image_ratio_height",
                         &(*hv_WindowImageRatio));
            //
            // Get previous window coordinates.
            GetDictTuple(hv_MetaInfo,
                         hv_WindowHandleKey + "_child_window_coordinates",
        ...
        open_child_window(hv_ParentWindowHandle, hv_Font, hv_FontSize, hv_Text,
                          hv_PrevWindowCoordinates, hv_WindowHandleDict,
                          hv_WindowHandleKey, &hv_ChildWindowHandle,
                          &(*hv_PrevWindowCoordinatesOut));
        SetWindowParam(hv_ChildWindowHandle, "flush", "false");
        SetDictTuple(hv_WindowHandleDict, hv_WindowHandleKey,
                     hv_ParentWindowHandle.TupleConcat(hv_ChildWindowHandle));
    }
    //
    return;
}
...
void get_classification_ground_truth(
    ...
    if (0 != (int((hv_SampleKeys.TupleFind("image_label_id")) != -1))) {
        ...
        GetDictTuple(hv_DLSample, "image_label_ids",
                     &(*hv_ClassificationLabelIDGroundTruth));
    } else {
        throw HException("Ground truth class label cannot be found in DLSample.");
    }
...
void get_classification_result(HTuple hv_ResultKeys, HTuple hv_DLResult,
    ...
    if (0 != (int((hv_ResultKeys.TupleFind("classification_class_ids")) != -1))) {
        GetDictTuple(hv_DLResult, "classification_class_ids",
                     &(*hv_ClassificationClassID));
        if (0 != (int(((*hv_ClassificationClassID).TupleLength()) > 0))) {
            (*hv_ClassificationClassID) =
                    ((const HTuple &)(*hv_ClassificationClassID))[0];
        }
    } else if (0 !=
               (int((hv_ResultKeys.TupleFind("selected_class_ids")) != -1))) {
        GetDictTuple(hv_DLResult, "selected_class_ids",
                     &(*hv_ClassificationClassID));
    } else {
        throw HException("Key entry 'classification_class_ids' or "
                         "'selected_class_ids' could not be found in DLResult.");
    }
    //
    return;
}
...
void get_confidence_image(HObject *ho_ImageConfidence, HTuple hv_ResultKeys,
                          HTuple hv_DLResult) {
    ...
    if (0 != (int((hv_ResultKeys.TupleFind("segmentation_confidence")) != -1))) {
        GetDictObject(&(*ho_ImageConfidence), hv_DLResult,
                      "segmentation_confidence");
    } else if (0 != (int((hv_ResultKeys.TupleFind("segmentation_confidences")) !=
                         -1))) {
        GetDictObject(&(*ho_ImageConfidence), hv_DLResult,
...
void get_distinct_colors(HTuple hv_NumColors, HTuple hv_Random,
                         HTuple hv_StartColor, HTuple hv_EndColor,
                         HTuple *hv_Colors) {
    ...
    if (0 != (int(hv_NumColors != 1))) {
        hv_Hue =
                (hv_StartColor +
                 ((((hv_EndColor - hv_StartColor) *
                    (HTuple::TupleGenSequence(0, hv_NumColors - 1, 1).TupleReal())) /
                   ((hv_NumColors - 1).TupleReal()))
                  .TupleInt())) %
                255;
    } else {
    ...
    if (0 != hv_Random) {
        hv_Hue = ((const HTuple &)
                  hv_Hue)[HTuple::TupleRand(hv_NumColors).TupleSortIndex()];
        hv_Lightness =
                (((5.0 + HTuple::TupleRand(hv_NumColors)) * 255.0) / 10.0).TupleInt();
        hv_Saturation =
                (((9.0 + HTuple::TupleRand(hv_NumColors)) * 255.0) / 10.0).TupleInt();
    } else {
    ...
    TransToRgb(ho_HLSImageH, ho_HLSImageL, ho_HLSImageS, &ho_ImageR, &ho_ImageG,
               &ho_ImageB, "hls");
    //
    ...
    (*hv_Colors) =
            (("#" + (hv_Red.TupleString("02x"))) + (hv_Green.TupleString("02x"))) +
            (hv_Blue.TupleString("02x"));
    return;
    //
}
...
void get_dl_class_colors(HTuple hv_ClassNames,
                         HTuple hv_AdditionalGreenClassNames,
                         HTuple *hv_Colors) {
    ...
    // Get distinct colors without randomness makes neighboring colors look very
    // similar. We use a workaround to get deterministic colors where subsequent
    ...
    // If a class 'OK','ok', 'good' or 'GOOD' or a class specified in
    // AdditionalGreenClassNames is present set this class to green. Only the
    // first occurrence found is set to a green shade.
    TupleUnion((((HTuple("good").Append("GOOD")).Append("ok")).Append("OK")),
               hv_AdditionalGreenClassNames, &hv_ClassNamesGood);
    {
        HTuple end_val13 = (hv_ClassNamesGood.TupleLength()) - 1;
        HTuple step_val13 = 1;
        for (hv_IndexFind = 0; hv_IndexFind.Continue(end_val13, step_val13);
             hv_IndexFind += step_val13) {
            hv_GoodIdx =
                    hv_ClassNames.TupleFindFirst(HTuple(hv_ClassNamesGood[hv_IndexFind]));
            if (0 != (HTuple(int(hv_GoodIdx != -1))
                      .TupleAnd(int((hv_ClassNames.TupleLength()) <= 8)))) {
                ...
                break;
            } else if (0 != (HTuple(int(hv_GoodIdx != -1))
                             .TupleAnd(int((hv_ClassNames.TupleLength()) > 8)))) {
                // If number of classes is larger than 8, set the respective color to
                // green.
                (*hv_Colors)[hv_GoodIdx] = "#00ff00";
                break;
            }
...
void get_dl_sample_image(HObject *ho_Image, HTuple hv_SampleKeys,
                         HTuple hv_DLSample, HTuple hv_Key) {
    ...
    if (0 != (int((hv_SampleKeys.TupleFind(hv_Key)) != -1))) {
        ...
        throw HException(("Image with key '" + hv_Key) +
                         "' could not be found in DLSample.");
    }
    return;
}
...
void get_gripping_points_from_dict(HTuple hv_DLResult, HTuple *hv_Rows,
                                   HTuple *hv_Columns) {
    ...
    TupleGenConst(hv_NumberOfResults, 0, &(*hv_Columns));
    {
        ...
        HTuple step_val12 = 1;
        for (hv_IndexGrippingPointsPoint = 0;
             hv_IndexGrippingPointsPoint.Continue(end_val12, step_val12);
             hv_IndexGrippingPointsPoint += step_val12) {
            hv_GrippingPointCoordDict =
                    HTuple(hv_GrippingPoints[hv_IndexGrippingPointsPoint]);
            (*hv_Rows)[hv_IndexGrippingPointsPoint] =
                    hv_GrippingPointCoordDict.TupleGetDictTuple("row");
            (*hv_Columns)[hv_IndexGrippingPointsPoint] =
                    hv_GrippingPointCoordDict.TupleGetDictTuple("column");
        }
    }
    return;
}
...
void get_next_window(HTuple hv_Font, HTuple hv_FontSize,
                     HTuple hv_ShowBottomDesc, HTuple hv_WidthImage,
                     HTuple hv_HeightImage, HTuple hv_MapColorBarWidth,
                     HTuple hv_ScaleWindows, HTuple hv_ThresholdWidth,
                     HTuple hv_PrevWindowCoordinates,
                     HTuple hv_WindowHandleDict, HTuple hv_WindowHandleKey,
                     HTuple *hv_CurrentWindowHandle,
                     HTuple *hv_WindowImageRatioHeight,
                     HTuple *hv_PrevWindowCoordinatesOut) {
    ...
    if (0 != (hv_OpenNewWindow.TupleNot())) {
        ...
        if (HDevWindowStack::IsOpen())
            ClearWindow(HDevWindowStack::GetActive());
        set_display_font((*hv_CurrentWindowHandle), hv_FontSize, hv_Font, "true",
                         "false");
        //
        // Calculate MarginBottom.
        if (0 != hv_ShowBottomDesc) {
            GetStringExtents((*hv_CurrentWindowHandle), "test_string", &hv_Ascent,
                             &hv_Descent, &hv__, &hv__);
        ...
        update_window_meta_information(
                    (*hv_CurrentWindowHandle), hv_WidthImage, hv_HeightImage, 0, 0,
                    hv_MapColorBarWidth, hv_MarginBottom, &(*hv_WindowImageRatioHeight),
                    &hv_WindowImageRatioWidth, &hv_SetPartRow2, &hv_SetPartColumn2,
                    &(*hv_PrevWindowCoordinatesOut));
        //
        // Update meta information.
        GetDictTuple(hv_WindowHandleDict, "meta_information", &hv_MetaInfo);
        SetDictTuple(hv_MetaInfo, hv_WindowHandleKey + "_window_image_ratio_height",
                     (*hv_WindowImageRatioHeight));
        SetDictTuple(hv_MetaInfo, hv_WindowHandleKey + "_window_image_ratio_width",
                     hv_WindowImageRatioWidth);
        SetDictTuple(hv_MetaInfo, hv_WindowHandleKey + "_set_part_row2",
                     hv_SetPartRow2);
        SetDictTuple(hv_MetaInfo, hv_WindowHandleKey + "_set_part_column2",
                     hv_SetPartColumn2);
        SetDictTuple(hv_MetaInfo, hv_WindowHandleKey + "_margin_bottom",
                     hv_MarginBottom);
        SetDictTuple(hv_MetaInfo, hv_WindowHandleKey + "_map_color_bar_with",
                     hv_MapColorBarWidth);
        SetDictTuple(hv_MetaInfo, hv_WindowHandleKey + "_window_coordinates",
                     (*hv_PrevWindowCoordinatesOut));
    } else {
        //
        // Open a new window.
        open_next_window(hv_Font, hv_FontSize, hv_ShowBottomDesc, hv_WidthImage,
                         hv_HeightImage, hv_MapColorBarWidth, hv_ScaleWindows,
                         hv_ThresholdWidth, hv_PrevWindowCoordinates,
                         hv_WindowHandleDict, hv_WindowHandleKey,
                         &(*hv_CurrentWindowHandle), &(*hv_WindowImageRatioHeight),
                         &(*hv_PrevWindowCoordinatesOut));
...
void get_segmentation_image_ground_truth(
        HObject *ho_SegmentationImagGroundTruth, HTuple hv_SampleKeys,
        HTuple hv_DLSample) {
    ...
    //
    if (0 != (int((hv_SampleKeys.TupleFind("segmentation_image")) != -1))) {
        GetDictObject(&(*ho_SegmentationImagGroundTruth), hv_DLSample,
                      "segmentation_image");
    } else {
        throw HException(
                    "Ground truth segmentation image could not be found in DLSample.");
    }
    return;
}
...
void get_segmentation_image_result(HObject *ho_SegmentationImageResult,
    ...
    if (0 != (int((hv_ResultKeys.TupleFind("segmentation_image")) != -1))) {
        GetDictObject(&(*ho_SegmentationImageResult), hv_DLResult,
                      "segmentation_image");
    } else {
        throw HException(
                    "Result segmentation data could not be found in DLSample.");
    }
    return;
}
...
void get_weight_image(HObject *ho_ImageWeight, HTuple hv_SampleKeys,
                      HTuple hv_DLSample) {
...
void list_image_files(HTuple hv_ImageDirectory, HTuple hv_Extensions,
    ...
    //
    if (0 != (HTuple(HTuple(int(hv_Extensions == HTuple()))
                     .TupleOr(int(hv_Extensions == HTuple(""))))
              .TupleOr(int(hv_Extensions == HTuple("default"))))) {
        hv_Extensions.Clear();
        hv_Extensions[0] = "ima";
        hv_Extensions[1] = "tif";
    ...
    // Loop through all given image directories.
    {
        ...
        HTuple step_val27 = 1;
        for (hv_ImageDirectoryIndex = 0;
             hv_ImageDirectoryIndex.Continue(end_val27, step_val27);
             hv_ImageDirectoryIndex += step_val27) {
            hv_ImageFilesTmp = HTuple();
            hv_CurrentImageDirectory =
                    HTuple(hv_ImageDirectory[hv_ImageDirectoryIndex]);
            ...
            hv_Directories = hv_CurrentImageDirectory;
            {
                ...
                HTuple step_val41 = 1;
                for (hv_Index = 0; hv_Index.Continue(end_val41, step_val41);
                     hv_Index += step_val41) {
                    hv_Directories = hv_Directories.TupleConcat(
                                (HTuple(hv_HalconImages[hv_Index]) + "/") +
                                hv_CurrentImageDirectory);
                }
            }
            ...
            if (0 != (int((hv_OS.TupleSubstr(0, 2)) == HTuple("Win")))) {
                {
                    ...
                    HTuple step_val47 = 1;
                    for (hv_Index = 0; hv_Index.Continue(end_val47, step_val47);
                         hv_Index += step_val47) {
                        if (0 !=
                                (int((HTuple(hv_Directories[hv_Index]).TupleStrlen()) > 1))) {
                            TupleStrFirstN(HTuple(hv_Directories[hv_Index]), 1,
                                           &hv_Substring);
                            if (0 != (HTuple(int(hv_Substring == HTuple("//")))
                                      .TupleOr(int(hv_Substring == HTuple("\\\\"))))) {
            ...
            hv_ImageFilesTmp = HTuple();
            {
                ...
                HTuple step_val57 = 1;
                for (hv_Index = 0; hv_Index.Continue(end_val57, step_val57);
                     hv_Index += step_val57) {
                    FileExists(HTuple(hv_Directories[hv_Index]), &hv_FileExists);
                    if (0 != hv_FileExists) {
                        ListFiles(HTuple(hv_Directories[hv_Index]),
                                  HTuple("files").TupleConcat(hv_Options), &hv_AllFiles);
                        hv_ImageFilesTmp = HTuple();
                        {
                            HTuple end_val62 = (hv_Extensions.TupleLength()) - 1;
                            HTuple step_val62 = 1;
                            for (hv_i = 0; hv_i.Continue(end_val62, step_val62);
                                 hv_i += step_val62) {
                                TupleRegexpSelect(hv_AllFiles,
                                                  ((".*" + HTuple(hv_Extensions[hv_i])) + "$")
                                                  .TupleConcat("ignore_case"),
                                                  &hv_Selection);
                                hv_ImageFilesTmp = hv_ImageFilesTmp.TupleConcat(hv_Selection);
                            }
                        }
                        TupleRegexpReplace(hv_ImageFilesTmp,
                                           (HTuple("\\\\").Append("replace_all")), "/",
                                           &hv_ImageFilesTmp);
...
void make_neighboring_colors_distinguishable(HTuple hv_ColorsRainbow,
                                             HTuple *hv_Colors) {
    ...
    if (0 != (int(hv_NumColors >= 8))) {
        ...
        hv_StartIdx = 0;
        {
            ...
            for (hv_S = 0; hv_S.Continue(end_val19, step_val19); hv_S += step_val19) {
                ...
                hv_IdxsLeft =
                        HTuple::TupleGenSequence(hv_S, hv_NumColors - 1, hv_NumChunks);
                hv_IdxsRight = HTuple::TupleGenSequence(hv_StartIdx, hv_EndIdx, 1);
                (*hv_Colors)[HTuple::TupleGenSequence(hv_S, hv_NumColors - 1,
                                                      hv_NumChunks)] =
                        hv_ColorsRainbow.TupleSelectRange(hv_StartIdx, hv_EndIdx);
...
void open_child_window(HTuple hv_WindowHandleFather, HTuple hv_Font,
                       HTuple hv_FontSize, HTuple hv_Text,
                       HTuple hv_PrevWindowCoordinates,
                       HTuple hv_WindowHandleDict, HTuple hv_WindowHandleKey,
                       HTuple *hv_WindowHandleChild,
                       HTuple *hv_PrevWindowCoordinatesOut) {
    ...
    hv_StringWidth = 150;
    {
        ...
        HTuple step_val6 = 1;
        for (hv_IndexText = 0; hv_IndexText.Continue(end_val6, step_val6);
             hv_IndexText += step_val6) {
            GetStringExtents(hv_WindowHandleFather, HTuple(hv_Text[hv_IndexText]),
                             &hv__, &hv__, &hv_TextWidth, &hv__);
    ...
    hv_WindowColumn = (HTuple(hv_PrevWindowCoordinates[1]) +
                       HTuple(hv_PrevWindowCoordinates[2])) +
            5;
    hv_WindowWidth = hv_StringWidth + (2 * 12.0);
    hv_WindowHeight = ((const HTuple &)hv_PrevWindowCoordinates)[3];
    //
    SetWindowAttr("background_color", "black");
    OpenWindow(hv_WindowRow, hv_WindowColumn, hv_WindowWidth, hv_WindowHeight, 0,
               "visible", "", &(*hv_WindowHandleChild));
    HDevWindowStack::Push((*hv_WindowHandleChild));
    set_display_font((*hv_WindowHandleChild), hv_FontSize, hv_Font, "true",
                     "false");
    //
    ...
    GetDictTuple(hv_WindowHandleDict, "meta_information", &hv_MetaInfo);
    SetDictTuple(hv_MetaInfo, hv_WindowHandleKey + "_child_window_coordinates",
                 (*hv_PrevWindowCoordinatesOut));
...
// Chapter: Graphics / Window
// Short Description: Open a new window, either next to the last ones, or in a
// new row.
void open_next_window(HTuple hv_Font, HTuple hv_FontSize,
                      HTuple hv_ShowBottomDesc, HTuple hv_WidthImage,
                      HTuple hv_HeightImage, HTuple hv_MapColorBarWidth,
                      HTuple hv_ScaleWindows, HTuple hv_ThresholdWidth,
                      HTuple hv_PrevWindowCoordinates,
                      HTuple hv_WindowHandleDict, HTuple hv_WindowHandleKey,
                      HTuple *hv_WindowHandleNew,
                      HTuple *hv_WindowImageRatioHeight,
                      HTuple *hv_PrevWindowCoordinatesOut) {
    ...
    //
    if (0 !=
            (int((hv_PrevWindowColumn + hv_PrevWindowWidth) > hv_ThresholdWidth))) {
    ...
    dev_open_window_fit_size(
                hv_WindowRow, hv_WindowColumn, hv_WidthImage, hv_HeightImage,
                (HTuple(500).Append(800)) * hv_ScaleWindows,
                (HTuple(400).Append(600)) * hv_ScaleWindows, &(*hv_WindowHandleNew));
    set_display_font((*hv_WindowHandleNew), hv_FontSize, hv_Font, "true",
                     "false");
    //
    // Add MarginBottom and MapColorBarWidth to window.
    if (0 != hv_ShowBottomDesc) {
        GetStringExtents((*hv_WindowHandleNew), "Test_string", &hv_Ascent,
                         &hv_Descent, &hv__, &hv__);
        hv_NumLines = hv_ShowBottomDesc;
        hv_MarginBottom = (hv_NumLines * (hv_Ascent + hv_Descent)) + (2 * 12);
    } else {
    ...
    GetWindowExtents((*hv_WindowHandleNew), &hv__, &hv__, &hv_WindowWidth,
                     &hv_WindowHeight);
    if (HDevWindowStack::IsOpen())
        SetWindowExtents(HDevWindowStack::GetActive(), hv_WindowRow,
                         hv_WindowColumn, hv_WindowWidth + hv_MapColorBarWidth,
                         hv_WindowHeight + hv_MarginBottom);
    //
    // Get and set meta information of new window handle.
    update_window_meta_information(
                (*hv_WindowHandleNew), hv_WidthImage, hv_HeightImage, hv_WindowRow,
                hv_WindowColumn, hv_MapColorBarWidth, hv_MarginBottom,
...
// Chapter: Deep Learning / Model
// Short Description: Preprocess 3D data for deep-learning-based training and
// inference.
void preprocess_dl_model_3d_data(HTuple hv_DLSample,
                                 HTuple hv_DLPreprocessParam) {
    ...
    if (0 != (hv_HasNormals.TupleNot())) {
        ...
        if (0 != (hv_HasFullXYZ.TupleNot())) {
            throw HException(HTuple(
                                 "The given input DLSample does not contain necessary images 'x','y' "
                                 "and 'z'. This is required if no normals are provided."));
        }
    } else {
        // At least Z is required if normals are given.
        if (0 != (hv_HasZ.TupleNot())) {
            throw HException(HTuple(
                                 "The given input DLSample does not contain at least the depth image "
                                 "'z'. This is required because normals are provided. Optionally, 'x' "
                                 "and 'y' images might be provided additionally."));
        }
        CountChannels(hv_DLSample.TupleGetDictObject("normals"), &hv_NumChannels);
        if (0 != (int(hv_NumChannels != 3))) {
            throw HException(
                        "The given input DLSample.normals has to have three channels.");
        }
    ...
    }
    {
        ...
        HTuple step_val31 = 1;
        for (hv_Index = 0; hv_Index.Continue(end_val31, step_val31);
             hv_Index += step_val31) {
            if (0 != (HTuple(hv_HasXYZ[hv_Index]))) {
                ...
                if (0 != (int(hv_NumChannels != 1))) {
                    throw HException(("The given input DLSample." + hv_Key) +
    ...
    GetDictParam(hv_DLPreprocessParam, "key_exists",
                 (HTuple("min_z").Append("max_z")), &hv_ZMinMaxExist);
    if (0 != (HTuple(hv_ZMinMaxExist[0]))) {
        Threshold(ho_ImageZ, &ho_Region, "min",
                  hv_DLPreprocessParam.TupleGetDictTuple("min_z"));
        Difference(ho_Domain, ho_Region, &ho_Domain);
    }
    if (0 != (HTuple(hv_ZMinMaxExist[1]))) {
        Threshold(ho_ImageZ, &ho_Region,
                  hv_DLPreprocessParam.TupleGetDictTuple("max_z"), "max");
        Difference(ho_Domain, ho_Region, &ho_Domain);
    }
    ...
    //
    // Before we zoom any 3D images we want to set all pixels outside of the
    // domain to an invalid value.
    ...
    if (0 != hv_HasFullXYZ) {
        ...
        OverpaintRegion(ho_ImageX, ho_DomainComplement, hv_GrayvalOutsideInit,
                        "fill");
        OverpaintRegion(ho_ImageY, ho_DomainComplement, hv_GrayvalOutsideInit,
                        "fill");
        OverpaintRegion(ho_ImageZ, ho_DomainComplement, hv_GrayvalOutsideInit,
                        "fill");
        ...
        if (0 != (hv_HasNormals.TupleNot())) {
            ...
            GetDictParam(hv_DLPreprocessParam, "key_exists",
                         (HTuple("normal_image_width").Append("normal_image_height")),
                         &hv_NormalSizeExists);
            if (0 != (HTuple(hv_NormalSizeExists[0]).TupleNot())) {
                hv_NormalWidth =
                        ((hv_DLPreprocessParam.TupleGetDictTuple("image_width")) * 1.5)
                        .TupleInt();
            } else {
                hv_NormalWidth =
                        hv_DLPreprocessParam.TupleGetDictTuple("normal_image_width");
            }
            if (0 != (HTuple(hv_NormalSizeExists[1]).TupleNot())) {
                hv_NormalHeight =
                        ((hv_DLPreprocessParam.TupleGetDictTuple("image_height")) * 1.5)
                        .TupleInt();
            } else {
                hv_NormalHeight =
                        hv_DLPreprocessParam.TupleGetDictTuple("normal_image_height");
            }

            GetImageSize(ho_ImageZ, &hv_WidthZ, &hv_HeightZ);
            hv_ZoomNormals = HTuple(int(hv_NormalWidth != hv_WidthZ))
                    .TupleOr(int(hv_NormalHeight != hv_HeightZ));

            if (0 != hv_ZoomNormals) {
                Compose3(ho_ImageX, ho_ImageY, ho_ImageZ, &ho_ImageXYZ);
                GetImageSize(ho_ImageXYZ, &hv_Width, &hv_Height);
                ZoomImageSize(ho_ImageXYZ, &ho_ImageXYZ, hv_NormalWidth,
                ...
                remove_invalid_3d_pixels(ho_ImageX, ho_ImageY, ho_ImageZ, ho_Domain,
                                         &ho_Domain, hv_GrayvalOutsideInit);
                Complement(ho_Domain, &ho_DomainComplement);
            }

            compute_normals_xyz(ho_ImageX, ho_ImageY, ho_ImageZ, &ho_NXImage,
                                &ho_NYImage, &ho_NZImage, 1);
        } else {
    ...
    OverpaintRegion(ho_NXImage, ho_DomainComplement, hv_GrayvalOutsideInit,
                    "fill");
    OverpaintRegion(ho_NYImage, ho_DomainComplement, hv_GrayvalOutsideInit,
                    "fill");
    OverpaintRegion(ho_NZImage, ho_DomainComplement, hv_GrayvalOutsideInit,
                    "fill");
    OverpaintRegion(ho_ImageZ, ho_DomainComplement, hv_GrayvalOutsideInit,
                    "fill");

    Compose4(ho_NXImage, ho_NYImage, ho_NZImage, ho_ImageZ,
             &ho_MultiChannelImage);
    ...
    ZoomImageSize(ho_MultiChannelImage, &ho_MultiChannelImage,
                  hv_DLPreprocessParam.TupleGetDictTuple("image_width"),
                  hv_DLPreprocessParam.TupleGetDictTuple("image_height"),
                  "nearest_neighbor");

    Decompose4(ho_MultiChannelImage, &ho_NXImage, &ho_NYImage, &ho_NZImage,
               &ho_ImageZ);
    if (0 != hv_HasX) {
    ...
    // Zoom the domain
    hv_ScaleWidth = (hv_DLPreprocessParam.TupleGetDictTuple("image_width")) /
            (hv_Width.TupleReal());
    hv_ScaleHeight = (hv_DLPreprocessParam.TupleGetDictTuple("image_height")) /
            (hv_Height.TupleReal());
    ZoomRegion(ho_Domain, &ho_Domain, hv_ScaleWidth, hv_ScaleHeight);
    remove_invalid_3d_pixels(ho_NXImage, ho_NYImage, ho_NZImage, ho_Domain,
                             &ho_Domain, hv_GrayvalOutsideInit);
    ...
    ReduceDomain(hv_DLSample.TupleGetDictObject("normals"), ho_Domain,
                 &ho___Tmp_Obj_0);
    SetDictObject(ho___Tmp_Obj_0, hv_DLSample, "normals");
...
// Chapter: Deep Learning / Model
// Short Description: Preprocess anomaly images for evaluation and visualization
// of deep-learning-based anomaly detection or Global Context Anomaly Detection.
void preprocess_dl_model_anomaly(HObject ho_AnomalyImages,
                                 HObject *ho_AnomalyImagesPreprocessed,
                                 HTuple hv_DLPreprocessParam) {
    ...
    } else if (0 != (HTuple(int(hv_DomainHandling == HTuple("keep_domain")))
                     .TupleAnd(
                         int(hv_ModelType == HTuple("anomaly_detection"))))) {
        // The option 'keep_domain' is only supported for models of 'type' =
        // 'anomaly_detection'
    } else {
        throw HException("Unsupported parameter value for 'domain_handling'");
    }
    //
    MinMaxGray(ho_AnomalyImages, ho_AnomalyImages, 0, &hv_Min, &hv_Max,
               &hv_Range);
    if (0 != (int(hv_Min < 0.0))) {
    ...
    if (0 != (HTuple(int((hv_EqualWidth.TupleMin()) == 0))
              .TupleOr(int((hv_EqualHeight.TupleMin()) == 0)))) {
        ZoomImageSize(ho_AnomalyImages, &ho_AnomalyImages, hv_ImageWidth,
    ...
    if (0 != (int(hv_IndicesWrongChannels != -1))) {
        throw HException(
                    "Number of channels in anomaly image is not supported. Please check "
                    "for anomaly images with a number of channels different from 1.");
    }
    //
...
// Chapter: Deep Learning / Model
// Short Description: Preprocess the provided DLSample image for augmentation
// purposes.
void preprocess_dl_model_augmentation_data(HTuple hv_DLSample,
                                           HTuple hv_DLPreprocessParam) {
    ...
    // This procedure preprocesses the provided DLSample image for augmentation
    // purposes.
    //
    // Check the validity of the preprocessing parameters.
    check_dl_preprocess_param(hv_DLPreprocessParam);
    ...
    hv_ImageNumChannels =
            hv_DLPreprocessParam.TupleGetDictTuple("image_num_channels");
    hv_ModelType = hv_DLPreprocessParam.TupleGetDictTuple("model_type");
    //
    // Determine whether the preprocessing is required or not.
    GetDictParam(hv_DLPreprocessParam, "key_exists", "augmentation",
                 &hv_AugmentationKeyExists);
    ...
    if (0 != (((hv_DLPreprocessParam.TupleConcat(hv___Tmp_Ctrl_Dict_Init_0))
               .TupleTestEqualDictItem("augmentation", "comp"))
              .TupleNot())) {
        return;
    }
    hv___Tmp_Ctrl_Dict_Init_0 = HTuple::TupleConstant("HNULL");
    if (0 != (HTuple(int(hv_ModelType != HTuple("ocr_detection")))
    ...
    if (0 != (int(hv_ModelType == HTuple("ocr_recognition")))) {
        if (0 != (int(hv_ImageNumChannels != 1))) {
            throw HException("The only 'image_num_channels' value supported for "
                             "ocr_recognition models is 1.");
        }
        if (0 != (int((hv_ImageType.TupleRegexpTest("byte|real")) != 1))) {
            throw HException("Please provide only images of type 'byte' or 'real' "
                             "for ocr_recognition models.");
        }
        if (0 != (int((HTuple((hv_NumChannels.TupleEqualElem(1))
                              .TupleOr(hv_NumChannels.TupleEqualElem(3)))
                       .TupleSum()) != 1))) {
            throw HException("Please provide only 1- or 3-channels images for "
                             "ocr_recognition models.");
        }
        //
        ...
        hv_InputImageWidthHeightRatio =
                hv_InputImageWidth / (hv_InputImageHeight.TupleReal());
        ...
        hv_HasPadding =
                int(((hv_ImageHeight * hv_InputImageWidthHeightRatio).TupleInt()) <
                    hv_ImageWidth);
        if (0 !=
                (HTuple(int(hv_ZoomHeight > hv_ImageHeight)).TupleOr(hv_HasPadding))) {
            ZoomImageSize(ho_ImageHighRes, &ho_ImageHighRes, hv_ZoomWidth,
            ...
            SetDictObject(ho_ImageHighRes,
                          hv_DLSample.TupleGetDictTuple("augmentation_data"),
                          "image_high_res");
            SetDictTuple(hv_DLSample.TupleGetDictTuple("augmentation_data"),
                         "preprocess_params", hv_DLPreprocessParam);
        }
    } else if (0 != (int(hv_ModelType == HTuple("ocr_detection")))) {
        if (0 != (int(hv_ImageNumChannels != 3))) {
            throw HException("The only 'image_num_channels' value supported for "
                             "ocr_detection models is 3.");
        }
        if (0 != (int((hv_ImageType.TupleRegexpTest("byte|real")) != 1))) {
            throw HException("Please provide only images of type 'byte' or 'real' "
                             "for ocr_detection models.");
        }
        if (0 != (int((HTuple((hv_NumChannels.TupleEqualElem(1))
                              .TupleOr(hv_NumChannels.TupleEqualElem(3)))
                       .TupleSum()) != 1))) {
            throw HException("Please provide only 1- or 3-channels images for "
                             "ocr_detection models.");
        }
        //
        ...
        calculate_dl_image_zoom_factors(hv_InputImageWidth, hv_InputImageHeight,
                                        2 * hv_ImageWidth, 2 * hv_ImageHeight,
                                        hv_DLPreprocessParam, &hv_ZoomFactorWidth,
                                        &hv_ZoomFactorHeight);
        ...
        //
        // Use the better size for high resolution: 2x resolution size of preprocess
        // image or input image size.
        hv_UseZoomImage = HTuple(int(hv_ZoomWidth < hv_InputImageWidth))
                .TupleOr(int(hv_ZoomHeight < hv_InputImageHeight));
        CopyDict(hv_DLSample, HTuple(), HTuple(), &hv_DLSampleHighRes);
        CopyDict(hv_DLPreprocessParam, HTuple(), HTuple(),
                 &hv_DLPreprocessParamHighRes);
        //
        FullDomain(ho_InputImage, &ho_ImageHighRes);
        if (0 != hv_UseZoomImage) {
            ...
            preprocess_dl_model_bbox_rect2(ho_ImageHighRes, hv_DLSampleHighRes,
                                           hv_DLPreprocessParamHighRes);
            gen_dl_ocr_detection_targets(hv_DLSampleHighRes,
                                         hv_DLPreprocessParamHighRes);
            ZoomImageSize(ho_ImageHighRes, &ho_ImageHighRes, hv_ZoomWidth,
                          hv_ZoomHeight, "constant");
        } else {
            SetDictTuple(hv_DLPreprocessParamHighRes, "image_width",
                         hv_InputImageWidth);
            SetDictTuple(hv_DLPreprocessParamHighRes, "image_height",
                         hv_InputImageHeight);
            gen_dl_ocr_detection_targets(hv_DLSampleHighRes,
                                         hv_DLPreprocessParamHighRes);
        }
...
// Chapter: Deep Learning / Object Detection and Instance Segmentation
// Short Description: Preprocess the bounding boxes of type 'rectangle1' for a
// given sample.
void preprocess_dl_model_bbox_rect1(HObject ho_ImageRaw, HTuple hv_DLSample,
                                    HTuple hv_DLPreprocessParam) {
    ...
    //
    // This procedure preprocesses the bounding boxes of type 'rectangle1' for a
    // given sample.
    //
    // Check the validity of the preprocessing parameters.
    check_dl_preprocess_param(hv_DLPreprocessParam);
    ...
    // catch (Exception)
        ...
        throw HException(
                    (("An error has occurred during preprocessing image_id " + hv_ImageId) +
                     " when getting bounding box coordinates : ") +
                    hv_ExceptionMessage);
    }
    //
    ...
    if (0 != (int((hv_BBoxRow1.TupleLength()) > 0))) {
        hv_BoxesInvalid =
                (hv_BBoxRow1.TupleGreaterEqualElem(hv_BBoxRow2))
                .TupleOr(hv_BBoxCol1.TupleGreaterEqualElem(hv_BBoxCol2));
        if (0 != (int((hv_BoxesInvalid.TupleSum()) > 0))) {
            GetDictTuple(hv_DLSample, "image_id", &hv_ImageId);
            throw HException(
                        ("An error has occurred during preprocessing image_id " +
                         hv_ImageId) +
                        HTuple(": Sample contains at least one box with zero-area, i.e. "
                               "bbox_col1 >= bbox_col2 or bbox_row1 >= bbox_row2."));
        }
    } else {
    ...
    if (0 != (int(hv_DomainHandling == HTuple("crop_domain")))) {
        ...
        SmallestRectangle1(ho_DomainRaw, &hv_DomainRow1, &hv_DomainColumn1,
        ...
        hv_MaskDelete = (hv_Row1.TupleGreaterEqualElem(hv_Row2))
                .TupleOr(hv_Col1.TupleGreaterEqualElem(hv_Col2));
        hv_MaskNewBbox = 1 - hv_MaskDelete;
        // Store the preprocessed bounding box entries.
        hv_BBoxCol1New =
                (hv_Col1.TupleSelectMask(hv_MaskNewBbox)) - hv_DomainColumn1;
        hv_BBoxCol2New =
                (hv_Col2.TupleSelectMask(hv_MaskNewBbox)) - hv_DomainColumn1;
        ...
        //
        // If we remove/select bounding boxes we also need to filter the
        // corresponding instance segmentation masks if they exist.
    ...
    //
    // Only rescale bounding boxes if the required image dimensions are not the
    // raw dimensions.
    if (0 != (HTuple(int(hv_ImageHeight != hv_HeightRaw))
              .TupleOr(int(hv_ImageWidth != hv_WidthRaw)))) {
        ...
        // As we use XLD-coordinates we temporarily move the boxes by (.5,.5) for
        // rescaling. Doing so, the center of the XLD-coordinate system (-0.5,-0.5)
        // is used for scaling, hence the scaling is performed w.r.t. the pixel
        // coordinate system.
    ...
    if (0 != (int((hv_BBoxRow1New.TupleLength()) > 0))) {
        hv_MaskDelete =
                (hv_BBoxRow1New.TupleGreaterEqualElem(hv_BBoxRow2New))
                .TupleOr(hv_BBoxCol1New.TupleGreaterEqualElem(hv_BBoxCol2New));
        ...
        // If we remove/select bounding boxes we also need to filter the
        // corresponding instance segmentation masks if they exist.
        filter_dl_sample_instance_segmentation_masks(hv_DLSample,
                                                     1 - hv_MaskDelete);
    }
    //
...
// Chapter: Deep Learning / Object Detection and Instance Segmentation
// Short Description: Preprocess the bounding boxes of type 'rectangle2' for a
// given sample.
void preprocess_dl_model_bbox_rect2(HObject ho_ImageRaw, HTuple hv_DLSample,
                                    HTuple hv_DLPreprocessParam) {
    ...
    HTuple hv_ClassIDsNoOrientationIndices, hv_Index,
    ...
    // This procedure preprocesses the bounding boxes of type 'rectangle2' for a
    // given sample.
    //
    check_dl_preprocess_param(hv_DLPreprocessParam);
    //
    ...
    GetDictParam(hv_DLPreprocessParam, "key_exists",
                 (HTuple("ignore_direction").Append("class_ids_no_orientation")),
                 &hv_KeyExists);
    ...
    if (0 != (HTuple(hv_KeyExists[1]))) {
        GetDictTuple(hv_DLPreprocessParam, "class_ids_no_orientation",
                     &hv_ClassIDsNoOrientation);
    }
    //
    // Get bounding box coordinates and labels.
    ...
    if (0 != (int((hv_BBoxRow.TupleLength()) > 0))) {
        hv_BoxesInvalid = ((hv_BBoxLength1.TupleEqualElem(0)).TupleSum()) +
                ((hv_BBoxLength2.TupleEqualElem(0)).TupleSum());
        if (0 != (int(hv_BoxesInvalid > 0))) {
            GetDictTuple(hv_DLSample, "image_id", &hv_ImageId);
            throw HException(
                        ("An error has occurred during preprocessing image_id " +
                         hv_ImageId) +
                        HTuple(": Sample contains at least one bounding box with zero-area, "
                               "i.e. bbox_length1 == 0 or bbox_length2 == 0!"));
        }
    } else {
        // There are no bounding boxes, hence nothing to do.
        return;
    ...
    if (0 != (int(hv_DomainHandling == HTuple("crop_domain")))) {
        ...
        hv_MaskDelete =
                HTuple(HTuple((hv_BBoxRow.TupleLessElem(hv_DomainRow1))
                              .TupleOr(hv_BBoxCol.TupleLessElem(hv_DomainColumn1)))
                       .TupleOr(hv_BBoxRow.TupleGreaterElem(hv_DomainRow2)))
                .TupleOr(hv_BBoxCol.TupleGreaterElem(hv_DomainColumn2));
        hv_MaskNewBbox = 1 - hv_MaskDelete;
        // Store the preprocessed bounding box entries.
        hv_BBoxRowNew =
                (hv_BBoxRow.TupleSelectMask(hv_MaskNewBbox)) - hv_DomainRow1;
        hv_BBoxColNew =
                (hv_BBoxCol.TupleSelectMask(hv_MaskNewBbox)) - hv_DomainColumn1;
    ...
    //
    // Generate smallest enclosing axis-aligned bounding box for classes in
    // ClassIDsNoOrientation.
    hv_ClassIDsNoOrientationIndices = HTuple();
    {
        HTuple end_val98 = (hv_ClassIDsNoOrientation.TupleLength()) - 1;
        HTuple step_val98 = 1;
        for (hv_Index = 0; hv_Index.Continue(end_val98, step_val98);
             hv_Index += step_val98) {
            hv_ClassIDsNoOrientationIndicesTmp =
                    (hv_BBoxLabelNew.TupleEqualElem(
                         HTuple(hv_ClassIDsNoOrientation[hv_Index])))
                    .TupleFind(1);
            if (0 != (int(hv_ClassIDsNoOrientationIndicesTmp != -1))) {
                hv_ClassIDsNoOrientationIndices =
                        hv_ClassIDsNoOrientationIndices.TupleConcat(
                            hv_ClassIDsNoOrientationIndicesTmp);
            }
        }
    }
    if (0 != (int((hv_ClassIDsNoOrientationIndices.TupleLength()) > 0))) {
        ...
        hv_DirectionLength1Row =
                -(HTuple(hv_BBoxPhiNew[hv_ClassIDsNoOrientationIndices]).TupleSin());
        hv_DirectionLength1Col =
                HTuple(hv_BBoxPhiNew[hv_ClassIDsNoOrientationIndices]).TupleCos();
        hv_DirectionLength2Row = -hv_DirectionLength1Col;
        hv_DirectionLength2Col = hv_DirectionLength1Row;
        hv_Corner1Row =
                (HTuple(hv_BBoxLength1New[hv_ClassIDsNoOrientationIndices]) *
                 hv_DirectionLength1Row) +
                (HTuple(hv_BBoxLength2New[hv_ClassIDsNoOrientationIndices]) *
                 hv_DirectionLength2Row);
        hv_Corner1Col =
                (HTuple(hv_BBoxLength1New[hv_ClassIDsNoOrientationIndices]) *
                 hv_DirectionLength1Col) +
                (HTuple(hv_BBoxLength2New[hv_ClassIDsNoOrientationIndices]) *
                 hv_DirectionLength2Col);
        hv_Corner2Row =
                (HTuple(hv_BBoxLength1New[hv_ClassIDsNoOrientationIndices]) *
                 hv_DirectionLength1Row) -
                (HTuple(hv_BBoxLength2New[hv_ClassIDsNoOrientationIndices]) *
        ...
        hv_BBoxLength1New[hv_ClassIDsNoOrientationIndices] =
                (hv_Corner1Col.TupleAbs()).TupleMax2(hv_Corner2Col.TupleAbs());
        hv_BBoxLength2New[hv_ClassIDsNoOrientationIndices] =
                (hv_Corner1Row.TupleAbs()).TupleMax2(hv_Corner2Row.TupleAbs());
    }
    //
    ...
    if (0 != (HTuple(int(hv_ImageHeight != hv_HeightRaw))
        ...
        calculate_dl_image_zoom_factors(hv_WidthRaw, hv_HeightRaw, hv_ImageWidth,
                                        hv_ImageHeight, hv_DLPreprocessParam,
                                        &hv_FactorResampleWidth,
                                        &hv_FactorResampleHeight);
        //
        if (0 != (HTuple(int(hv_FactorResampleHeight != hv_FactorResampleWidth))
                  .TupleAnd(int((hv_BBoxRowNew.TupleLength()) > 0)))) {
            // In order to preserve the correct orientation we have to transform the
            // points individually. Get the coordinates of the four corner points.
            convert_rect2_5to8param(
                        hv_BBoxRowNew, hv_BBoxColNew, hv_BBoxLength1New, hv_BBoxLength2New,
            ...
            //
            // The rectangles will get sheared, that is why new rectangles have to be
            // found. Generate homography to scale rectangles.
            HomMat2dIdentity(&hv_HomMat2DIdentity);
            HomMat2dScale(hv_HomMat2DIdentity, hv_FactorResampleHeight,
                          hv_FactorResampleWidth, 0, 0, &hv_HomMat2DScale);
            // Generate XLD contours for the rectangles.
            GenRectangle2ContourXld(&ho_Rectangle2XLD, hv_BBoxRowNew, hv_BBoxColNew,
                                    hv_BBoxPhiNew, hv_BBoxLength1New,
                                    hv_BBoxLength2New);
            // Scale the XLD contours --> results in sheared regions.
            AffineTransContourXld(ho_Rectangle2XLD, &ho_Rectangle2XLDSheared,
                                  hv_HomMat2DScale);
            SmallestRectangle2Xld(ho_Rectangle2XLDSheared, &hv_BBoxRowNew,
                                  &hv_BBoxColNew, &hv_BBoxPhiNew, &hv_BBoxLength1New,
                                  &hv_BBoxLength2New);
            //
            // smallest_rectangle2_xld might change the orientation of the bounding
            // box. Hence, take the orientation that is closest to the one obtained
            // out of the 4 corner points.
            convert_rect2_8to5param(
                        hv_BBoxRow1New, hv_BBoxCol1New, hv_BBoxRow2New, hv_BBoxCol2New,
                        hv_BBoxRow3New, hv_BBoxCol3New, hv_BBoxRow4New, hv_BBoxCol4New,
                        hv_IgnoreDirection, &hv__, &hv__, &hv__, &hv__, &hv_BBoxPhiTmp);
            hv_PhiDelta =
                    (hv_BBoxPhiTmp - hv_BBoxPhiNew).TupleFmod(HTuple(360).TupleRad());
            // Guarantee that angles are positive.
            hv_PhiDeltaNegativeIndices =
                    (hv_PhiDelta.TupleLessElem(0.0)).TupleFind(1);
            if (0 != (int(hv_PhiDeltaNegativeIndices != -1))) {
                hv_PhiDelta[hv_PhiDeltaNegativeIndices] =
                        HTuple(hv_PhiDelta[hv_PhiDeltaNegativeIndices]) +
                        (HTuple(360).TupleRad());
            }
            hv_IndicesRot90 =
                    HTuple((hv_PhiDelta.TupleGreaterElem(HTuple(45).TupleRad()))
                           .TupleAnd(hv_PhiDelta.TupleLessEqualElem(
                                         HTuple(135).TupleRad())))
                    .TupleFind(1);
            hv_IndicesRot180 =
                    HTuple((hv_PhiDelta.TupleGreaterElem(HTuple(135).TupleRad()))
                           .TupleAnd(hv_PhiDelta.TupleLessEqualElem(
                                         HTuple(225).TupleRad())))
                    .TupleFind(1);
            hv_IndicesRot270 =
                    HTuple((hv_PhiDelta.TupleGreaterElem(HTuple(225).TupleRad()))
                           .TupleAnd(hv_PhiDelta.TupleLessEqualElem(
                                         HTuple(315).TupleRad())))
                    .TupleFind(1);
            hv_SwapIndices = HTuple();
            if (0 != (int(hv_IndicesRot90 != -1))) {
                hv_BBoxPhiNew[hv_IndicesRot90] =
                        HTuple(hv_BBoxPhiNew[hv_IndicesRot90]) + (HTuple(90).TupleRad());
                hv_SwapIndices = hv_SwapIndices.TupleConcat(hv_IndicesRot90);
            }
            if (0 != (int(hv_IndicesRot180 != -1))) {
                hv_BBoxPhiNew[hv_IndicesRot180] =
                        HTuple(hv_BBoxPhiNew[hv_IndicesRot180]) + (HTuple(180).TupleRad());
            }
            if (0 != (int(hv_IndicesRot270 != -1))) {
                hv_BBoxPhiNew[hv_IndicesRot270] =
                        HTuple(hv_BBoxPhiNew[hv_IndicesRot270]) + (HTuple(270).TupleRad());
            ...
            if (0 != (int(hv_SwapIndices != HTuple()))) {
                ...
                hv_BBoxLength1New[hv_SwapIndices] =
                        HTuple(hv_BBoxLength2New[hv_SwapIndices]);
                hv_BBoxLength2New[hv_SwapIndices] = hv_Tmp;
            }
            // Change angles such that they lie in the range (-180£, 180£].
            hv_BBoxPhiNewIndices =
                    (hv_BBoxPhiNew.TupleGreaterElem(HTuple(180).TupleRad())).TupleFind(1);
            if (0 != (int(hv_BBoxPhiNewIndices != -1))) {
                hv_BBoxPhiNew[hv_BBoxPhiNewIndices] =
                        HTuple(hv_BBoxPhiNew[hv_BBoxPhiNewIndices]) -
                        (HTuple(360).TupleRad());
            }
            //
        } else {
    ...
    hv_PhiThreshold =
            (HTuple(180).TupleRad()) - (hv_IgnoreDirection * (HTuple(90).TupleRad()));
    hv_PhiDelta = 2 * hv_PhiThreshold;
    // Correct angles that are too large.
    hv_PhiToCorrect =
            (hv_BBoxPhiNew.TupleGreaterElem(hv_PhiThreshold)).TupleFind(1);
    if (0 != (HTuple(int(hv_PhiToCorrect != -1))
              .TupleAnd(int(hv_PhiToCorrect != HTuple())))) {
        hv_NumCorrections =
                (((HTuple(hv_BBoxPhiNew[hv_PhiToCorrect]) - hv_PhiThreshold) /
                  hv_PhiDelta)
                 .TupleInt()) +
                1;
        hv_BBoxPhiNew[hv_PhiToCorrect] = HTuple(hv_BBoxPhiNew[hv_PhiToCorrect]) -
                (hv_NumCorrections * hv_PhiDelta);
    }
    // Correct angles that are too small.
    hv_PhiToCorrect =
            (hv_BBoxPhiNew.TupleLessEqualElem(-hv_PhiThreshold)).TupleFind(1);
    if (0 != (HTuple(int(hv_PhiToCorrect != -1))
              .TupleAnd(int(hv_PhiToCorrect != HTuple())))) {
        hv_NumCorrections =
                ((((HTuple(hv_BBoxPhiNew[hv_PhiToCorrect]) + hv_PhiThreshold)
    ...
    if (0 != (int((hv_BBoxRowNew.TupleLength()) > 0))) {
        hv_BoxesInvalid = ((hv_BBoxLength1New.TupleEqualElem(0)).TupleSum()) +
                ((hv_BBoxLength2New.TupleEqualElem(0)).TupleSum());
        if (0 != (int(hv_BoxesInvalid > 0))) {
            GetDictTuple(hv_DLSample, "image_id", &hv_ImageId);
            throw HException(
                        ("An error has occurred during preprocessing image_id " +
                         hv_ImageId) +
                        HTuple(": Sample contains at least one box with zero-area, i.e. "
                               "bbox_length1 == 0 or bbox_length2 == 0!"));
        }
    }
...
// Chapter: Deep Learning / Model
// Short Description: Preprocess images for deep-learning-based training and
// inference.
void preprocess_dl_model_images(HObject ho_Images,
                                HObject *ho_ImagesPreprocessed,
                                HTuple hv_DLPreprocessParam) {
    ...
    // in the dictionary DLPreprocessParam. Note that depending on the images,
    // additional preprocessing steps might be beneficial.
    //
    ...
    GetDictTuple(hv_DLPreprocessParam, "image_width", &hv_ImageWidth);
    GetDictTuple(hv_DLPreprocessParam, "image_height", &hv_ImageHeight);
    GetDictTuple(hv_DLPreprocessParam, "image_num_channels",
                 &hv_ImageNumChannels);
    GetDictTuple(hv_DLPreprocessParam, "image_range_min", &hv_ImageRangeMin);
    GetDictTuple(hv_DLPreprocessParam, "image_range_max", &hv_ImageRangeMax);
    GetDictTuple(hv_DLPreprocessParam, "domain_handling", &hv_DomainHandling);
    GetDictTuple(hv_DLPreprocessParam, "normalization_type",
                 &hv_NormalizationType);
    GetDictTuple(hv_DLPreprocessParam, "model_type", &hv_ModelType);
    //
    ...
    if (0 != (int(hv_NumMatches != hv_NumImages))) {
        throw HException(
                    HTuple("Please provide only images of type 'byte', 'int1', 'int2', "
                           "'uint2', 'int4', 'int8', or 'real'."));
    }
    //
    // Handle ocr_recognition models.
    if (0 != (int(hv_ModelType == HTuple("ocr_recognition")))) {
        preprocess_dl_model_images_ocr_recognition(
                    ho_Images, &(*ho_ImagesPreprocessed), hv_DLPreprocessParam);
        return;
    }
    //
    ...
    // Only for 'image_num_channels' 1 and 3 combinations of 1- and 3-channel
    // images are allowed.
    if (0 != (HTuple(int(hv_ImageNumChannels == 1))
              .TupleOr(int(hv_ImageNumChannels == 3)))) {
        hv_NumChannels1 = HTuple(hv_NumImages, 1);
        hv_NumChannels3 = HTuple(hv_NumImages, 3);
        hv_AreInputNumChannels1 =
                hv_InputNumChannels.TupleEqualElem(hv_NumChannels1);
        hv_AreInputNumChannels3 =
                hv_InputNumChannels.TupleEqualElem(hv_NumChannels3);
        hv_AreInputNumChannels1Or3 =
                hv_AreInputNumChannels1 + hv_AreInputNumChannels3;
        hv_ValidNumChannels = int(hv_AreInputNumChannels1Or3 == hv_NumChannels1);
        hv_ValidNumChannelsText =
                "Valid numbers of channels for the specified model are 1 or 3.";
    } else {
        hv_ValidNumChannels = int(hv_InputNumChannels == hv_OutputNumChannels);
        hv_ValidNumChannelsText =
                ("Valid number of channels for the specified model is " +
                 hv_ImageNumChannels) +
                ".";
    }
    if (0 != (hv_ValidNumChannels.TupleNot())) {
        throw HException("Please provide images with a valid number of channels. " +
                         hv_ValidNumChannelsText);
    }
    // Preprocess the images.
    //
    // For models of type '3d_gripping_point_detection', the preprocessing steps
    // need to be performed on full domain images while the domains are preserved
    // and set back into the images after the preprocessing.
    hv_PreserveDomain = 0;
    if (0 !=
            (HTuple(int(hv_ModelType == HTuple("3d_gripping_point_detection")))
             .TupleAnd(HTuple(int(hv_DomainHandling == HTuple("crop_domain")))
                       .TupleOr(int(hv_DomainHandling ==
                                    HTuple("keep_domain")))))) {
        hv_PreserveDomain = 1;
    ...
    if (0 != (int(hv_DomainHandling == HTuple("full_domain")))) {
        ...
        if (0 != hv_PreserveDomain) {
            // In case of preserved domain, the crop is performed with the smallest
            // rectangle of the domain to avoid out of domain pixels being set to 0.
            SmallestRectangle1(ho_PreservedDomains, &hv_Row1, &hv_Column1, &hv_Row2,
                               &hv_Column2);
            ...
            if (0 != (HTuple(HTuple(HTuple(int((hv_UniqRow1.TupleLength()) == 1))
                                    .TupleAnd(
                                        int((hv_UniqColumn1.TupleLength()) == 1)))
                             .TupleAnd(int((hv_UniqRow2.TupleLength()) == 1)))
                      .TupleAnd(int((hv_UniqColumn2.TupleLength()) == 1)))) {
                CropRectangle1(ho_Images, &ho_Images, hv_UniqRow1, hv_UniqColumn1,
                               hv_UniqRow2, hv_UniqColumn2);
                MoveRegion(ho_PreservedDomains, &ho_PreservedDomains, -hv_UniqRow1,
                           -hv_UniqColumn1);
            } else {
                {
                    HTuple end_val87 = (hv_Row1.TupleLength()) - 1;
                    HTuple step_val87 = 1;
                    for (hv_RectangleIndex = 0;
                         hv_RectangleIndex.Continue(end_val87, step_val87);
                         hv_RectangleIndex += step_val87) {
                        SelectObj(ho_Images, &ho_ImageSelected, hv_RectangleIndex + 1);
                        CropRectangle1(ho_ImageSelected, &ho_ImageSelected,
                                       HTuple(hv_Row1[hv_RectangleIndex]),
                                       HTuple(hv_Column1[hv_RectangleIndex]),
                                       HTuple(hv_Row2[hv_RectangleIndex]),
                                       HTuple(hv_Column2[hv_RectangleIndex]));
                        ReplaceObj(ho_Images, ho_ImageSelected, &ho_Images,
                                   hv_RectangleIndex + 1);
                        SelectObj(ho_PreservedDomains, &ho_DomainSelected,
                                  hv_RectangleIndex + 1);
                        MoveRegion(ho_DomainSelected, &ho_DomainSelected,
                                   -HTuple(hv_Row1[hv_RectangleIndex]),
                                   -HTuple(hv_Column1[hv_RectangleIndex]));
                        ReplaceObj(ho_PreservedDomains, ho_DomainSelected,
                                   &ho_PreservedDomains, hv_RectangleIndex + 1);
                    }
                }
            }
        } else {
            CropDomain(ho_Images, &ho_Images);
               ...
               (HTuple(int(hv_DomainHandling == HTuple("keep_domain")))
                .TupleAnd(
                    HTuple(int(hv_ModelType == HTuple("anomaly_detection")))
                    .TupleOr(
                        int(hv_ModelType ==
                            HTuple("3d_gripping_point_detection")))))) {
        // The option 'keep_domain' is only supported for models of 'type' =
        // 'anomaly_detection' or '3d_gripping_point_detection'.
    } else {
        throw HException("Unsupported parameter value for 'domain_handling'.");
    }
    //
    ...
    if (0 != hv_PreserveDomain) {
        ...
        if (0 != (HTuple(int((hv_UniqWidth.TupleLength()) == 1))
                  .TupleAnd(int((hv_UniqHeight.TupleLength()) == 1)))) {
            hv_ScaleWidth = hv_ImageWidth / (hv_UniqWidth.TupleReal());
            hv_ScaleHeight = hv_ImageHeight / (hv_UniqHeight.TupleReal());
            ZoomRegion(ho_PreservedDomains, &ho_PreservedDomains, hv_ScaleWidth,
                       hv_ScaleHeight);
        } else {
            ...
            hv_ScaleHeight = hv_ImageHeight / (hv_OriginalHeight.TupleReal());
            {
                ...
                HTuple step_val117 = 1;
                for (hv_ScaleIndex = 0; hv_ScaleIndex.Continue(end_val117, step_val117);
                     hv_ScaleIndex += step_val117) {
                    SelectObj(ho_PreservedDomains, &ho_DomainSelected, hv_ScaleIndex + 1);
                    ZoomRegion(ho_DomainSelected, &ho_DomainSelected,
                               HTuple(hv_ScaleWidth[hv_ScaleIndex]),
                               HTuple(hv_ScaleHeight[hv_ScaleIndex]));
                    ReplaceObj(ho_PreservedDomains, ho_DomainSelected,
                               &ho_PreservedDomains, hv_ScaleIndex + 1);
                }
            }
        }
    }
    ...
    if (0 != (int((hv_Type.TupleRegexpTest("int1|int4|int8")) == 0))) {
        ZoomImageSize(ho_Images, &ho_Images, hv_ImageWidth, hv_ImageHeight,
                      "constant");
    ...
    if (0 != (int(hv_NormalizationType == HTuple("all_channels")))) {
        // Scale for each image the gray values of all channels to
        // ImageRangeMin-ImageRangeMax.
        GenEmptyObj(&ho_ImagesScaled);
        {
            HTuple end_val138 = hv_NumImages;
            HTuple step_val138 = 1;
            for (hv_ImageIndex = 1; hv_ImageIndex.Continue(end_val138, step_val138);
                 hv_ImageIndex += step_val138) {
                ...
                GenEmptyObj(&ho_ImageScaled);
                {
                    ...
                    for (hv_ChannelIndex = 1;
                         hv_ChannelIndex.Continue(end_val142, step_val142);
                         hv_ChannelIndex += step_val142) {
                        ...
                        if (0 != (int((hv_Max - hv_Min) == 0))) {
                            ...
                            hv_Scale =
                                    (hv_ImageRangeMax - hv_ImageRangeMin) / (hv_Max - hv_Min);
                        }
        ...
        // Scale for each image the gray values of first channel to
        // ImageRangeMin-ImageRangeMax.
        GenEmptyObj(&ho_ImagesScaled);
        {
            HTuple end_val160 = hv_NumImages;
            HTuple step_val160 = 1;
            for (hv_ImageIndex = 1; hv_ImageIndex.Continue(end_val160, step_val160);
                 hv_ImageIndex += step_val160) {
                SelectObj(ho_Images, &ho_ImageSelected, hv_ImageIndex);
                MinMaxGray(ho_ImageSelected, ho_ImageSelected, 0, &hv_Min, &hv_Max,
                           &hv_Range);
                if (0 != (int((hv_Max - hv_Min) == 0))) {
                    hv_Scale = 1;
                } else {
        ...
        // Scale for each image the gray values of all channels to the corresponding
        // channel DeviationValues[].
        try {
            GetDictTuple(hv_DLPreprocessParam, "mean_values_normalization",
                         &hv_MeanValues);
            GetDictTuple(hv_DLPreprocessParam, "deviation_values_normalization",
                         &hv_DeviationValues);
        ...
        GenEmptyObj(&ho_ImagesScaled);
        {
            ...
            for (hv_ImageIndex = 1; hv_ImageIndex.Continue(end_val185, step_val185);
                 hv_ImageIndex += step_val185) {
                SelectObj(ho_Images, &ho_ImageSelected, hv_ImageIndex);
                CountChannels(ho_ImageSelected, &hv_NumChannels);
                // Ensure that the number of channels is equal |DeviationValues| and
                // |MeanValues|
                if (0 != hv_UseDefaultNormalizationValues) {
                    if (0 != (int(hv_NumChannels == 1))) {
                        Compose3(ho_ImageSelected, ho_ImageSelected, ho_ImageSelected,
                                 &ho_ImageSelected);
                        CountChannels(ho_ImageSelected, &hv_NumChannels);
                    } else if (0 != (int(hv_NumChannels != 3))) {
                        throw HException(
                                    "Using default values for normalization type 'constant_values' "
                                    "is allowed only for 1- and 3-channel images.");
                    }
                }
                if (0 != (HTuple(int((hv_MeanValues.TupleLength()) != hv_NumChannels))
                          .TupleOr(int((hv_DeviationValues.TupleLength()) !=
                                       hv_NumChannels)))) {
                    throw HException(
                                "The length of mean and deviation values for normalization type "
                                "'constant_values' have to be the same size as the number of "
                ...
                GenEmptyObj(&ho_ImageScaled);
                {
                    ...
                    for (hv_ChannelIndex = 1;
                         hv_ChannelIndex.Continue(end_val201, step_val201);
                         hv_ChannelIndex += step_val201) {
                        AccessChannel(ho_ImageSelected, &ho_Channel, hv_ChannelIndex);
        ...
        if (0 != (int(hv_Indices != -1))) {
            // Shift the gray values from [0-255] to the expected range for byte
            // images.
            hv_RescaleRange = (hv_ImageRangeMax - hv_ImageRangeMin) / 255.0;
            SelectObj(ho_Images, &ho_ImageSelected, hv_Indices + 1);
            ScaleImage(ho_ImageSelected, &ho_ImageSelected, hv_RescaleRange,
                       hv_ImageRangeMin);
    ...
    //
    // Ensure that the number of channels of the resulting images is consistent
    // with the number of channels of the given model. The only exceptions that
    // are adapted below are combinations of 1- and 3-channel images if
    // ImageNumChannels is either 1 or 3.
    if (0 != (HTuple(int(hv_ImageNumChannels == 1))
              .TupleOr(int(hv_ImageNumChannels == 3)))) {
        CountChannels(ho_Images, &hv_CurrentNumChannels);
        TupleFind(hv_CurrentNumChannels.TupleNotEqualElem(hv_OutputNumChannels), 1,
                  &hv_DiffNumChannelsIndices);
        if (0 != (int(hv_DiffNumChannelsIndices != -1))) {
            {
                HTuple end_val231 = (hv_DiffNumChannelsIndices.TupleLength()) - 1;
                HTuple step_val231 = 1;
                for (hv_Index = 0; hv_Index.Continue(end_val231, step_val231);
                     hv_Index += step_val231) {
                    ...
                    SelectObj(ho_Images, &ho_ImageSelected, hv_ImageIndex);
                    if (0 != (HTuple(int(hv_NumChannels == 1))
                              .TupleAnd(int(hv_ImageNumChannels == 3)))) {
                        // Conversion from 1- to 3-channel image required
                        Compose3(ho_ImageSelected, ho_ImageSelected, ho_ImageSelected,
                                 &ho_ThreeChannelImage);
                        ReplaceObj(ho_Images, ho_ThreeChannelImage, &ho_Images,
                                   hv_ImageIndex);
                    } else if (0 != (HTuple(int(hv_NumChannels == 3))
                                     .TupleAnd(int(hv_ImageNumChannels == 1)))) {
                        // Conversion from 3- to 1-channel image required
                        Rgb1ToGray(ho_ImageSelected, &ho_SingleChannelImage);
                        ReplaceObj(ho_Images, ho_SingleChannelImage, &ho_Images,
                                   hv_ImageIndex);
                    } else {
                        throw HException(
                                    ((("Unexpected error adapting the number of channels. The "
                                       "number of channels of the resulting image is " +
                                       hv_NumChannels) +
                                      HTuple(", but the number of channels of the model is ")) +
                                     hv_ImageNumChannels) +
                                    ".");
                    }
                }
    ...
    //
    // In case the image domains were preserved, they need to be set back into the
    // images.
    if (0 != hv_PreserveDomain) {
        CountObj(ho_PreservedDomains, &hv_NumDomains);
        {
            ...
            for (hv_DomainIndex = 1; hv_DomainIndex.Continue(end_val254, step_val254);
                 hv_DomainIndex += step_val254) {
...
// Chapter: OCR / Deep OCR
// Short Description: Preprocess images for deep-learning-based training and
// inference of Deep OCR detection models.
void preprocess_dl_model_images_ocr_detection(HObject ho_Images,
                                              HObject *ho_ImagesPreprocessed,
                                              HTuple hv_DLPreprocessParam) {
    ...
    if (0 != (int(hv_ModelType != HTuple("ocr_detection")))) {
        throw HException(
                    "The only 'model_type' value supported is'ocr_detection'.");
    }
    if (0 != (int(hv_ImageNumChannels != 3))) {
        throw HException("The only 'image_num_channels' value supported for "
                         "ocr_detection models is 3.");
    ...
    if (0 != (int(hv_DomainHandling != HTuple("full_domain")))) {
        throw HException("The only 'domain_handling' value supported for "
                         "ocr_detection models is 'full_domain'.");
    }
    if (0 !=
            (HTuple(int(hv_NormalizationType != HTuple("none")))
             .TupleAnd(int(hv_NormalizationType != HTuple("all_channels"))))) {
        throw HException("The 'normalization_type' values supported for "
                         "ocr_detection models are 'all_channels' and 'none'.");
    }
    //
    ...
    if (0 != (int(hv_NumImages !=
                  (HTuple((hv_NumChannels.TupleEqualElem(1))
                          .TupleOr(hv_NumChannels.TupleEqualElem(3)))
                   .TupleSum())))) {
        throw HException("Please provide only 1- or 3-channels images for "
                         "ocr_detection models.");
    ...
    hv_ImageRange = (hv_ImageRangeMax - hv_ImageRangeMin).TupleReal();
    {
        ...
        for (hv_I = 0; hv_I.Continue(end_val49, step_val49); hv_I += step_val49) {
            ...
            hv_ZoomHeight = (hv_ZoomFactorHeight * hv_InputImageHeight).TupleRound();
            hv_ZoomWidth = (hv_ZoomFactorWidth * hv_InputImageWidth).TupleRound();
            ZoomImageSize(ho_Image, &ho_Image, hv_ZoomWidth, hv_ZoomHeight,
                          "constant");
            //
            ...
            if (0 != (int(hv_NormalizationType == HTuple("all_channels")))) {
                GenEmptyObj(&ho_ImageScaled);
                {
                    ...
                    HTuple step_val67 = 1;
                    for (hv_ChannelIndex = 1;
                         hv_ChannelIndex.Continue(end_val67, step_val67);
                         hv_ChannelIndex += step_val67) {
                ...
                ScaleImage(ho_Image, &ho_Image, hv_ImageRange / 255.0,
                           hv_ImageRangeMin);
            }
            //
            // Obtain an RGB image.
...
// Chapter: OCR / Deep OCR
// Short Description: Preprocess images for deep-learning-based training and
// inference of Deep OCR recognition models.
void preprocess_dl_model_images_ocr_recognition(HObject ho_Images,
                                                HObject *ho_ImagesPreprocessed,
                                                HTuple hv_DLPreprocessParam) {
    ...
    HTuple hv_I, hv_InputImageWidth, hv_InputImageHeight,
            hv_InputImageWidthHeightRatio;
    ...
    if (0 != (int(hv_ModelType != HTuple("ocr_recognition")))) {
        throw HException(
                    "The only 'model_type' value supported is'ocr_recognition'.");
    }
    if (0 != (int(hv_ImageNumChannels != 1))) {
        throw HException("The only 'image_num_channels' value supported for "
                         "ocr_recognition models is 1.");
    ...
    if (0 != (int(hv_DomainHandling != HTuple("full_domain")))) {
        throw HException("The only 'domain_handling' value supported for "
                         "ocr_recognition models is 'full_domain'.");
    }
    if (0 !=
            (HTuple(
                 HTuple(int(hv_NormalizationType != HTuple("none")))
                 .TupleAnd(int(hv_NormalizationType != HTuple("first_channel"))))
             .TupleAnd(int(hv_NormalizationType != HTuple("all_channels"))))) {
        throw HException(
                    HTuple("The 'normalization_type' values supported for ocr_recognition "
                           "models are 'first_channel', 'all_channels' and 'none'."));
    }
    //
    // Get the image properties.
    CountObj(ho_Images, &hv_NumImages);
    ...
    if (0 !=
            (int(hv_NumImages != (hv_ImageTypes.TupleRegexpTest("byte|real"))))) {
        throw HException("Please provide only images of type 'byte' or 'real'.");
    }
    if (0 != (int(hv_NumImages !=
                  (HTuple((hv_NumChannels.TupleEqualElem(1))
    ...
    OverpaintRegion(ho_TargetImage, ho_TargetImage, hv_PaddingGrayval, "fill");
    {
        ...
        for (hv_I = 0; hv_I.Continue(end_val52, step_val52); hv_I += step_val52) {
            ...
            if (0 !=
                    (HTuple(int(hv_NormalizationType == HTuple("first_channel")))
                     .TupleOr(int(hv_NormalizationType == HTuple("all_channels"))))) {
                MinMaxGray(ho_Image, ho_Image, 0, &hv_GrayvalMin, &hv_GrayvalMax,
                           &hv_Range);
...
// Chapter: Deep Learning / Object Detection and Instance Segmentation
// Short Description: Preprocess the instance segmentation masks for a sample
// given by the dictionary DLSample.
void preprocess_dl_model_instance_masks(HObject ho_ImageRaw, HTuple hv_DLSample,
    ...
    if (0 != (int(hv_DomainHandling == HTuple("crop_domain")))) {
        ...
        SmallestRectangle1(ho_Domain, &hv_DomainRow1, &hv_DomainColumn1,
                           &hv_DomainRow2, &hv_DomainColumn2);
        //
        // Clip the remaining regions to the domain.
        ClipRegion(ho_InstanceMasks, &ho_InstanceMasks, hv_DomainRow1,
        ...
        MoveRegion(ho_InstanceMasks, &ho_InstanceMasks, -hv_DomainRow1,
                   -hv_DomainColumn1);
    } else if (0 != (int(hv_DomainHandling != HTuple("full_domain")))) {
        throw HException("Unsupported parameter value for 'domain_handling'");
    }
    ...
    if (0 != ((hv_ImageWidth.TupleNotEqualElem(hv_WidthRaw))
              .TupleOr(hv_ImageHeight.TupleNotEqualElem(hv_HeightRaw)))) {
        // Calculate rescaling factor.
        hv_FactorResampleWidth = (hv_ImageWidth.TupleReal()) / hv_WidthRaw;
        hv_FactorResampleHeight = (hv_ImageHeight.TupleReal()) / hv_HeightRaw;

        // Zoom the masks.
        ZoomRegion(ho_InstanceMasks, &ho_InstanceMasks, hv_FactorResampleWidth,
                   hv_FactorResampleHeight);
    }
    //
...
// Chapter: Deep Learning / Semantic Segmentation and Edge Extraction
// Short Description: Preprocess segmentation and weight images for
// deep-learning-based segmentation training and inference.
void preprocess_dl_model_segmentations(HObject ho_ImagesRaw,
                                       HObject ho_Segmentations,
                                       HObject *ho_SegmentationsPreprocessed,
                                       HTuple hv_DLPreprocessParam) {
    ...
    if (0 != (int(hv_NumberImages != hv_NumberSegmentations))) {
        throw HException(
                    "Equal number of images given in ImagesRaw and Segmentations required");
    }
    // Size of images.
    GetImageSize(ho_ImagesRaw, &hv_Width, &hv_Height);
    GetImageSize(ho_Segmentations, &hv_WidthSeg, &hv_HeightSeg);
    if (0 != (HTuple(int(hv_Width != hv_WidthSeg))
              .TupleOr(int(hv_Height != hv_HeightSeg)))) {
        throw HException("Equal size of the images given in ImagesRaw and "
                         "Segmentations required.");
    }
    // Check the validity of the preprocessing parameters.
    check_dl_preprocess_param(hv_DLPreprocessParam);
    //
    ...
    if (0 != (int(hv_SetBackgroundID != HTuple()))) {
        ...
        if (0 != (int(hv_DLModelType != HTuple("segmentation")))) {
            throw HException("Setting class IDs to background is only implemented "
                             "for segmentation.");
        }
        ...
        if (0 != (int((hv_SetBackgroundID.TupleLength()) != 1))) {
            ...
            throw HException(
                        "The class_id given as 'set_background_id' has to be of type int.");
        }
        ...
        if (0 != (int((hv_ClassesToBackground.TupleLength()) == 0))) {
            ...
            throw HException(
                        HTuple("If 'set_background_id' is given, 'class_ids_background' must "
                               "at least contain this class ID."));
        } else if (0 != (int((hv_ClassesToBackground.TupleIntersection(
                                  hv_IgnoreClassIDs)) != HTuple()))) {
            // Check that class_ids_background is not included in the ignore_class_ids
            // of the DLModel.
            throw HException("The given 'class_ids_background' must not be included "
                             "in the 'ignore_class_ids' of the model.");
        }
    }
    //
    // Domain handling of the image to be preprocessed.
    //
    if (0 != (HTuple(int(hv_DomainHandling == HTuple("full_domain")))
              .TupleOr(int(hv_DomainHandling == HTuple("keep_domain"))))) {
        ...
        GetDomain(ho_ImagesRaw, &ho_Domain);
        {
            ...
            HTuple step_val66 = 1;
            for (hv_IndexImage = 1; hv_IndexImage.Continue(end_val66, step_val66);
                 hv_IndexImage += step_val66) {
                ...
                ReplaceObj(ho_Segmentations, ho_SelectedSeg, &ho_Segmentations,
                           hv_IndexImage);
            }
        }
    ...
    if (0 != (int(hv_SetBackgroundID != HTuple()))) {
        reassign_pixel_values(ho_Segmentations, &ho_Segmentations,
    ...
    if (0 != (HTuple(int((hv_EqualWidth.TupleMin()) == 0))
              .TupleOr(int((hv_EqualHeight.TupleMin()) == 0)))) {
        ZoomImageSize(ho_Segmentations, &ho_Segmentations, hv_ImageWidth,
                      hv_ImageHeight, "nearest_neighbor");
    }
    //
    // Check the type of the input images
...
// Chapter: Deep Learning / Model
// Short Description: Preprocess given DLSamples according to the preprocessing
// parameters given in DLPreprocessParam.
void preprocess_dl_samples(HTuple hv_DLSampleBatch,
                           HTuple hv_DLPreprocessParam) {
    ...
    //
    {
        ...
        HTuple step_val18 = 1;
        for (hv_SampleIndex = 0; hv_SampleIndex.Continue(end_val18, step_val18);
             hv_SampleIndex += step_val18) {
            ...
            if (0 != hv_ImageExists) {
                ...
                preprocess_dl_model_images(ho_ImageRaw, &ho_ImagePreprocessed,
                ...
                GetDictParam(hv_DLSample, "key_exists",
                             (((((HTuple("anomaly_ground_truth").Append("bbox_row1"))
                                 .Append("bbox_phi"))
                                .Append("mask"))
                               .Append("segmentation_image"))
                              .Append("word")),
                             &hv_KeysExists);
                ...
                if (0 != hv_AnomalyParamExist) {
                    ...
                    GetDictObject(&ho_AnomalyImageRaw, hv_DLSample,
                                  "anomaly_ground_truth");
                    //
                    // Preprocess the anomaly image.
                    preprocess_dl_model_anomaly(ho_AnomalyImageRaw,
                                                &ho_AnomalyImagePreprocessed,
                                                hv_DLPreprocessParam);
                    //
                    // Set preprocessed anomaly image.
                    SetDictObject(ho_AnomalyImagePreprocessed, hv_DLSample,
                                  "anomaly_ground_truth");
                }
                //
                ...
                if (0 != hv_Rectangle1ParamExist) {
                    ...
                    preprocess_dl_model_bbox_rect1(ho_ImageRaw, hv_DLSample,
                                                   hv_DLPreprocessParam);
                } else if (0 != hv_Rectangle2ParamExist) {
                    //
                    // Preprocess the bounding boxes of type 'rectangle2'.
                    preprocess_dl_model_bbox_rect2(ho_ImageRaw, hv_DLSample,
                                                   hv_DLPreprocessParam);
                }
                if (0 != hv_InstanceMaskParamExist) {
                    //
                    // Preprocess the instance masks.
                    preprocess_dl_model_instance_masks(ho_ImageRaw, hv_DLSample,
                                                       hv_DLPreprocessParam);
                }
                //
                ...
                if (0 != hv_SegmentationParamExist) {
                    ...
                    preprocess_dl_model_segmentations(ho_ImageRaw, ho_SegmentationRaw,
                                                      &ho_SegmentationPreprocessed,
                                                      hv_DLPreprocessParam);
                    //
                    // Set preprocessed segmentation image.
                    SetDictObject(ho_SegmentationPreprocessed, hv_DLSample,
                                  "segmentation_image");
                }
                //
                ...
                GetDictParam(
                            hv_DLSample, "key_exists",
                            (((HTuple("x").Append("y")).Append("z")).Append("normals")),
                            &hv_KeysExists);
                if (0 != (hv_KeysExists.TupleMax())) {
                    ...
                    crop_dl_sample_image(ho_ImageRawDomain, hv_DLSample, "x",
                                         hv_DLPreprocessParam);
                    crop_dl_sample_image(ho_ImageRawDomain, hv_DLSample, "y",
                                         hv_DLPreprocessParam);
                    crop_dl_sample_image(ho_ImageRawDomain, hv_DLSample, "z",
                                         hv_DLPreprocessParam);
                    crop_dl_sample_image(ho_ImageRawDomain, hv_DLSample, "normals",
                                         hv_DLPreprocessParam);
                    //
                    preprocess_dl_model_3d_data(hv_DLSample, hv_DLPreprocessParam);
                }
            } else {
                throw HException((HTuple("All samples processed need to include an "
                                         "image, but the sample with index ") +
                                  hv_SampleIndex) +
                                 " does not.");
            }
        }
...
void reassign_pixel_values(HObject ho_Image, HObject *ho_ImageOut,
    ...
    GenEmptyRegion(&ho_RegionToChange);
    {
        ...
        HTuple step_val5 = 1;
        for (hv_IndexReset = 0; hv_IndexReset.Continue(end_val5, step_val5);
             hv_IndexReset += step_val5) {
            Threshold(ho_Image, &ho_RegionClass,
...
void remove_invalid_3d_pixels(HObject ho_ImageX, HObject ho_ImageY,
                              HObject ho_ImageZ, HObject ho_Domain,
                              HObject *ho_DomainOut,
                              HTuple hv_InvalidPixelValue) {
    ...
    Threshold(ho_ImageXOut, &ho_RegionInvalX, hv_InvalidPixelValue,
              hv_InvalidPixelValue);
    Threshold(ho_ImageYOut, &ho_RegionInvalY, hv_InvalidPixelValue,
              hv_InvalidPixelValue);
    Threshold(ho_ImageZOut, &ho_RegionInvalZ, hv_InvalidPixelValue,
              hv_InvalidPixelValue);
...
void replace_legacy_preprocessing_parameters(HTuple hv_DLPreprocessParam) {
    ...
    hv_Exception = 0;
    try {
        GetDictParam(hv_DLPreprocessParam, "key_exists", "normalization_type",
                     &hv_NormalizationTypeExists);
        //
    ...
    hv_Exception = 0;
    try {
        GetDictParam(hv_DLPreprocessParam, "key_exists", "contrast_normalization",
                     &hv_LegacyNormalizationKeyExists);
        if (0 != hv_LegacyNormalizationKeyExists) {
            GetDictTuple(hv_DLPreprocessParam, "contrast_normalization",
                         &hv_ContrastNormalization);
...
// Short Description: Scale the gray values of an image from the interval
// [Min,Max] to [0,255]
void scale_image_range(HObject ho_Image, HObject *ho_ImageScaled, HTuple hv_Min,
    ...
    CountObj(ho_Image, &hv_NumImages);
    {
        ...
        HTuple step_val51 = 1;
        for (hv_ImageIndex = 1; hv_ImageIndex.Continue(end_val51, step_val51);
             hv_ImageIndex += step_val51) {
            ...
            CountChannels(ho_ImageSelected, &hv_Channels);
            {
                ...
                for (hv_ChannelIndex = 1;
                     ...
                     hv_ChannelIndex += step_val54) {
                    AccessChannel(ho_ImageSelected, &ho_SelectedChannel, hv_ChannelIndex);
                    MinMaxGray(ho_SelectedChannel, ho_SelectedChannel, 0, &hv_MinGray,
                               &hv_MaxGray, &hv_Range);
                    Threshold(ho_SelectedChannel, &ho_LowerRegion,
                              (hv_MinGray.TupleConcat(hv_LowerLimit)).TupleMin(),
                              hv_LowerLimit);
                    Threshold(ho_SelectedChannel, &ho_UpperRegion, hv_UpperLimit,
                              (hv_UpperLimit.TupleConcat(hv_MaxGray)).TupleMax());
                    PaintRegion(ho_LowerRegion, ho_SelectedChannel, &ho_SelectedChannel,
                                hv_LowerLimit, "fill");
                    PaintRegion(ho_UpperRegion, ho_SelectedChannel, &ho_SelectedChannel,
                                hv_UpperLimit, "fill");
                    if (0 != (int(hv_ChannelIndex == 1))) {
                        CopyObj(ho_SelectedChannel, &ho_ImageSelectedScaled, 1, 1);
                    } else {
                        AppendChannel(ho_ImageSelectedScaled, ho_SelectedChannel,
                                      &ho_ImageSelectedScaled);
                    }
                }
            }
...
void disp_message_1(HTuple hv_WindowHandle, HTuple hv_String,
    ...
    //      -- color string: A box is displayed in the given color, e.g., 'white',
    //      '#FF00CC'.
    //    - Box[1] (Optional): Controls the shadow of the box. Possible values:
    //      -- 'true' (Default): A shadow is displayed in
    //                darker orange if Box[0] is not a color and in 'white'
    //                otherwise.
    //      -- 'false': No shadow is displayed.
    //      -- color string: A shadow is displayed in the given color, e.g.,
    //      'white', '#FF00CC'.
    //
    ...
    if (0 !=
            (HTuple(int(hv_Row == HTuple())).TupleOr(int(hv_Column == HTuple())))) {
        return;
    }
    if (0 != (int(hv_Row == -1))) {
        hv_Row = 12;
...
void generate_shape_model_dynamic(
        HObject ho_ImageModel, HObject ho_RegionROI, HObject ho_ModelRegions,
        HObject *ho_ModelRegionsOrigin, HTuple hv_WindowHandle, HTuple hv_ModelIDs,
        HTuple hv_ModelWidthAll, HTuple hv_ModelHeightAll, HTuple hv_ModelGrays,
        HTuple hv_Threshold, HTuple hv_WdiffValue, HTuple hv_IsDynamic,
        HTuple *hv_ModelID, HTuple *hv_ModelWidth, HTuple *hv_ModelHeight,
        HTuple *hv_ModelArea, HTuple *hv_ModelGrayVal, HTuple *hv_isTrained) {

    // Local iconic variables
    HObject ho_RegionBorder, ho_NewObj, ho_DetectedObj;
    HObject ho_IO_Obj, ho_ObjectSelected, ho_ModelRegionROI;
    HObject ho_ImageResult, ho_ShapeModelImages, ho_ShapeModelRegions;
    HObject ho_ModelContours;

    // Local control variables
    HTuple hv_MinScore, hv_Score, hv_isDefect, hv_imgWidth;
    HTuple hv_imgHeight, hv_AreaMax, hv_tmp_WdiffValue, hv_MinDefectInnerRadian;
    HTuple hv_MinDefectArea, hv_ObjNum, hv_Area, hv_CenterRow;
    HTuple hv_CenterColumn, hv_ROILeftR, hv_ROILeftC, hv_ROIRightR;
    HTuple hv_ROIRightC, hv_MaxToySize, hv_NewRow, hv_NewColumn;
    HTuple hv_NewWidth, hv_NewArea, hv_Model, hv_Left, hv_j;
    HTuple hv_objRow, hv_ObjColumn, hv_Rect1RowL, hv_Rect1ColL;
    HTuple hv_Rect1RowR, hv_Rect1ColR, hv_Row1, hv_Column1;
    HTuple hv_Phi, hv_ModelWidthHalf, hv_ModelHeightHalf, hv_Deviation;
    HTuple hv_AreaModelRegions, hv_RowModelRegions, hv_ColumnModelRegions;
    HTuple hv_HeightPyramid, hv_NumLevels, hv_i, hv_ScaleMin;
    HTuple hv_ScaleMax, hv_MinContrast, hv_NumLevels1, hv_AngleStart;
    HTuple hv_AngleExtent, hv_AngleStep, hv_ScaleMin1, hv_ScaleMax1;
    HTuple hv_ScaleStep, hv_Metric, hv_MinContrast1;

    (*hv_isTrained) = 0;

    hv_MinScore = 0.7;
    hv_Score = HTuple();
    (*hv_ModelWidth) = HTuple();
    (*hv_ModelHeight) = HTuple();
    hv_isDefect = HTuple();
    SetColor(hv_WindowHandle, "green");
    SetDraw(hv_WindowHandle, "margin");
    SetLineWidth(hv_WindowHandle, 3);
    // dev_set_color ('green')
    // boundary (RegionROI, RegionBorder, 'outer')
    // dilation_circle (RegionBorder, RegionBorder, 2)
    // paint_region (RegionBorder, ImageModel, ImageTemp, 255, 'fill')
    GetImageSize(ho_ImageModel, &hv_imgWidth, &hv_imgHeight);
    hv_AreaMax = hv_imgWidth * hv_imgHeight;
    hv_tmp_WdiffValue = (3 * hv_imgWidth) / 320;
    hv_MinDefectInnerRadian = (3 * hv_imgWidth) / 320;
    hv_MinDefectArea = (50 * hv_imgWidth) / 320;

    // reduce_domain (ImageModel, RegionROI, ImageReduced)
    // threshold (ImageReduced, ModelRegionsOrigin, 0, Threshold)
    // count_obj (ModelRegionsOrigin, ObjNum)
    AreaCenter(ho_RegionROI, &hv_Area, &hv_CenterRow, &hv_CenterColumn);
    SmallestRectangle1(ho_RegionROI, &hv_ROILeftR, &hv_ROILeftC, &hv_ROIRightR,
                       &hv_ROIRightC);
    hv_MaxToySize = (hv_ROIRightR - hv_ROILeftR) * 0.98;

    Boundary(ho_RegionROI, &ho_RegionBorder, "outer");
    search_shape_model_gray(
                ho_ImageModel, ho_RegionROI, ho_ModelRegions, &(*ho_ModelRegionsOrigin),
                hv_WindowHandle, hv_ModelIDs, hv_ModelGrays, hv_Threshold, hv_WdiffValue,
                hv_MinScore, hv_MinDefectInnerRadian, &hv_NewRow, &hv_NewColumn,
                &hv_NewWidth, &hv_NewArea, &hv_Model, &hv_isDefect);

    // 考虑到挨在一起的物料，以及杂料。
    // 未匹配到物料时，isDefect 物料数量可能大于模版数量 |Model|
    // 重复匹配时，则反过来
    hv_ObjNum = hv_isDefect.TupleLength();
    DispImage(ho_ImageModel, hv_WindowHandle);
    DispRegion(ho_RegionBorder, hv_WindowHandle);
    SetColor(hv_WindowHandle, "magenta");
    DispRegion((*ho_ModelRegionsOrigin), hv_WindowHandle);

    // 如果无料直接退出
    if (0 != (int(hv_ObjNum == 0))) {
        return;
    } else {

        // 看最左边的料是否出边界，出的话 isTrained = 3 ，先计数
        TupleMin(hv_NewColumn, &hv_Left);
        if (0 != (int(hv_Left < (hv_ROILeftC + ((15 * hv_imgWidth) / 320))))) {
            // 如果出视野结果为3
            (*hv_isTrained) = 3;
            return;
        }
        ...
        // 如果有多个料，分手动和自动两种模式
        GenEmptyObj(&ho_NewObj);
        GenEmptyObj(&ho_DetectedObj);
        GenEmptyObj(&ho_IO_Obj);
        // 手动模式下画面内不允许存在多个物料
        if (0 != (int(hv_IsDynamic == 0))) {
            if (0 != (int(hv_ObjNum == 1))) {
                ConcatObj(ho_NewObj, (*ho_ModelRegionsOrigin), &ho_NewObj);
            } else {
                SetColor(hv_WindowHandle, "red");
                DispRegion((*ho_ModelRegionsOrigin), hv_WindowHandle);
                disp_message_1(
                            hv_WindowHandle, //'检测到多个物料！ '
                            "\274\354\262\342\265\275\266\340\270\366\316\357\301\317\243\241 ",
                            "window", -1, -1, "red", "true");
                return;
            }
        } else {
            SortRegion((*ho_ModelRegionsOrigin), &(*ho_ModelRegionsOrigin),
                       "upper_right", "true", "column");
            SelectObj((*ho_ModelRegionsOrigin), &ho_ObjectSelected, 1);
            {
                HTuple end_val72 = hv_ObjNum - 1;
                HTuple step_val72 = 1;
                for (hv_j = 0; hv_j.Continue(end_val72, step_val72);
                     hv_j += step_val72) {
                    if (0 != (int(HTuple(hv_NewColumn[hv_j]) == hv_Left))) {
                        // 只训练处理最左边那个
                        if (0 != (HTuple(int(HTuple(hv_isDefect[hv_j]) == 1))
                                  .TupleOr(int(HTuple(hv_isDefect[hv_j]) == 3)))) {
                            ConcatObj(ho_NewObj, ho_ObjectSelected, &ho_NewObj);
                            break;
                        } else if (0 != (int(HTuple(hv_isDefect[hv_j]) == 2))) {
                            ConcatObj(ho_IO_Obj, ho_ObjectSelected, &ho_IO_Obj);
                            (*hv_isTrained) = 0;
                        } else {
                            if (0 != (int((hv_Model.TupleLength()) > 0))) {
                                (*hv_ModelID) = HTuple(hv_ModelIDs[HTuple(hv_Model[hv_j])]);
                            } else {
                                (*hv_ModelID) = 0;
                            }
                            ConcatObj(ho_DetectedObj, ho_ObjectSelected, &ho_DetectedObj);
                            (*hv_isTrained) = 2;
                        }
                    }
                }
            }
            SetColor(hv_WindowHandle, "orange");
            DispRegion(ho_IO_Obj, hv_WindowHandle);
            // 如果都是训练过的，则不创建模版
            if (0 != (int((*hv_isTrained) == 2))) {
                SetColor(hv_WindowHandle, "magenta");
                // get_rgb (WindowHandle, Red, Green, Blue)
                DispRegion(ho_DetectedObj, hv_WindowHandle);
                disp_message_1(hv_WindowHandle, //'检测到已训练物料ID：'
                               "\274\354\262\342\265\275\322\321\321\265\301\267\316"
                               "\357\301\317ID\243\272" +
                               hv_Model,
                               "window", -1, -1, "magenta", "true");
                return;
            }
        }
    }

    AreaCenter(ho_NewObj, &(*hv_ModelArea), &hv_objRow, &hv_ObjColumn);
    SmallestRectangle1(ho_NewObj, &hv_Rect1RowL, &hv_Rect1ColL, &hv_Rect1RowR,
                       &hv_Rect1ColR);
    SmallestRectangle2(ho_NewObj, &hv_Row1, &hv_Column1, &hv_Phi,
                       &hv_ModelWidthHalf, &hv_ModelHeightHalf);
    (*hv_ModelWidth) = hv_ModelWidthHalf * 2;
    (*hv_ModelHeight) = hv_ModelHeightHalf * 2;

    // 正在进入和出去视野的物料，一律视为杂料并忽略
    if (0 != (HTuple(HTuple(HTuple(int(hv_Rect1ColL > (hv_ROILeftC + 5)))
                            .TupleAnd(int(hv_Rect1ColR < (hv_ROIRightC - 5))))
                     .TupleAnd(int((*hv_ModelArea) > 30)))
              .TupleAnd(int(hv_ObjNum > 0)))) {
        DilationCircle(ho_NewObj, &ho_ModelRegionROI, 3);
        FillUp(ho_ModelRegionROI, &ho_ModelRegionROI);
        ReduceDomain(ho_ImageModel, ho_ModelRegionROI, &ho_ImageResult);
        Intensity(ho_NewObj, ho_ImageResult, &(*hv_ModelGrayVal), &hv_Deviation);
        AreaCenter(ho_NewObj, &hv_Area, &hv_CenterRow, &hv_CenterColumn);
        // fill_up (NewObj, NewObj)
        // paint_region (NewObj, ImageResult, ImageResult, 0, 'fill')

        // step 2: create the model
        InspectShapeModel(ho_ImageResult, &ho_ShapeModelImages,
                          &ho_ShapeModelRegions, 15, 30);

        // dev_display (ShapeModelRegions)
        AreaCenter(ho_ShapeModelRegions, &hv_AreaModelRegions, &hv_RowModelRegions,
                   &hv_ColumnModelRegions);
        CountObj(ho_ShapeModelRegions, &hv_HeightPyramid);
        hv_NumLevels = 0;
        {
            HTuple end_val130 = hv_HeightPyramid;
            HTuple step_val130 = 1;
            for (hv_i = 1; hv_i.Continue(end_val130, step_val130);
                 hv_i += step_val130) {
                if (0 != (int(HTuple(hv_AreaModelRegions[hv_i - 1]) >= 15))) {
                    hv_NumLevels = hv_i;
                }
            }
        }
        ...
        hv_ScaleMax = 1.15;
        hv_MinContrast = 30;
        // create_scaled_shape_model (ImageResult, NumLevels, 0, rad(360), 'auto',
        // ScaleMin, ScaleMax, 'auto', 'auto', 'ignore_local_polarity', 70,
        // MinContrast, ModelID)
        CreateShapeModel(ho_ImageResult, hv_NumLevels, 0, HTuple(360).TupleRad(),
                         "auto", "auto", "ignore_local_polarity", 70,
                         hv_MinContrast, &(*hv_ModelID));
        // create_shape_model (ImageResult, NumLevels, 0, rad(360), 'auto',
        // 'point_reduction_high', 'use_polarity', 70, MinContrast, ModelID)
        // set_shape_model_origin (ModelID, -CenterRow, -CenterColumn)
        GetShapeModelParams((*hv_ModelID), &hv_NumLevels1, &hv_AngleStart,
                            &hv_AngleExtent, &hv_AngleStep, &hv_ScaleMin1,
                            &hv_ScaleMax1, &hv_ScaleStep, &hv_Metric,
                            &hv_MinContrast1);
        GetShapeModelContours(&ho_ModelContours, (*hv_ModelID), 1);
        SetColor(hv_WindowHandle, "green");
        DispRegion(ho_NewObj, hv_WindowHandle);

        Union1(ho_NewObj, &(*ho_ModelRegionsOrigin));
        disp_message_1(hv_WindowHandle, //'创建模版成功！ '
                       "\264\264\275\250\304\243\260\346\263\311\271\246\243\241 ",
                       "window", -1, -1, "green", "true");
        (*hv_isTrained) = 1;
    }
...
void search_shape_model_gray(HObject ho_ImageNew, HObject ho_RegionROI,
                             HObject ho_ModelRegionsOrigins,
                             HObject *ho_ObjRegions, HTuple hv_WindowHandle,
                             HTuple hv_ModelIDs, HTuple hv_ModelGrays,
                             HTuple hv_Threshold, HTuple hv_WdiffValue,
                             HTuple hv_MinScore, HTuple hv_MinDefectInnerRadian,
                             HTuple *hv_NewRow, HTuple *hv_NewColumn,
                             HTuple *hv_NewWidth, HTuple *hv_NewArea,
                             HTuple *hv_Model, HTuple *hv_Result) {

    // Local iconic variables
    HObject ho_ModelSelected, ho_RegionBorder, ho_RegionBorderVer;
    HObject ho_RegionBorderHor, ho_ImageReduced, ho_RegionFillUp;
    HObject ho_RegionThresh, ho_Region, ho_ObjRegionsShadow;
    HObject ho_ObjRegionsFind, ho_RegionDilation, ho_RegionComplement;
    HObject ho_ImageReducedObj, ho_SingleModelRegionsOrigin;
    HObject ho_RegionAffineTrans, ho_OKRegion, ho_NGRegion;
    HObject ho_IORegion, ho_RegionDefect, ho_ObjDiff, ho_ObjSelect;
    HObject ho_Rectangle, ho_ImageMedian, ho_RegionDynThresh;
    HObject ho_ConnectedRegions, ho_FillUps, ho_BlackInsides;
    HObject ho_BlackHoles, ho_BlackHolesSelect, ho_BlackHolesUnion;
    HObject ho_ImageReducedInsides, ho_BlackInsides2, ho_BlackHoles2;
    HObject ho_RegionTransCircle, ho_RegionDilation1, ho_RegionCircle;
    HObject ho_ImageCircle, ho_CircleDefectRegion, ho_ObjModel;
    HObject ho_RegionIntersection, ho_ObjModelClosing, ho_ObjModelDiff;
    HObject ho_ObjSelectClosing, ho_ImageReduced1, ho_ObjSelectDiff;
    HObject ho_SelectedRegions, ho_ObjDiffAffine, ho_ObjDiffAffineSelect;
    HObject ho_RegionDefectInverse, ho_RegionTrans, ho_RectDiff;
    HObject ho_RegionDefectInner, ho_RegionClosed, ho_RegionDefect2;
    HObject ho_RegionVisited, ho_ObjModelDilated, ho_RegionIntersect;
    HObject ho_RegionVisitedDilate, ho_OverSizeRegion;

    // Local control variables
    HTuple hv_Width, hv_Height, hv_AreaMax, hv_tmp_WdiffValue;
    HTuple hv_ADiffValue, hv_MinDefectInnerRadianScaled, hv_MinDefectArea;
    HTuple hv_min_IOU, hv_MaxShapeModelTimeOut, hv_m_detectLineC;
    HTuple hv_use_details1, hv_use_inner_DarkCircleDefect;
    HTuple hv_use_details_rectangle, hv_use_ignore_InnerDetect;
    HTuple hv_ModelAreaAll, hv_ModelWidthAll, hv_ModelHeightAll;
    HTuple hv_MinModelArea, hv_MaxModelArea, hv_MinModelWidth;
    HTuple hv_MaxModelWidth, hv_MinModelHeight, hv_MaxModelHeight;
    HTuple hv_ModelNumber, hv_Index, hv_ModelArea, hv_Row3;
    HTuple hv_Column3, hv_Row1, hv_Column1, hv_Phi, hv_ModelWidthHalf;
    HTuple hv_ModelHeightHalf, hv_ModelWidth, hv_ModelHeight;
    HTuple hv_Area, hv_CenterRow, hv_CenterColumn, hv_ROILeftR;
    HTuple hv_ROILeftC, hv_ROIRightR, hv_ROIRightC, hv_MaxToySize;
    HTuple hv_ObjNum, hv_ObjsRow, hv_ObjsColumn, hv_Phi1, hv_Length1;
    HTuple hv_Length2, hv_MaxLength2, hv_Score, hv_ScaleMin;
    HTuple hv_ScaleMax, hv_S1, hv_RowCheck, hv_ColumnCheck;
    HTuple hv_AngleCheck, hv_S2, hv_time, hv_Exception, hv_Error;
    HTuple hv_i, hv_MovementOfObject, hv_FindNum, hv_DefectNumber;
    HTuple hv_matched_no, hv_ObjRow, hv_ObjColumn, hv_ObjWidth;
    HTuple hv_ObjHeight, hv_Rect1RowL, hv_Rect1ColL, hv_Rect1RowR;
    HTuple hv_Rect1ColR, hv_ObjArea, hv_Row, hv_Column, hv_is_not_diff;
    HTuple hv_BlackThresh, hv_Deviation1, hv_Circularity, hv_j;
    HTuple hv_Indices, hv_Row4, hv_Column4, hv_ObjType1, hv_Row6;
    HTuple hv_Column6, hv_Phi2, hv_DiffLength11, hv_DiffLength21;
    HTuple hv_DiffLength1, hv_Length11, hv_Length21, hv_MovementOfObjectDiff;
    HTuple hv_ObjType2, hv_ObjDiffLength11, hv_ObjDiffLength21;
    HTuple hv_DiffLength2, hv_Rectangularity, hv_AreaObjDiff;
    HTuple hv_Row5, hv_Column5, hv_Mean, hv_Deviation, hv_ColorMatch;
    HTuple hv_Index1, hv_temp, hv_AreaIntersect, hv_AreaModel;
    HTuple hv_ratio, hv_AreaDiff, hv_Row2, hv_Column2, hv_tempModelWidth;
    HTuple hv_tempModelHeight, hv_Number, hv_DiffNumber, hv_tmp_ResultFlag;
    HTuple hv_OK_num, hv_NG_num, hv_IO_num, hv_NewRowTemp;
    HTuple hv_NewColumnTemp, hv_NewWidthTemp, hv_NewAreaTemp;
    HTuple hv_ResultTemp, hv_AreaObj, hv_tmp_Flag, hv_objRect1Width;
    HTuple hv_I;

    GetImageSize(ho_ImageNew, &hv_Width, &hv_Height);
    if (HDevWindowStack::IsOpen())
        SetPart(HDevWindowStack::GetActive(), 0, 0, hv_Height - 1, hv_Width - 1);
    hv_AreaMax = hv_Width * hv_Height;
    // 长宽尺寸误差容许值
    hv_tmp_WdiffValue = (hv_WdiffValue * hv_Width) / 320;
    hv_ADiffValue = 0.05;
    // 物料匹配限制参数
    // MinScore := 0.8
    // 最小缺陷内径
    // MinDefectInnerRadianScaled := MinDefectInnerRadian *(Width*
    // Height)/(600*800)
    hv_MinDefectInnerRadianScaled = hv_MinDefectInnerRadian;
    // 最小缺陷面积
    hv_MinDefectArea = (30 * (hv_Width * hv_Height)) / (600 * 800);
    // 最小重叠区域占比
    hv_min_IOU = 0.7;
    // 最大查找轮廓超时限制
    hv_MaxShapeModelTimeOut = -1;
    // MaxShapeModelTimeOut := 10
    // 跟踪线   50 *Width/320
    hv_m_detectLineC = 10;
    ...
    hv_use_details1 = 0;
    // 是否启用检测黑料圆圈内部缺胶
    hv_use_inner_DarkCircleDefect = 0;
    // 是否采用矩形检测缺角
    hv_use_details_rectangle = 0;
    // 内轮廓比较开关
    hv_use_ignore_InnerDetect = 1;
    ...
    hv_ModelAreaAll = HTuple();
    hv_ModelWidthAll = HTuple();
    hv_ModelHeightAll = HTuple();

    // 提取长宽尺寸
    hv_MinModelArea = 99999;
    hv_MaxModelArea = 0;
    hv_MinModelWidth = 99999;
    hv_MaxModelWidth = 0;
    hv_MinModelHeight = 99999;
    hv_MaxModelHeight = 0;
    CountObj(ho_ModelRegionsOrigins, &hv_ModelNumber);
    {
        HTuple end_val57 = hv_ModelNumber;
        HTuple step_val57 = 1;
        for (hv_Index = 1; hv_Index.Continue(end_val57, step_val57);
             hv_Index += step_val57) {
            SelectObj(ho_ModelRegionsOrigins, &ho_ModelSelected, hv_Index);
            AreaCenter(ho_ModelSelected, &hv_ModelArea, &hv_Row3, &hv_Column3);
            hv_ModelAreaAll = hv_ModelAreaAll.TupleConcat(hv_ModelArea);
            if (0 != (int(hv_ModelArea < hv_MinModelArea))) {
                hv_MinModelArea = hv_ModelArea;
            }
            if (0 != (int(hv_ModelArea > hv_MaxModelArea))) {
                hv_MaxModelArea = hv_ModelArea;
            }

            SmallestRectangle2(ho_ModelSelected, &hv_Row1, &hv_Column1, &hv_Phi,
                               &hv_ModelWidthHalf, &hv_ModelHeightHalf);
            hv_ModelWidth = hv_ModelWidthHalf * 2;
            hv_ModelHeight = hv_ModelHeightHalf * 2;
            hv_ModelWidthAll = hv_ModelWidthAll.TupleConcat(hv_ModelWidth);
            hv_ModelHeightAll = hv_ModelHeightAll.TupleConcat(hv_ModelHeight);

            if (0 != (int(hv_ModelWidth < hv_MinModelWidth))) {
                hv_MinModelWidth = hv_ModelWidth;
            }
            if (0 != (int(hv_ModelWidth > hv_MaxModelWidth))) {
                hv_MaxModelWidth = hv_ModelWidth;
            }

            if (0 != (int(hv_ModelHeight < hv_MinModelHeight))) {
                hv_MinModelHeight = hv_ModelHeight;
            }
            if (0 != (int(hv_ModelHeight > hv_MaxModelHeight))) {
                hv_MaxModelHeight = hv_ModelHeight;
            }
        }
    }
    ...
    Boundary(ho_RegionROI, &ho_RegionBorder, "outer");
    // dilation_circle (RegionBorder, RegionBorder, 2)
    ErosionRectangle1(ho_RegionBorder, &ho_RegionBorderVer, 1, 11);
    ErosionRectangle1(ho_RegionBorder, &ho_RegionBorderHor, 11, 1);
    ReduceDomain(ho_ImageNew, ho_RegionROI, &ho_ImageReduced);
    GenEmptyRegion(&ho_RegionFillUp);
    // 增加边缘提取法(针对半透明物料）
    // edges_image (ImageReduced, ImaAmp, ImaDir, 'sobel_fast', 1.0, 'nms', 15,
    // 40) threshold (ImaAmp, Margin, 10, 255) dilation_circle (Margin,
    // RegionClosing, 2) union2 (RegionClosing, RegionBorderVer, RegionClosingVer)
    // union2 (RegionClosing, RegionBorderHor, RegionClosingHor)
    // union2 (RegionClosingVer, RegionBorderHor, RegionClosing)
    // fill_up (RegionClosing, RegionFillUp)
    // 将阈值分割结果与轮廓填充结果合并
    Threshold(ho_ImageReduced, &ho_RegionThresh, 0, hv_Threshold);
    Union2(ho_RegionFillUp, ho_RegionThresh, &ho_RegionFillUp);
    // 去除边框
    Difference(ho_RegionFillUp, ho_RegionBorder, &ho_RegionFillUp);
    // 剔除中间纯白区域
    Threshold(ho_ImageReduced, &ho_Region, 0, hv_Threshold);
    Intersection(ho_RegionROI, ho_Region, &(*ho_ObjRegions));
    Intersection((*ho_ObjRegions), ho_RegionFillUp, &(*ho_ObjRegions));
    Connection((*ho_ObjRegions), &(*ho_ObjRegions));
    SelectShape((*ho_ObjRegions), &(*ho_ObjRegions),
                (HTuple("area").Append("height")), "and",
                hv_MinDefectArea.TupleConcat(0),
                hv_AreaMax.TupleConcat(hv_MaxToySize));
    // 合并倒影
    Union1((*ho_ObjRegions), &ho_ObjRegionsShadow);
    DilationCircle(ho_ObjRegionsShadow, &ho_ObjRegionsShadow, 2);
    ClosingRectangle1(ho_ObjRegionsShadow, &ho_ObjRegionsShadow, 10, 1);
    OpeningRectangle1(ho_ObjRegionsShadow, &ho_ObjRegionsShadow, 1, 2);
    FillUp(ho_ObjRegionsShadow, &ho_ObjRegionsShadow);
    Connection(ho_ObjRegionsShadow, &ho_ObjRegionsShadow);
    Intersection(ho_ObjRegionsShadow, (*ho_ObjRegions), &(*ho_ObjRegions));
    // 检测出物料后，根据配置决定是否填充内轮廓，true-填充，忽略内轮廓
    if (0 != hv_use_ignore_InnerDetect) {
        FillUp((*ho_ObjRegions), &(*ho_ObjRegions));
    }

    // 分割粘黏物料
    // distance_transform (ObjRegionsShadow, DistanceImage, 'octagonal', 'true',
    // Width, Height) convert_image_type (DistanceImage, DistanceImageByte,
    // 'byte') invert_image (DistanceImageByte, DistanceImageInv) scale_image_max
    // (DistanceImageInv, DistanceImageInvScaled) watersheds_threshold
    // (DistanceImageInvScaled, Basins, 100) intersection (Basins,
    // ObjRegionsShadow, ObjRegionsShadow2) opening_rectangle1 (ObjRegionsShadow,
    // ObjRegionsShadow, 1, 2) connection (ObjRegionsShadow2, ObjRegionsShadow)
    // intersection (ObjRegionsShadow, ObjRegions, ObjRegions)

    CountObj((*ho_ObjRegions), &hv_ObjNum);
    GenEmptyObj(&ho_ObjRegionsFind);
    if (0 != (int(hv_ObjNum > 0))) {
        SmallestRectangle2((*ho_ObjRegions), &hv_ObjsRow, &hv_ObjsColumn, &hv_Phi1,
                           &hv_Length1, &hv_Length2);
        TupleMax(hv_Length2, &hv_MaxLength2);
        TupleMax2(hv_MaxLength2, 10, &hv_MaxLength2);
    } else {
        hv_MaxLength2 = 10;
    }
    // 至少5个像素膨胀
    DilationCircle((*ho_ObjRegions), &ho_RegionDilation, hv_MaxLength2 / 1);
    ShapeTrans(ho_RegionDilation, &ho_RegionDilation, "convex");
    Union1(ho_RegionDilation, &ho_RegionDilation);
    Complement(ho_RegionDilation, &ho_RegionComplement);
    // paint_region (RegionComplement, ImageReduced, ImageReduced2, 0, 'fill')
    ReduceDomain(ho_ImageReduced, ho_RegionDilation, &ho_ImageReducedObj);
    hv_Score = HTuple();
    if (0 != (HTuple(int((hv_ModelIDs.TupleLength()) > 0))
              .TupleAnd(int(hv_use_inner_DarkCircleDefect == 0)))) {
        ...
        try {
            ...
            // 如果模版有缩放，则激活该句代码
            // find_scaled_shape_models (ImageReducedObj, ModelIDs, -rad(180),
            // rad(180), ScaleMin, ScaleMax, 0.5, 0, 0.2, 'least_squares', 3, 0.9,
            // RowCheck, ColumnCheck, AngleCheck, Scale, Score, Model)
            // find_scaled_shape_models (ImageReducedObj, ModelIDs, 0, rad(360),
            // ScaleMin, ScaleMax, MinScore, 0, 0.2, 'least_squares', 9, 0.9,
            // RowCheck, ColumnCheck, AngleCheck, Scale, Score, Model)
            FindShapeModels(ho_ImageReducedObj, hv_ModelIDs, 0,
                            HTuple(360).TupleRad(), 0.7, 0, 0.2, "least_squares", 9,
                            0.9, &hv_RowCheck, &hv_ColumnCheck, &hv_AngleCheck,
                            &hv_Score, &(*hv_Model));
            CountSeconds(&hv_S2);
            hv_time = (hv_S2 - hv_S1) * 1000.0;
            // The timeout error code is 9400.
            // disp_message (WindowHandle, 'match time: ' + time$'.4', 'window', 60,
            // -1, 'lime green', 'true')
        }
        // catch (Exception)
        catch (HException &HDevExpDefaultException) {
            ...
            CountSeconds(&hv_S2);
            hv_time = (hv_S2 - hv_S1) * 1000.0;
            HException::GetExceptionData(hv_Exception, "error_code", &hv_Error);
            if (0 != (int(hv_Error == 9400))) {
                disp_message_1(
                            hv_WindowHandle,
                            ("Timeout occurred after " + (hv_time.TupleString(".4"))) + " ms",
                            "window", 100, -1, "red", "false");
            } else {
                throw HException(hv_Exception);
            }
        }
        ...
        if (0 != (int((hv_Score.TupleLength()) > 0))) {
            {
                HTuple end_val190 = (hv_Score.TupleLength()) - 1;
                HTuple step_val190 = 1;
                for (hv_i = 0; hv_i.Continue(end_val190, step_val190);
                     hv_i += step_val190) {
                    // step 3: determine the affine transformation
                    SelectObj(ho_ModelRegionsOrigins, &ho_SingleModelRegionsOrigin,
                              HTuple((*hv_Model)[hv_i]) + 1);
                    // vector_angle_to_rigid (Height/2, Width/2, 0, RowCheck[i],
                    // ColumnCheck[i], AngleCheck[i], MovementOfObject)
                    VectorAngleToRigid(hv_Height / 2, hv_Width / 2, 0,
                                       HTuple(hv_RowCheck[hv_i]),
                                       HTuple(hv_ColumnCheck[hv_i]),
                                       HTuple(hv_AngleCheck[hv_i]), &hv_MovementOfObject);
                    // 如果模版有缩放，则激活该句代码
                    // hom_mat2d_scale (MovementOfObject, Scale[i], Scale[i], RowCheck[i],
                    // ColumnCheck[i], MovementOfObject)
                    AffineTransRegion(ho_SingleModelRegionsOrigin, &ho_RegionAffineTrans,
                                      hv_MovementOfObject, "false");
                    ConcatObj(ho_ObjRegionsFind, ho_RegionAffineTrans,
                              &ho_ObjRegionsFind);
                    // disp_message (WindowHandle, 'Score: ' + Score[i]$'.3' + '   Scale:
                    // ' + Scale[i]$'.3', 'window', RowCheck[i], ColumnCheck[i], 'red',
                    // 'false')
                }
            }
        } else if (0 != (int(hv_ObjNum > 0))) {
            disp_message_1(hv_WindowHandle,
                           ("match no object: " + (hv_time.TupleString(".4"))) +
                           " ms",
                           "window", 60, -1, "red", "false");
            // wait_seconds (0.5)
        }
    }

    CountObj(ho_ObjRegionsFind, &hv_FindNum);
    GenEmptyObj(&ho_OKRegion);
    GenEmptyObj(&ho_NGRegion);
    GenEmptyObj(&ho_IORegion);
    GenEmptyRegion(&ho_RegionDefect);
    hv_DefectNumber = 0;
    hv_matched_no = HTuple();
    {
        HTuple end_val217 = hv_ObjNum;
        HTuple step_val217 = 1;
        for (hv_i = 1; hv_i.Continue(end_val217, step_val217);
             hv_i += step_val217) {
            GenEmptyObj(&ho_ObjDiff);
            SelectObj((*ho_ObjRegions), &ho_ObjSelect, hv_i);
            SmallestRectangle2(ho_ObjSelect, &hv_ObjRow, &hv_ObjColumn, &hv_Phi,
                               &hv_ObjWidth, &hv_ObjHeight);
            GenRectangle2(&ho_Rectangle, hv_ObjRow, hv_ObjColumn, hv_Phi, hv_ObjWidth,
                          hv_ObjHeight);
            //   *disp_message(WindowHandle, ObjWidth*2*0.105+'mm * '
            //   +ObjHeight*2*0.105+'mm', 'window', 60, -1,  'red', 'false')
            SmallestRectangle1(ho_ObjSelect, &hv_Rect1RowL, &hv_Rect1ColL,
                               &hv_Rect1RowR, &hv_Rect1ColR);
            AreaCenter(ho_ObjSelect, &hv_ObjArea, &hv_Row, &hv_Column);
            if (0 != (HTuple(int(hv_Rect1ColL < (hv_ROILeftC + hv_m_detectLineC)))
                      .TupleOr(int(hv_Rect1ColR > (hv_ROIRightC - 5))))) {
                ConcatObj(ho_IORegion, ho_ObjSelect, &ho_IORegion);
                continue;
            }
            hv_is_not_diff = 1;

            if (0 != (int(hv_use_inner_DarkCircleDefect == 1))) {
                MedianImage(ho_ImageReducedObj, &ho_ImageMedian, "circle", 30,
                            "mirrored");
                DynThreshold(ho_ImageReducedObj, ho_ImageMedian, &ho_RegionDynThresh,
                             20, "light");
                Connection(ho_RegionDynThresh, &ho_ConnectedRegions);
                FillUp(ho_ConnectedRegions, &ho_FillUps);
                Difference(ho_FillUps, ho_ConnectedRegions, &ho_BlackInsides);
                // union1 (BlackInsides, BlackInsides)
                Connection(ho_BlackInsides, &ho_BlackHoles);
                FillUp(ho_BlackHoles, &ho_BlackHoles);
                SelectShape(ho_BlackHoles, &ho_BlackHolesSelect,
                            (HTuple("area").Append("circularity")), "and",
                            (hv_MinDefectArea * 3).TupleConcat(0.7),
                            hv_AreaMax.TupleConcat(1));
                // 利用已获得一个圆的亮度，对另一个圆做分割
                Union1(ho_BlackHolesSelect, &ho_BlackHolesUnion);
                Intensity(ho_BlackHolesUnion, ho_ImageReducedObj, &hv_BlackThresh,
                          &hv_Deviation1);
                ReduceDomain(ho_ImageNew, ho_BlackHolesUnion, &ho_ImageReducedInsides);
                Threshold(ho_ImageReducedInsides, &ho_BlackInsides2, 0,
                          hv_BlackThresh + (hv_Deviation1 * 2));
                Connection(ho_BlackInsides2, &ho_BlackHoles2);
                FillUp(ho_BlackHoles2, &ho_BlackHoles2);
                SelectShape(ho_BlackHoles2, &ho_BlackHoles2,
                            (HTuple("area").Append("circularity")), "and",
                            (hv_MinDefectArea * 3).TupleConcat(0.8),
                            hv_AreaMax.TupleConcat(1));
                Circularity(ho_BlackHoles2, &hv_Circularity);
                ShapeTrans(ho_BlackHoles2, &ho_RegionTransCircle, "outer_circle");
                DilationCircle(ho_RegionTransCircle, &ho_RegionDilation1, 15);
                Difference(ho_RegionDilation1, ho_RegionTransCircle, &ho_RegionCircle);
                Union1(ho_RegionCircle, &ho_RegionCircle);
                ReduceDomain(ho_ImageReducedObj, ho_RegionCircle, &ho_ImageCircle);
                Threshold(ho_ImageCircle, &ho_CircleDefectRegion, hv_Threshold, 255);
                Connection(ho_CircleDefectRegion, &ho_CircleDefectRegion);
                SelectShape(ho_CircleDefectRegion, &ho_ObjDiff,
                            (HTuple("area").Append("inner_radius")), "and",
                            hv_MinDefectArea.TupleConcat(hv_MinDefectInnerRadianScaled),
                            hv_AreaMax.TupleConcat(hv_Height));
                Union1(ho_ObjDiff, &ho_RegionDefect);
                CountObj(ho_RegionDefect, &hv_DefectNumber);
                if (0 != (int(hv_DefectNumber > 0))) {
                    ConcatObj(ho_NGRegion, ho_ObjSelect, &ho_NGRegion);
                } else {
                    ConcatObj(ho_OKRegion, ho_ObjSelect, &ho_OKRegion);
                }
                // dev_set_color ('red')
                // dev_display (RegionDefect)
                continue;
            }
            if (0 != (int(hv_FindNum > 0))) {
                {
                    HTuple end_val271 = hv_FindNum;
                    HTuple step_val271 = 1;
                    for (hv_j = 1; hv_j.Continue(end_val271, step_val271);
                         hv_j += step_val271) {
                        // 匹配过的不再对比
                        TupleFind(hv_matched_no, hv_j, &hv_Indices);
                        if (0 != (int(hv_Indices < 0))) {
                            SelectObj(ho_ObjRegionsFind, &ho_ObjModel, hv_j);
                            Intersection(ho_ObjModel, ho_ObjSelect, &ho_RegionIntersection);
                            AreaCenter(ho_RegionIntersection, &hv_Area, &hv_Row4,
                                       &hv_Column4);
                            if (0 != (int((hv_Area.TupleSum()) > 0))) {

                                if (0 != hv_use_details1) {
                                    ClosingCircle(ho_ObjModel, &ho_ObjModelClosing, 5);
                                    Difference(ho_ObjModelClosing, ho_ObjModel, &ho_ObjModelDiff);
                                    ClosingRectangle1(ho_ObjModelDiff, &ho_ObjModelDiff, 5, 1);
                                    Connection(ho_ObjModelDiff, &ho_ObjModelDiff);
                                    SelectShape(ho_ObjModelDiff, &ho_ObjModelDiff,
                                                (HTuple("area").Append("width")), "and",
                                                HTuple(50).TupleConcat(hv_ObjWidth * 0.5),
                                                hv_AreaMax.TupleConcat(hv_ObjWidth * 1.3));
                                    Union1(ho_ObjModelDiff, &ho_ObjModelDiff);
                                    CountObj(ho_ObjModelDiff, &hv_ObjType1);
                                    if (0 != (int(hv_ObjType1 > 0))) {
                                        SmallestRectangle2(ho_ObjModelDiff, &hv_Row6, &hv_Column6,
                                                           &hv_Phi2, &hv_DiffLength11,
                                                           &hv_DiffLength21);
                                        TupleMedian(hv_DiffLength11, &hv_DiffLength1);
                                        // disp_rectangle2 (3600, Row6, Column6, Phi2, DiffLength11,
                                        // DiffLength21)
                                    }

                                    ClosingCircle(ho_ObjSelect, &ho_ObjSelectClosing, 5);
                                    ErosionCircle(ho_ObjSelectClosing, &ho_ObjSelectClosing, 5);
                                    ReduceDomain(ho_ImageReducedObj, ho_ObjSelectClosing,
                                                 &ho_ImageReduced1);
                                    Threshold(ho_ImageReduced1, &ho_ObjSelectDiff, hv_Threshold,
                                              255);
                                    Connection(ho_ObjSelectDiff, &ho_SelectedRegions);
                                    SelectShapeStd(ho_SelectedRegions, &ho_SelectedRegions,
                                                   "max_area", 70);
                                    SmallestRectangle2(ho_SelectedRegions, &hv_Row6, &hv_Column6,
                                                       &hv_Phi2, &hv_Length11, &hv_Length21);
                                    // disp_rectangle2 (3600, Row6, Column6, Phi2, Length11,
                                    // Length21) 转到水平方向做闭运算
                                    VectorAngleToRigid(hv_Row6, hv_Column6, hv_Phi2, hv_Row6,
                                                       hv_Column6, 0, &hv_MovementOfObjectDiff);
                                    AffineTransRegion(ho_ObjSelectDiff, &ho_ObjSelectDiff,
                                                      hv_MovementOfObjectDiff, "false");
                                    ClosingRectangle1(ho_ObjSelectDiff, &ho_ObjDiffAffine, 8, 1);
                                    Connection(ho_ObjDiffAffine, &ho_ObjDiffAffine);
                                    SelectShape(ho_ObjDiffAffine, &ho_ObjDiffAffineSelect,
                                                (HTuple("area").Append("width")), "and",
                                                HTuple(40).TupleConcat(hv_ObjWidth * 0.2),
                                                hv_AreaMax.TupleConcat(hv_ObjWidth * 1.3));
                                    Union1(ho_ObjDiffAffineSelect, &ho_ObjDiffAffineSelect);
                                    CountObj(ho_ObjDiffAffineSelect, &hv_ObjType2);
                                    if (0 != (int(hv_ObjType2 > 0))) {
                                        SmallestRectangle2(
                                                    ho_ObjDiffAffineSelect, &hv_Row6, &hv_Column6, &hv_Phi2,
                                                    &hv_ObjDiffLength11, &hv_ObjDiffLength21);
                                        TupleMedian(hv_ObjDiffLength11, &hv_DiffLength2);

                                        //*
                                        //if(abs(max2(0,int((ObjType1-1)/2))-max2(0,int((ObjType2-1)/2)))>0)
                                        // 如果两者的宽度差，大于最小宽度的一半，说明两者宽度不同
                                        if (0 !=
                                                (int(((hv_DiffLength1 - hv_DiffLength2).TupleAbs()) >
                                                     ((hv_DiffLength1.TupleMin2(hv_DiffLength2)) *
                                                      0.5)))) {
                                            ConcatObj(ho_NGRegion, ho_ObjSelect, &ho_NGRegion);
                                        } else {
                                            ConcatObj(ho_OKRegion, ho_ObjSelect, &ho_OKRegion);
                                ...
                                Difference(ho_ObjModel, ho_ObjSelect, &ho_RegionDefect);
                                Difference(ho_ObjSelect, ho_ObjModel, &ho_RegionDefectInverse);
                                Union2(ho_RegionDefect, ho_RegionDefectInverse,
                                       &ho_RegionDefect);
                                if (0 != hv_use_details_rectangle) {
                                    // 如果是矩形，提高边角检测精度
                                    Rectangularity(ho_RegionIntersection, &hv_Rectangularity);
                                    if (0 != (int(hv_Rectangularity > 0.92))) {
                                        ShapeTrans(ho_RegionIntersection, &ho_RegionTrans,
                                                   "rectangle2");
                                        Difference(ho_RegionTrans, ho_RegionIntersection,
                                                   &ho_RectDiff);
                                        Union2(ho_RegionDefect, ho_RectDiff, &ho_RegionDefect);
                                    }
                                }
                                OpeningCircle(ho_RegionDefect, &ho_RegionDefect, 1);
                                Connection(ho_RegionDefect, &ho_RegionDefect);
                                GenEmptyRegion(&ho_RegionDefectInner);
                                // 检测内部缺陷
                                ClosingCircle(ho_ObjSelect, &ho_RegionClosed, 20);
                                Intersection(ho_RegionDefect, ho_RegionClosed,
                                             &ho_RegionDefectInner);
                                SelectShape(ho_RegionDefectInner, &ho_RegionDefectInner,
                                            ((HTuple("area").Append("inner_radius"))
                                             .Append("convexity")),
                                            "and",
                                            ((hv_MinDefectArea * 3)
                                             .TupleConcat(hv_MinDefectInnerRadianScaled))
                                            .TupleConcat(0.1),
                                            (hv_AreaMax.TupleConcat(hv_Height)).TupleConcat(1));
                                SelectShape(ho_RegionDefect, &ho_RegionDefect2,
                                            ((HTuple("area").Append("inner_radius"))
                                             .Append("convexity")),
                                            "and",
                                            (hv_MinDefectArea.TupleConcat(
                                                 hv_MinDefectInnerRadianScaled))
                                            .TupleConcat(0.1),
                                            (hv_AreaMax.TupleConcat(hv_Height)).TupleConcat(1));
                                Union2(ho_RegionDefect2, ho_RegionDefectInner,
                                       &ho_RegionDefect);
                                CountObj(ho_RegionDefect, &hv_DefectNumber);

                                if (0 != (int(hv_DefectNumber > 0))) {
                                    // 缺胶
                                    Union2(ho_OKRegion, ho_NGRegion, &ho_RegionVisited);
                                    Union2(ho_RegionVisited, ho_IORegion, &ho_RegionVisited);
                                    Difference(ho_ObjSelect, ho_RegionVisited, &ho_ObjDiff);
                                    AreaCenter(ho_ObjDiff, &hv_AreaObjDiff, &hv_Row5,
                                               &hv_Column5);
                                    if (0 != (int(hv_AreaObjDiff > hv_MinDefectArea))) {
                                        ConcatObj(ho_NGRegion, ho_ObjDiff, &ho_NGRegion);
                                    }
                                } else {
                                    Intensity(ho_ObjSelect, ho_ImageReducedObj, &hv_Mean,
                                              &hv_Deviation);
                                    if (0 != (int((hv_ModelGrays.TupleLength()) > 0))) {
                                        hv_ColorMatch = 0;
                                        {
                                            HTuple end_val362 = (hv_ModelGrays.TupleLength()) - 1;
                                            HTuple step_val362 = 1;
                                            for (hv_Index1 = 0;
                                                 hv_Index1.Continue(end_val362, step_val362);
                                                 hv_Index1 += step_val362) {
                                                hv_temp = HTuple(hv_ModelGrays[hv_Index1]);
                                                if (0 !=
                                                        (int(((hv_Mean - hv_temp).TupleAbs()) < 110))) {
                                                    hv_ColorMatch = 1;
                                                    break;
                                                }
                                            }
                                        }
                                        if (0 != (int(hv_ColorMatch == 0))) {
                                            //* 任何一个亮度都没匹配上，就按杂料处理
                                            ConcatObj(ho_NGRegion, ho_ObjSelect, &ho_NGRegion);
                                            continue;
                                        }
                                    }
                                    AreaCenter(ho_RegionIntersection, &hv_AreaIntersect, &hv_Row1,
                                               &hv_Column1);
                                    AreaCenter(ho_ObjModel, &hv_AreaModel, &hv_ObjRow,
                                               &hv_ObjColumn);
                                    TupleMax2(hv_AreaModel * (1 - hv_min_IOU), 150, &hv_ratio);
                                    DilationRectangle1(ho_ObjModel, &ho_ObjModelDilated, 15, 15);
                                    Difference(ho_ObjSelect, ho_ObjModelDilated, &ho_ObjDiff);
                                    // 需要删除已经访问过的区域
                                    Union2(ho_OKRegion, ho_NGRegion, &ho_RegionVisited);
                                    Union2(ho_RegionVisited, ho_IORegion, &ho_RegionVisited);
                                    Difference(ho_ObjDiff, ho_RegionVisited, &ho_ObjDiff);
                                    AreaCenter(ho_ObjDiff, &hv_AreaDiff, &hv_Row2, &hv_Column2);

                                    if (0 != (HTuple(int(((hv_AreaIntersect - hv_AreaModel)
                                                          .TupleAbs()) < hv_ratio))
                                              .TupleOr(int(hv_AreaDiff < 200)))) {
                                        // 重合度大于0.9认为是合格料。
                                        hv_matched_no = hv_matched_no.TupleConcat(hv_j);
                                        if (0 != (int(((hv_ObjArea - hv_AreaModel).TupleAbs()) <
                                                      (hv_MinModelArea * 0.7)))) {
                                            // 面积差异控制
                                            hv_tempModelWidth = HTuple(
                                                        hv_ModelWidthAll[HTuple((*hv_Model)[hv_j - 1])]);
                                            hv_tempModelHeight = HTuple(
                                                        hv_ModelHeightAll[HTuple((*hv_Model)[hv_j - 1])]);
                                            // tempModelWidth := ModelWidthAll[Model[j-1]]*Scale[j-1]
                                            // tempModelHeight :=
                                            // ModelHeightAll[Model[j-1]]*Scale[j-1]
                                            if (0 !=
                                                    (HTuple(HTuple(int(((hv_ObjArea - hv_AreaModel)
                                                                        .TupleAbs()) <
                                                                       (hv_AreaModel * 0.35)))
                                                            .TupleAnd(int((((hv_ObjWidth * 2) -
                                                                            hv_tempModelWidth)
                                                                           .TupleAbs()) <
                                                                          (hv_tmp_WdiffValue * 3.3))))
                                                     .TupleAnd(int(
                                                                   (((hv_ObjHeight * 2) - hv_tempModelHeight)
                                                                    .TupleAbs()) <
                                                                   (hv_tmp_WdiffValue * 3))))) {
                                                ConcatObj(ho_OKRegion, ho_ObjSelect, &ho_OKRegion);
                                            } else {
                                            ...
                                            if (0 != (int(hv_AreaDiff < 200))) {
                                                hv_is_not_diff = 0;
                                                break;
                                            }
                                        } else {
                                            // 存在翻滚的情况
                                            ConcatObj(ho_OKRegion, ho_ObjModel, &ho_OKRegion);
                                        }
                                    }
                                }
                            }
            ...
            if (0 != hv_is_not_diff) {
                Union2(ho_OKRegion, ho_NGRegion, &ho_RegionVisited);
                Union2(ho_RegionVisited, ho_IORegion, &ho_RegionVisited);
                Intersection(ho_ObjSelect, ho_RegionVisited, &ho_RegionIntersect);
                DilationCircle(ho_RegionVisited, &ho_RegionVisitedDilate, 2);
                Difference(ho_ObjSelect, ho_RegionVisitedDilate, &ho_ObjDiff);
                CountObj(ho_RegionIntersect, &hv_Number);
                if (0 != (int(hv_Number > 0))) {
                    ClosingRectangle1(ho_ObjDiff, &ho_ObjDiff, 10, 1);
                    OpeningCircle(ho_ObjDiff, &ho_ObjDiff, 2);
                }
                Connection(ho_ObjDiff, &ho_ObjDiff);
                Intersection(ho_ObjRegionsShadow, ho_ObjDiff, &ho_ObjDiff);
            }
            SelectShape(
                        ho_ObjDiff, &ho_ObjDiff, (HTuple("area").Append("inner_radius")),
                        "and",
                        hv_MinDefectArea.TupleConcat(hv_MinDefectInnerRadianScaled / 2),
                        hv_AreaMax.TupleConcat(hv_Height));
            CountObj(ho_ObjDiff, &hv_DiffNumber);
            if (0 != (int(hv_DiffNumber > 0))) {
                SmallestRectangle1(ho_ObjDiff, &hv_Rect1RowL, &hv_Rect1ColL,
                                   &hv_Rect1RowR, &hv_Rect1ColR);
                if (0 != (HTuple(int(hv_Rect1ColL < (hv_ROILeftC + hv_m_detectLineC)))
                          .TupleOr(int(hv_Rect1ColR > (hv_ROIRightC - 5))))) {
                    ConcatObj(ho_IORegion, ho_ObjDiff, &ho_IORegion);
                } else {
                    hv_tmp_ResultFlag = 1;
                    // 是否启用尺寸比对，如果尺寸比对通过也视为合格料
                    // smallest_rectangle2 (ObjDiff, ObjRow, ObjColumn, Phi, ObjWidth,
                    // ObjHeight) for j := 1 to ModelNumber by 1
                    // 如果物料的长宽其中任意一项超过模版标准尺寸±10%直接视为杂料
                    // if ((abs(ObjWidth*2 - ModelWidthAll[j-1]) < tmp_WdiffValue) and
                    // (abs(ObjHeight*2 - ModelHeightAll[j-1]) < tmp_WdiffValue))
                    // area_center (ObjDiff, AreaDiff, Row2, Column2)
                    // tempArea := ModelAreaAll[j-1] + ((ObjHeight*2 - ModelHeightAll[j-1]
                    // - 1) / 2 * ModelWidthAll[j-1])	+ ((ObjWidth*2 -
                    // ModelWidthAll[j-1] - 1) / 2 * ModelHeightAll[j-1]) if (abs(AreaDiff
                    // - tempArea) < tempArea * ADiffValue) tmp_ResultFlag := 0 concat_obj
                    // (OKRegion, ObjDiff, OKRegion) 匹配上则调出循环 break endif endif
                    // endfor

                    if (0 != (int(hv_tmp_ResultFlag > 0))) {
                        ConcatObj(ho_NGRegion, ho_ObjDiff, &ho_NGRegion);
                    }
                }
            }
    ...
    CountObj(ho_OKRegion, &hv_OK_num);
    CountObj(ho_NGRegion, &hv_NG_num);
    CountObj(ho_IORegion, &hv_IO_num);
    hv_NewRowTemp = HTuple();
    hv_NewColumnTemp = HTuple();
    hv_NewWidthTemp = HTuple();
    hv_NewAreaTemp = HTuple();
    // 0：OK, 1：NG
    hv_ResultTemp = HTuple();

    GenEmptyRegion(&ho_OverSizeRegion);
    if (HDevWindowStack::IsOpen())
        ClearWindow(HDevWindowStack::GetActive());
    DispImage(ho_ImageNew, hv_WindowHandle);

    // 统计合格的料
    {
        HTuple end_val478 = hv_OK_num;
        HTuple step_val478 = 1;
        for (hv_i = 1; hv_i.Continue(end_val478, step_val478);
             hv_i += step_val478) {
            SelectObj(ho_OKRegion, &ho_ObjSelect, hv_i);
            SmallestRectangle2(ho_ObjSelect, &hv_ObjRow, &hv_ObjColumn, &hv_Phi,
                               &hv_ObjWidth, &hv_ObjHeight);
            // disp_rectangle2 (WindowHandle, ObjRow, ObjColumn, Phi, ObjWidth,
            // ObjHeight)
            SmallestRectangle1(ho_ObjSelect, &hv_Rect1RowL, &hv_Rect1ColL,
                               &hv_Rect1RowR, &hv_Rect1ColR);
            AreaCenter(ho_ObjSelect, &hv_AreaObj, &hv_ObjRow, &hv_ObjColumn);
            // 使用物料最右端坐标作为输出
            hv_tmp_Flag = 0;
            hv_ObjRow = hv_Rect1RowL + ((hv_Rect1RowR - hv_Rect1RowL) / 2);
            hv_objRect1Width = hv_Rect1ColR - hv_Rect1ColL;
            DispCircle(hv_WindowHandle, hv_ObjRow, hv_Rect1ColR, 6);
            if (0 != (HTuple(int(hv_Rect1ColL < (hv_ROILeftC + hv_m_detectLineC)))
                      .TupleOr(int(hv_Rect1ColR > (hv_ROIRightC - 5))))) {
                hv_tmp_Flag = 2;
            }
            hv_NewRowTemp = hv_NewRowTemp.TupleConcat(hv_ObjRow);
            hv_NewColumnTemp = hv_NewColumnTemp.TupleConcat(hv_Rect1ColR);
            hv_NewWidthTemp = hv_NewWidthTemp.TupleConcat(hv_objRect1Width);
            hv_NewAreaTemp = hv_NewAreaTemp.TupleConcat(hv_AreaObj);
            hv_ResultTemp = hv_ResultTemp.TupleConcat(hv_tmp_Flag);
        }
    }

    // 统计杂料
    {
        HTuple end_val500 = hv_NG_num;
        HTuple step_val500 = 1;
        for (hv_i = 1; hv_i.Continue(end_val500, step_val500);
             hv_i += step_val500) {
            SelectObj(ho_NGRegion, &ho_ObjSelect, hv_i);
            SmallestRectangle2(ho_ObjSelect, &hv_ObjRow, &hv_ObjColumn, &hv_Phi,
                               &hv_ObjWidth, &hv_ObjHeight);
            SmallestRectangle1(ho_ObjSelect, &hv_Rect1RowL, &hv_Rect1ColL,
                               &hv_Rect1RowR, &hv_Rect1ColR);
            ...
            if (0 !=
                    (HTuple(int((hv_ObjWidth * 2) > (hv_MaxModelWidth + 12)))
                     .TupleOr(int((hv_ObjHeight * 2) > (hv_MaxModelHeight + 12))))) {
                ConcatObj(ho_OverSizeRegion, ho_ObjSelect, &ho_OverSizeRegion);
                hv_tmp_Flag = 3;
            }
    ...
    // 统计进出视野的料
    {
        HTuple end_val524 = hv_IO_num;
        HTuple step_val524 = 1;
        for (hv_i = 1; hv_i.Continue(end_val524, step_val524);
             hv_i += step_val524) {
            SelectObj(ho_IORegion, &ho_ObjSelect, hv_i);
            SmallestRectangle2(ho_ObjSelect, &hv_ObjRow, &hv_ObjColumn, &hv_Phi,
                               &hv_ObjWidth, &hv_ObjHeight);
            SmallestRectangle1(ho_ObjSelect, &hv_Rect1RowL, &hv_Rect1ColL,
                               &hv_Rect1RowR, &hv_Rect1ColR);
            ...
            hv_ObjRow = hv_Rect1RowL + ((hv_Rect1RowR - hv_Rect1RowL) / 2);
            hv_objRect1Width = hv_Rect1ColR - hv_Rect1ColL;
            // disp_circle (WindowHandle, ObjRow, Rect1ColR, 6)

            if (0 != (HTuple(HTuple(HTuple(int(hv_Rect1ColL >= (hv_ROILeftC + 12)))
                                    .TupleAnd(int(hv_MaxModelWidth > 0)))
                             .TupleAnd(int(hv_MaxModelHeight > 0)))
                      .TupleAnd(
                          HTuple(int((hv_ObjWidth * 2) > (hv_MaxModelWidth + 12)))
                          .TupleOr(int((hv_ObjHeight * 2) >
                                       (hv_MaxModelHeight + 12)))))) {
                ConcatObj(ho_OverSizeRegion, ho_ObjSelect, &ho_OverSizeRegion);
                hv_tmp_Flag = 3;
            }
    ...
    (*hv_NewRow) = hv_NewRowTemp;
    (*hv_NewColumn) = hv_NewColumnTemp;
    (*hv_NewWidth) = hv_NewWidthTemp;
    (*hv_NewArea) = hv_NewAreaTemp;
    // 0：OK,   1：NG   2:进出视野    3：超大物料
    (*hv_Result) = hv_ResultTemp;

    GenEmptyObj(&(*ho_ObjRegions));
    ConcatObj(ho_OKRegion, (*ho_ObjRegions), &(*ho_ObjRegions));
    ConcatObj(ho_NGRegion, (*ho_ObjRegions), &(*ho_ObjRegions));
    ConcatObj(ho_IORegion, (*ho_ObjRegions), &(*ho_ObjRegions));

    ShapeTrans(ho_IORegion, &ho_IORegion, "rectangle1");
    ShapeTrans(ho_OKRegion, &ho_OKRegion, "rectangle1");
    ShapeTrans(ho_NGRegion, &ho_NGRegion, "rectangle1");
    ShapeTrans(ho_OverSizeRegion, &ho_OverSizeRegion, "rectangle1");
    if (HDevWindowStack::IsOpen())
        SetColor(HDevWindowStack::GetActive(), "orange");
    DispRegion(ho_IORegion, hv_WindowHandle);
    if (HDevWindowStack::IsOpen())
        SetColor(HDevWindowStack::GetActive(), "green");
    DispRegion(ho_OKRegion, hv_WindowHandle);
    if (HDevWindowStack::IsOpen())
        SetColor(HDevWindowStack::GetActive(), "red");
    DispRegion(ho_NGRegion, hv_WindowHandle);
    if (0 != (int(hv_DefectNumber > 0))) {
        if (HDevWindowStack::IsOpen())
            SetColor(HDevWindowStack::GetActive(), "green");
        if (HDevWindowStack::IsOpen())
        ...
        if (HDevWindowStack::IsOpen())
            SetColor(HDevWindowStack::GetActive(), "red");
        if (HDevWindowStack::IsOpen())
            DispObj(ho_RegionDefect, HDevWindowStack::GetActive());
    }

    {
        HTuple end_val576 = (hv_Score.TupleLength()) - 1;
        HTuple step_val576 = 1;
        for (hv_I = 0; hv_I.Continue(end_val576, step_val576);
             hv_I += step_val576) {
            disp_message_1(hv_WindowHandle,
                           "Score: " + (HTuple(hv_Score[hv_I]).TupleString(".3")),
                           "window", HTuple(hv_RowCheck[hv_I]),
                           HTuple(hv_ColumnCheck[hv_I]), "red", "false");
        }
    }

    if (HDevWindowStack::IsOpen())
        SetColor(HDevWindowStack::GetActive(), "violet red");
    DispRegion(ho_OverSizeRegion, hv_WindowHandle);
    if (HDevWindowStack::IsOpen())
...
void set_display_font(HTuple hv_WindowHandle, HTuple hv_Size, HTuple hv_Font,
    ...
    hv_Font = "";
    {
        ...
        HTuple step_val48 = 1;
        for (hv_Fdx = 0; hv_Fdx.Continue(end_val48, step_val48);
             hv_Fdx += step_val48) {
...
void split_rectangle2(HTuple hv_Row, HTuple hv_Column, HTuple hv_Phi,
                      HTuple hv_Length1, HTuple hv_Length2, HTuple hv_NumSplits,
                      HTuple *hv_SplitRow, HTuple *hv_SplitColumn,
                      HTuple *hv_SplitPhi, HTuple *hv_SplitLength1Out,
                      HTuple *hv_SplitLength2Out) {
    ...
    if (0 != (int(hv_NumSplits > 0))) {
        ...
        hv_TCol = ((-hv_Length1) + hv_SplitLength) +
                ((HTuple::TupleGenSequence(0, hv_NumSplits - 1, 1) * 2) *
                 hv_SplitLength);
        HomMat2dIdentity(&hv_HomMat2D);
        ...
        AffineTransPoint2d(hv_HomMat2D, hv_TRow, hv_TCol, &(*hv_SplitRow),
                           &(*hv_SplitColumn));
    } else {
...
// Chapter: Deep Learning / Anomaly Detection and Global Context Anomaly
// Detection Short Description: Apply the given thresholds on anomaly detection
// and Global Context Anomaly Detection results for image classification and
// region segmentation.
void threshold_dl_anomaly_results(HTuple hv_AnomalySegmentationThreshold,
                                  HTuple hv_AnomalyClassificationThreshold,
                                  HTuple hv_DLResults) {
    ...
    // The region is returned in DLResult under one of the following keys,
    // depending on the anomaly image key stored in the DLResult:
    //- 'anomaly_region' (AD, GC_AD)
    //- 'anomaly_region_local' (GC-AD)
    //- 'anomaly_region_global' (GC-AD)
    // 2. Image classification: AnomalyClassificationThreshold is used as
    // threshold whether the image is classified as containing an anomaly ('nok' /
    // class_id: 1) or not ('ok' / class_id: 0). The class is returned in DLResult
    // under one of the following keys:
    //- 'anomaly_class' (AD, GC_AD): The classification result as a string ('ok'
    //or 'nok').
    //- 'anomaly_class_local' (GC-AD): The classification result as a string ('ok'
    //or 'nok').
    //- 'anomaly_class_global' (GC-AD): The classification result as a string
    //('ok' or 'nok').
    //- 'anomaly_class_id' (AD, GC_AD): The classification result as an integer (0
    //or 1).
    //- 'anomaly_class_id_local' (GC-AD): The classification result as an integer
    //(0 or 1).
    //- 'anomaly_class_id_global' (GC-AD): The classification result as an integer
    //(0 or 1).
    //
    ...
    if (0 != (int((hv_AnomalySegmentationThreshold.TupleLength()) != 1))) {
        throw HException(
                    "AnomalySegmentationThreshold must be specified by exactly one value.");
    }
    //
    // Check for invalid AnomalyClassificationThreshold.
    if (0 != (int((hv_AnomalyClassificationThreshold.TupleLength()) != 1))) {
        throw HException("AnomalyClassificationThreshold must be specified by "
                         "exactly one value.");
    }
    //
    // Evaluate each DLResult.
    {
        ...
        HTuple step_val34 = 1;
        for (hv_DLResultIndex = 0; hv_DLResultIndex.Continue(end_val34, step_val34);
             hv_DLResultIndex += step_val34) {
            //
            ...
            if (0 != (HTuple(int(hv_ImageKeys == HTuple()))
                      .TupleOr(int(hv_ScoreKeys == HTuple())))) {
                throw HException(
                            HTuple("DLResult must contain keys 'anomaly_image' (local, global) "
                                   "and 'anomaly_score' (local, global)."));
            }
            {
                HTuple end_val46 = (hv_ImageKeys.TupleLength()) - 1;
                HTuple step_val46 = 1;
                for (hv_Index = 0; hv_Index.Continue(end_val46, step_val46);
                     hv_Index += step_val46) {
                    // Apply AnomalyThreshold to the anomaly image.
                    GetDictObject(&ho_AnomalyImage, hv_DLResult,
                                  HTuple(hv_ImageKeys[hv_Index]));
                    Threshold(ho_AnomalyImage, &ho_AnomalyRegion,
                              hv_AnomalySegmentationThreshold, "max");
                    //
                    // Write AnomalyRegion to DLResult.
                    TupleRegexpMatch(HTuple(hv_ImageKeys[hv_Index]), "anomaly_image(.*)",
                                     &hv_AnomalyImageRegionType);
                    ...
                    TupleRegexpMatch(HTuple(hv_ScoreKeys[hv_Index]), "anomaly_score(.*)",
                                     &hv_AnomalyScoreType);
            ...
            SetDictTuple(hv_DLResult, "anomaly_classification_threshold",
                         hv_AnomalyClassificationThreshold);
            SetDictTuple(hv_DLResult, "anomaly_segmentation_threshold",
...
void tuple_shuffle(HTuple hv_Tuple, HTuple *hv_Shuffled) {
    ...
    if (0 != (int((hv_Tuple.TupleLength()) > 0))) {
        ...
        hv_ShuffleIndices =
                HTuple::TupleRand(hv_Tuple.TupleLength()).TupleSortIndex();
...
// Chapter: Graphics / Window
// Short Description: Set and return meta information to display images
// correctly.
void update_window_meta_information(
        HTuple hv_WindowHandle, HTuple hv_WidthImage, HTuple hv_HeightImage,
        HTuple hv_WindowRow1, HTuple hv_WindowColumn1, HTuple hv_MapColorBarWidth,
        HTuple hv_MarginBottom, HTuple *hv_WindowImageRatioHeight,
        HTuple *hv_WindowImageRatioWidth, HTuple *hv_SetPartRow2,
        HTuple *hv_SetPartColumn2, HTuple *hv_PrevWindowCoordinatesOut) {
    ...
    HTuple hv_ImageRow2, hv_ImageColumn2, hv_ImageRatio,
            hv_ImageWindowRatioHeight;
    HTuple hv_ImageRow2InWindow, hv_ImageCol2InWindow;

    //
    // This procedure sets and returns meta information to display images
    // correctly.
    //
    // Set part for the image to be displayed later and adapt window size (+
    // MarginBottom + MapColorBarWidth).
    GetWindowExtents(hv_WindowHandle, &hv__, &hv__, &hv_WindowWidth,
                     &hv_WindowHeight);
    ...
    //
    hv_ImageRow2 =
            hv_HeightImage + (hv_MarginBottom / (*hv_WindowImageRatioHeight));
    hv_ImageColumn2 =
            hv_WidthImage + (hv_MapColorBarWidth / (*hv_WindowImageRatioWidth));
    ...
    if (0 != (int(hv_ImageRatio > hv_WindowRatio))) {
        ...
        (*hv_SetPartRow2) = hv_ImageRow2 + ((hv_WindowRow2 - hv_ImageRow2InWindow) /
                                            (*hv_WindowImageRatioWidth));
    } else {
        //
        ...
        (*hv_SetPartColumn2) =
                hv_ImageColumn2 + ((hv_WindowColumn2 - hv_ImageCol2InWindow) /
                                   (*hv_WindowImageRatioHeight));
    }
    if (HDevWindowStack::IsOpen())
        SetPart(HDevWindowStack::GetActive(), 0, 0, (*hv_SetPartRow2) - 1,
                (*hv_SetPartColumn2) - 1);
    //
    // Return the coordinates of the new window.
    (*hv_PrevWindowCoordinatesOut).Clear();
...
// Local procedures
void clean_up_output(HTuple hv_OutputDir) {

    // Local iconic variables

    // Local control variables
    HTuple hv_WindowHandle, hv_WarningCleanup, hv_FilesToBeRemoved;
    HTuple hv_FileIndex, hv_FileExists;
    ...
    hv_WarningCleanup[1] = "";
    hv_WarningCleanup[2] =
            "Unless you would like to use the output data / model,";
    hv_WarningCleanup[3] = "press F5 to clean up.";
    if (HDevWindowStack::IsOpen())
        DispText(
                    HDevWindowStack::GetActive(), hv_WarningCleanup, "window", "center",
                    "center",
                    ((((HTuple("black").Append("black")).Append("coral")).Append("coral"))
                     .Append("coral")),
                    HTuple(), HTuple());
    //
    ...
    {
        HTuple end_val14 = (hv_FilesToBeRemoved.TupleLength()) - 1;
        HTuple step_val14 = 1;
        for (hv_FileIndex = 0; hv_FileIndex.Continue(end_val14, step_val14);
             hv_FileIndex += step_val14) {
            FileExists(HTuple(hv_FilesToBeRemoved[hv_FileIndex]), &hv_FileExists);
            if (0 != hv_FileExists) {
                DeleteFile(HTuple(hv_FilesToBeRemoved[hv_FileIndex]));
            }
        }
    }
    //
...
void get_image_dir(HTuple *hv_ImageDir) {
    ...
    (*hv_ImageDir) =
            (*hv_ImageDir)
            .TupleRegexpReplace((HTuple("\\\\+").Append("replace_all")), "/");
    return;
}

void get_random_test_anomaly_image_paths(HTuple hv_ImageDir,
                                         HTuple hv_MaxNumTestImages,
    ...
    //
    list_image_files(hv_ImageDir, "default", HTuple(), &hv_ImageFiles);
    tuple_shuffle(hv_ImageFiles, &hv_ImageFiles);
    hv_MaxNumTestImages =
            hv_MaxNumTestImages.TupleMin2(hv_ImageFiles.TupleLength());
    (*hv_ImagePaths) = hv_ImageFiles.TupleSelectRange(0, hv_MaxNumTestImages - 1);
    return;
}

void get_random_test_image_paths(HTuple hv_DLDataset,
                                 HTuple hv_MaxNumTestImages,
                                 HTuple *hv_ImagePaths) {
    ...
    // Local control variables
    HTuple hv_DLSamples, hv_TestSampleIndices, hv_NumTestImages;
    HTuple hv_TestSampleIndicesShuffled, hv_Index, hv_SampleIndex;
    HTuple hv_DLSample, hv_ImagePath;
    ...
    find_dl_samples(hv_DLSamples, "split", "test", "match",
                    &hv_TestSampleIndices);
    hv_NumTestImages =
            HTuple(hv_TestSampleIndices.TupleLength()).TupleMin2(hv_MaxNumTestImages);
    //
    tuple_shuffle(hv_TestSampleIndices, &hv_TestSampleIndicesShuffled);
    //
    (*hv_ImagePaths) = HTuple(hv_NumTestImages, -1);
    {
        HTuple end_val10 = hv_NumTestImages - 1;
        HTuple step_val10 = 1;
        for (hv_Index = 0; hv_Index.Continue(end_val10, step_val10);
             hv_Index += step_val10) {
            hv_SampleIndex = HTuple(hv_TestSampleIndicesShuffled[hv_Index]);
            hv_DLSample = HTuple(hv_DLSamples[hv_SampleIndex]);
            hv_ImagePath = ((hv_DLDataset.TupleGetDictTuple("image_dir")) + "/") +
                    (hv_DLSample.TupleGetDictTuple("image_file_name"));
...
void action() {
    ...
    // Local iconic variables
    HObject ho_Image, ho_RegionROI, ho_ObjRegions;
    ...
    HTuple hv_MaxNumInferenceImages, hv_AnomalyInferenceImagePaths;
    ...
    HTuple hv_score, hv_class, hv_classThresh, hv_segmentThresh;
    HTuple hv_OutputResult;

    //
    // This example demonstrates the inference workflow
    // for Global Context Anomaly Detection (GC-AD).
    //
    // The network consists of 2 subnetworks, the local and
    // the global network. While the local network is specialized in
    // detecting structural anomalies, the global network is
    // designed to find logical anomalies. See the Chapter Reference
    // for more details on this topic.
    //
    // The workflow typically consists of the following steps:
    // 1. Dataset preprocessing.
    // 2. Inference on pcb board images.
    // 3. (optional) Inspect results using local and global networks.
    //
    dev_update_off();
    if (HDevWindowStack::IsOpen())
        CloseWindow(HDevWindowStack::Pop());
    ...
    //
    // ImageDir is the base directory containing the folders with the
    // used images.
    // There must be a 'good' or 'ok' subfolder in ImageDir.
    // Images in subfolders named differently will be treated as
    // containing anomalies. They can be used for qualitative
    // evaluation later.
    // get_image_dir (HalconImages)
    // ImageDir := HalconImages + '/pcb_anomaly'
    // ImageSubDirs := ['good', 'anomaly']
    hv_ImageDir = "E:/DL/Halcon_DL/20240606/image";
    ...
    hv_OutputDir = "./anomaly_toy_bricks_data";
    //
    //
    //***   1.) PREPARE   ***      ׁȡʮ׈ѧϰģэ
    //
    ...
    ReadDlModel( //'E:/DL/Halcon_DL/20240606/ģэ/DL_model.hdl'
                 "E:/DL/Halcon_DL/20240606/\304\243\320\315/DL_model.hdl",
                 &hv_DLModelHandle);
    ...
    // query_available_dl_devices (['runtime', 'runtime', 'id'], ['gpu', 'cpu',
    // 0], DLDeviceHandles) if (DLDeviceHandles == []) throw ('No suitable CPU or
    // GPU was found.') endif DLDeviceHandle := DLDeviceHandles[0]
    //
    ...
    // query_available_dl_devices ('ai_accelerator_interface', 'tensorrt',
    // DLDeviceHandlesTensorRT) if (DLDeviceHandlesTensorRT == [] or DeviceType ==
    // 'cpu') Landing here means two cases:
    //- DLDeviceHandle was set initially to use CPU;
    //- or TensorRT is not available.
    // Proceed using just GPU/CPU device, by checking settings above.
    // else
    // try
    // Using float32 precision, adapt if other precisions are needed.
    // get_dl_device_param (DLDeviceHandlesTensorRT,
    // 'optimize_for_inference_params', OptiParams)
    // optimize_dl_model_for_inference (DLModelHandle, DLDeviceHandlesTensorRT,
    // 'float32', [], OptiParams, DLModelHandleConverted, ConvReport)
    // DLModelHandle := DLModelHandleConverted
    // catch (Exception)
    // In case of the exception, just be sure to use gpu/cpu device.
    // stop ()
    // endtry
    // endif
    ...
    //***   2.) INFER   ***
    //
    // To demonstrate the inference steps, we apply the trained model
    // to some randomly chosen images that have not been used during
    // training.

    //
    // Get thresholds for inference. These have been stored along
    // with the model in the meta data above.
    GetDlModelParam(hv_DLModelHandle, "meta_data", &hv_MetaData);
    // hv_InferenceClassificationThreshold =
    // (hv_MetaData.TupleGetDictTuple("anomaly_classification_threshold")).TupleNumber();
    // hv_InferenceSegmentationThreshold =
    // (hv_MetaData.TupleGetDictTuple("anomaly_segmentation_threshold")).TupleNumber();
    //
    //
    ...
    SetDictTuple(hv_DLDatasetInfo, "class_names",
                 (HTuple( //'ۏٱ'
                  ...
                  .Append( //'ȱݺ'
                           "\310\261\275\272")));
    SetDictTuple(hv_DLDatasetInfo, "class_ids", (HTuple(1).Append(2)));
    hv_MaxNumInferenceImages = 120;
    get_random_test_anomaly_image_paths(hv_ImageDir + "/ok",
                                        hv_MaxNumInferenceImages,
                                        &hv_AnomalyInferenceImagePaths);
    TupleSort(hv_AnomalyInferenceImagePaths, &hv_AnomalyInferenceImagePaths);
    ...
    {
        HTuple end_val137 = (hv_AnomalyInferenceImagePaths.TupleLength()) - 1;
        HTuple step_val137 = 1;
        for (hv_IndexInference = 0;
             hv_IndexInference.Continue(end_val137, step_val137);
             hv_IndexInference += step_val137) {
            //
            ReadImage(&ho_Image,
                      HTuple(hv_AnomalyInferenceImagePaths[hv_IndexInference]));
            GetDomain(ho_Image, &ho_RegionROI);
            GetImageSize(ho_Image, &hv_Width, &hv_Height);
            if (HDevWindowStack::IsOpen())
                SetPart(HDevWindowStack::GetActive(), 0, 0, hv_Height, hv_Width);
            if (HDevWindowStack::IsOpen())
                DispObj(ho_Image, HDevWindowStack::GetActive());
            // todo:
            // search_shape_model_gray(ho_Image, ho_RegionROI, &ho_ObjRegions,
            // hv_WindowHandle, 	hv_Threshold, &hv_NewRow, &hv_NewColumn, &hv_NewWidth,
            //&hv_NewArea, &hv_Result);

            if (0 != (int((hv_Result.TupleLength()) > 0))) {
                gen_dl_samples_from_images(ho_Image, &hv_DLSample);
                preprocess_dl_samples(hv_DLSample, hv_DLPreprocessParam);
                //'anomaly_image_global','anomaly_image_local'
                ApplyDlModel(hv_DLModelHandle, hv_DLSample, "anomaly_image_local",
                             &hv_DLResult);
                //
                ...
                // ['anomaly_result', 'anomaly_image'], [], WindowDict)
                dev_display_dl_data(hv_DLSample, hv_DLResult, hv_DLDatasetInfo,
                                    (HTuple("image").Append("anomaly_image_local")),
                                    HTuple(), hv_WindowDict);
                hv_Result = hv_DLResult.TupleGetDictTuple("anomaly_class_id_local");
                hv_score = hv_DLResult.TupleGetDictTuple("anomaly_score_local");
                hv_class = hv_DLResult.TupleGetDictTuple("anomaly_class_local");
                hv_classThresh =
                ...
                TupleGenConst(hv_Result.TupleLength(),
                              hv_DLResult.TupleGetDictTuple("anomaly_class_id_local"),
                              &hv_OutputResult);
            }
            if (HDevWindowStack::IsOpen())
                DispText(HDevWindowStack::GetActive(),
            ...
            if (HDevWindowStack::IsOpen())
                DispText(HDevWindowStack::GetActive(), hv_Result, "window", "bottom",
