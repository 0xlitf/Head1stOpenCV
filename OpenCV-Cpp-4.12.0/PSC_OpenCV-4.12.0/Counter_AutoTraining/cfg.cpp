...
{//读Sysparam.xml文件
	...
	try
	{
		...
		cout << "    minDefectInnerRadian:" << itemElemValue->ToText()->Value() << endl;
...
{//读trainData.xml文件
    ...
    for (TiXmlElement* contoursElem = root->FirstChildElement("contours"); contoursElem != NULL; contoursElem = contoursElem->NextSiblingElement())
    {
        ...
        {
            ptList.push_back(cv::Point(std::stoi(numberList[i * 2]), std::stoi(numberList[i * 2 + 1])));
        }
...
{//写trainData.xml文件
    ...
    for (int i = 0; i < resultList.trainArea.size(); i++) {
        ...
        for (int j = 0; j < resultList.trainContours[i].size(); j++) {
            str_point += to_string(resultList.trainContours[i][j].x) + "," + to_string(resultList.trainContours[i][j].y);
            if (j == resultList.trainContours[i].size() - 1) break;
            str_point += ",";
        }
