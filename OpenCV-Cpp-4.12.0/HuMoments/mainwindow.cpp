    ...
    : QMainWindow(parent)
{
    ...
    btnLayout->addWidget(btnLoadTemplate);
    btnLayout->addWidget(btnLoadScene);
    btnLayout->addWidget(btnMatch);

    // 中间图像显示区
    QHBoxLayout *imgLayout = new QHBoxLayout();
    lblTemplateDisplay = new QLabel("模板预览", this);
    lblTemplateDisplay->setAlignment(Qt::AlignCenter);
    lblTemplateDisplay->setStyleSheet("border: 1px solid gray; background: #eee;");
    ...
    // --- 2. 信号槽连接 ---
    connect(btnLoadTemplate, &QPushButton::clicked, this, &MainWindow::onLoadTemplate);
...
void MainWindow::onLoadTemplate() {
    QString fileName = QFileDialog::getOpenFileName(this, "选择模板图片", "", "Images (*.png *.jpg *.bmp)");
    if (fileName.isEmpty()) return;

    // 读取灰度图
    m_templateImg = cv::imread(fileName.toStdString(), cv::IMREAD_GRAYSCALE);
    if (m_templateImg.empty()) return;

    // 显示原图
    lblTemplateDisplay->setPixmap(cvMatToQPixmap(m_templateImg).scaled(lblTemplateDisplay->size(), Qt::KeepAspectRatio));
    ...
    if (!m_templateContour.empty()) {
        ...
        for(int i=0; i<7; i++) {
            // 使用 Log 变换方便查看数量级
            huStr += QString::number(-1 * copysign(1.0, hu[i]) * log10(abs(hu[i])), 'f', 2) + " ";
        }
        txtLog->append(huStr);
    }
}
...
void MainWindow::onLoadScene() {
    QString fileName = QFileDialog::getOpenFileName(this, "选择场景图片", "", "Images (*.png *.jpg *.bmp)");
    if (fileName.isEmpty()) return;

    // 这里读取彩色图，方便最后画绿色的框
    m_sceneImg = cv::imread(fileName.toStdString(), cv::IMREAD_COLOR);
    if (m_sceneImg.empty()) return;

    lblSceneDisplay->setPixmap(cvMatToQPixmap(m_sceneImg).scaled(lblSceneDisplay->size(), Qt::KeepAspectRatio));
    txtLog->append("<b>[场景加载成功]</b> 等待识别...");
}

// 核心：寻找最大轮廓
std::vector<cv::Point> MainWindow::findLargestContour(const cv::Mat &src, bool isTemplate) {
    cv::Mat thr;
    // 背光图片：物体黑(0)，背景白(255)。
    ...
    if (maxIdx != -1) return contours[maxIdx];
    return {};
}
...
void MainWindow::onRunMatching() {
    ...
    // 2. 提取场景所有轮廓
    std::vector<std::vector<cv::Point>> contours;
    cv::findContours(thrScene, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
    ...
    for (size_t i = 0; i < contours.size(); i++) {
        ...
        // 返回值越小越相似。0 表示完全一样。
        double score = cv::matchShapes(m_templateContour, contours[i], cv::CONTOURS_MATCH_I1, 0.0);
        ...
        if (score < 0.2) {
            ...
            for (int j = 0; j < 4; j++) {
                cv::line(resultImg, vertices[j], vertices[(j + 1) % 4], cv::Scalar(0, 255, 0), 3);
            }
            ...
            txtLog->append(QString("发现目标 -> 相似度: %1, 角度: %2, 坐标: (%3, %4)")
                               .arg(score).arg(rotRect.angle)
                               .arg(rotRect.center.x).arg(rotRect.center.y));
        } else {
            // 可选：绘制不匹配的轮廓为红色，方便调试
    ...
    // 更新界面显示
    lblSceneDisplay->setPixmap(cvMatToQPixmap(resultImg).scaled(lblSceneDisplay->size(), Qt::KeepAspectRatio));
}
