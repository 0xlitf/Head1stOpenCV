...
void disp_message_1(HTuple hv_WindowHandle, HTuple hv_String, HTuple hv_CoordSystem,
...
void generate_shape_model_dynamic(HObject ho_ImageModel, HObject ho_RegionROI, HObject ho_ModelRegions,
	HObject *ho_ModelRegionsOrigin, HTuple hv_WindowHandle, HTuple hv_ModelIDs, HTuple hv_ModelWidthAll,
	HTuple hv_ModelHeightAll, HTuple hv_ModelGrays, HTuple hv_Threshold, HTuple hv_WdiffValue,
	HTuple hv_IsDynamic, HTuple *hv_ModelID, HTuple *hv_ModelWidth, HTuple *hv_ModelHeight,
	HTuple *hv_ModelArea, HTuple *hv_ModelGrayVal, HTuple *hv_isTrained)
{
	...
	SmallestRectangle1(ho_RegionROI, &hv_ROILeftR, &hv_ROILeftC, &hv_ROIRightR, &hv_ROIRightC);
	hv_MaxToySize = (hv_ROIRightR - hv_ROILeftR)*0.98;

	Boundary(ho_RegionROI, &ho_RegionBorder, "outer");
	search_shape_model_gray(ho_ImageModel, ho_RegionROI, ho_ModelRegions, &(*ho_ModelRegionsOrigin),
		hv_WindowHandle, hv_ModelIDs, hv_ModelGrays, hv_Threshold, hv_WdiffValue, hv_MinScore,
		hv_MinDefectInnerRadian, &hv_NewRow, &hv_NewColumn, &hv_NewWidth, &hv_NewArea,
		&hv_Model, &hv_isDefect);
	...
	else
	{
		...
		if (0 != (int(hv_IsDynamic == 0)))
		{
			...
			else
			{
				...
				DispRegion((*ho_ModelRegionsOrigin), hv_WindowHandle);
				disp_message_1(hv_WindowHandle, //'检测到多个物料！ '
		...
		{
			SortRegion((*ho_ModelRegionsOrigin), &(*ho_ModelRegionsOrigin), "upper_right",
				"true", "column");
			SelectObj((*ho_ModelRegionsOrigin), &ho_ObjectSelected, 1);
			{
				HTuple end_val72 = hv_ObjNum - 1;
				HTuple step_val72 = 1;
				for (hv_j = 0; hv_j.Continue(end_val72, step_val72); hv_j += step_val72)
				{
					if (0 != (int(HTuple(hv_NewColumn[hv_j]) == hv_Left)))
					{
						//只训练处理最左边那个
						if (0 != (HTuple(int(HTuple(hv_isDefect[hv_j]) == 1)).TupleOr(int(HTuple(hv_isDefect[hv_j]) == 3))))
						{
			...
			if (0 != (int((*hv_isTrained) == 2)))
			{
				...
				disp_message_1(hv_WindowHandle, //'检测到已训练物料ID：'
					"\274\354\262\342\265\275\322\321\321\265\301\267\316\357\301\317ID\243\272" + hv_Model,
	...
	SmallestRectangle1(ho_NewObj, &hv_Rect1RowL, &hv_Rect1ColL, &hv_Rect1RowR, &hv_Rect1ColR);
	SmallestRectangle2(ho_NewObj, &hv_Row1, &hv_Column1, &hv_Phi, &hv_ModelWidthHalf,
		&hv_ModelHeightHalf);
	(*hv_ModelWidth) = hv_ModelWidthHalf * 2;
	(*hv_ModelHeight) = hv_ModelHeightHalf * 2;

	//正在进入和出去视野的物料，一律视为杂料并忽略
	if (0 != (HTuple(HTuple(HTuple(int(hv_Rect1ColL>(hv_ROILeftC + 5))).TupleAnd(int(hv_Rect1ColR<(hv_ROIRightC - 5)))).TupleAnd(int((*hv_ModelArea)>30))).TupleAnd(int(hv_ObjNum>0))))
	{
		...
		InspectShapeModel(ho_ImageResult, &ho_ShapeModelImages, &ho_ShapeModelRegions,
			15, 30);
		...
		hv_NumLevels = 0;
		{
			...
			for (hv_i = 1; hv_i.Continue(end_val130, step_val130); hv_i += step_val130)
			{
		...
		hv_ScaleMax = 1.15;
		hv_MinContrast = 30;
		//create_scaled_shape_model (ImageResult, NumLevels, 0, rad(360), 'auto', ScaleMin, ScaleMax, 'auto', 'auto', 'ignore_local_polarity', 70, MinContrast, ModelID)
		CreateShapeModel(ho_ImageResult, hv_NumLevels, 0, HTuple(360).TupleRad(), "auto",
			"auto", "ignore_local_polarity", 40, hv_MinContrast, &(*hv_ModelID));
		//create_shape_model (ImageResult, NumLevels, 0, rad(360), 'auto', 'point_reduction_high', 'use_polarity', 70, MinContrast, ModelID)
		//set_shape_model_origin (ModelID, -CenterRow, -CenterColumn)
		GetShapeModelParams((*hv_ModelID), &hv_NumLevels1, &hv_AngleStart, &hv_AngleExtent,
			&hv_AngleStep, &hv_ScaleMin1, &hv_ScaleMax1, &hv_ScaleStep, &hv_Metric, &hv_MinContrast1);
...
void search_shape_model_gray(HObject ho_ImageNew, HObject ho_RegionROI, HObject ho_ModelRegionsOrigins,
	HObject *ho_ObjRegions, HTuple hv_WindowHandle, HTuple hv_ModelIDs, HTuple hv_ModelGrays,
	HTuple hv_Threshold, HTuple hv_WdiffValue, HTuple hv_MinScore, HTuple hv_MinDefectInnerRadian,
	HTuple *hv_NewRow, HTuple *hv_NewColumn, HTuple *hv_NewWidth, HTuple *hv_NewArea,
	HTuple *hv_Model, HTuple *hv_Result)
{
	...
	//MinScore := 0.8
	//最小缺陷内径
	//MinDefectInnerRadianScaled := MinDefectInnerRadian *(Width* Height)/(600*800)
	...
	CountObj(ho_ModelRegionsOrigins, &hv_ModelNumber);
	{
		...
		HTuple step_val57 = 1;
		for (hv_Index = 1; hv_Index.Continue(end_val57, step_val57); hv_Index += step_val57)
		{
			...
			SmallestRectangle2(ho_ModelSelected, &hv_Row1, &hv_Column1, &hv_Phi, &hv_ModelWidthHalf,
				&hv_ModelHeightHalf);
			hv_ModelWidth = hv_ModelWidthHalf * 2;
			hv_ModelHeight = hv_ModelHeightHalf * 2;
	...
	//增加边缘提取法(针对半透明物料）
	//edges_image (ImageReduced, ImaAmp, ImaDir, 'sobel_fast', 1.0, 'nms', 15, 40)
	//threshold (ImaAmp, Margin, 10, 255)
	//dilation_circle (Margin, RegionClosing, 2)
	//union2 (RegionClosing, RegionBorderVer, RegionClosingVer)
	...
	SelectShape((*ho_ObjRegions), &(*ho_ObjRegions), (HTuple("area").Append("height")),
		"and", hv_MinDefectArea.TupleConcat(0), hv_AreaMax.TupleConcat(hv_MaxToySize));
	//合并倒影
	...
	//分割粘黏物料
	//distance_transform (ObjRegionsShadow, DistanceImage, 'octagonal', 'true', Width, Height)
	...
	//watersheds_threshold (DistanceImageInvScaled, Basins, 100)
	//intersection (Basins, ObjRegionsShadow, ObjRegionsShadow2)
	//opening_rectangle1 (ObjRegionsShadow, ObjRegionsShadow, 1, 2)
	//connection (ObjRegionsShadow2, ObjRegionsShadow)
	...
	if (0 != (int(hv_ObjNum>0)))
	{
		SmallestRectangle2((*ho_ObjRegions), &hv_ObjsRow, &hv_ObjsColumn, &hv_Phi1, &hv_Length1,
			&hv_Length2);
	...
	hv_Score = HTuple();
	if (0 != (HTuple(int((hv_ModelIDs.TupleLength())>0)).TupleAnd(int(hv_use_inner_DarkCircleDefect == 0))))
	{
		...
		try
		{
			...
			//如果模版有缩放，则激活该句代码
			//find_scaled_shape_models (ImageReducedObj, ModelIDs, -rad(180), rad(180), ScaleMin, ScaleMax, 0.5, 0, 0.2, 'least_squares', 3, 0.9, RowCheck, ColumnCheck, AngleCheck, Scale, Score, Model)
			//find_scaled_shape_models (ImageReducedObj, ModelIDs, 0, rad(360), ScaleMin, ScaleMax, MinScore, 0, 0.2, 'least_squares', 9, 0.9, RowCheck, ColumnCheck, AngleCheck, Scale, Score, Model)
			FindShapeModels(ho_ImageReducedObj, hv_ModelIDs, 0, HTuple(360).TupleRad(),
				0.7, 0, 0.2, "least_squares", 9, 0.9, &hv_RowCheck, &hv_ColumnCheck, &hv_AngleCheck,
				&hv_Score, &(*hv_Model));
			CountSeconds(&hv_S2);
			hv_time = (hv_S2 - hv_S1)*1000.0;
			//The timeout error code is 9400.
			//disp_message (WindowHandle, 'match time: ' + time$'.4', 'window', 60, -1, 'lime green', 'true')
		}
		// catch (Exception) 
		catch (HException &HDevExpDefaultException)
		{
			...
			{
				disp_message_1(hv_WindowHandle, ("Timeout occurred after " + (hv_time.TupleString(".4"))) + " ms",
		...
		{
			{
				...
				for (hv_i = 0; hv_i.Continue(end_val190, step_val190); hv_i += step_val190)
				{
					//step 3: determine the affine transformation
					SelectObj(ho_ModelRegionsOrigins, &ho_SingleModelRegionsOrigin, HTuple((*hv_Model)[hv_i]) + 1);
					//vector_angle_to_rigid (Height/2, Width/2, 0, RowCheck[i], ColumnCheck[i], AngleCheck[i], MovementOfObject)
					VectorAngleToRigid(hv_Height / 2, hv_Width / 2, 0, HTuple(hv_RowCheck[hv_i]),
						HTuple(hv_ColumnCheck[hv_i]), HTuple(hv_AngleCheck[hv_i]), &hv_MovementOfObject);
					//如果模版有缩放，则激活该句代码
					//hom_mat2d_scale (MovementOfObject, Scale[i], Scale[i], RowCheck[i], ColumnCheck[i], MovementOfObject)
					AffineTransRegion(ho_SingleModelRegionsOrigin, &ho_RegionAffineTrans, hv_MovementOfObject,
						"false");
					ConcatObj(ho_ObjRegionsFind, ho_RegionAffineTrans, &ho_ObjRegionsFind);
					//disp_message (WindowHandle, 'Score: ' + Score[i]$'.3' + '   Scale: ' + Scale[i]$'.3', 'window', RowCheck[i], ColumnCheck[i], 'red', 'false')
				}
			}
		}
		else if (0 != (int(hv_ObjNum>0)))
		{
			disp_message_1(hv_WindowHandle, ("match no object: " + (hv_time.TupleString(".4"))) + " ms",
	...
	hv_matched_no = HTuple();
	{
		...
		for (hv_i = 1; hv_i.Continue(end_val217, step_val217); hv_i += step_val217)
		{
			GenEmptyObj(&ho_ObjDiff);
			SelectObj((*ho_ObjRegions), &ho_ObjSelect, hv_i);
			SmallestRectangle2(ho_ObjSelect, &hv_ObjRow, &hv_ObjColumn, &hv_Phi, &hv_ObjWidth,
				&hv_ObjHeight);
			GenRectangle2(&ho_Rectangle, hv_ObjRow, hv_ObjColumn, hv_Phi, hv_ObjWidth, hv_ObjHeight);
			//   *disp_message(WindowHandle, ObjWidth*2*0.105+'mm * ' +ObjHeight*2*0.105+'mm', 'window', 60, -1,  'red', 'false')
			SmallestRectangle1(ho_ObjSelect, &hv_Rect1RowL, &hv_Rect1ColL, &hv_Rect1RowR,
				&hv_Rect1ColR);
			AreaCenter(ho_ObjSelect, &hv_ObjArea, &hv_Row, &hv_Column);
			if (0 != (HTuple(int(hv_Rect1ColL<(hv_ROILeftC + hv_m_detectLineC))).TupleOr(int(hv_Rect1ColR>(hv_ROIRightC - 5)))))
			{
			...
			{
				MedianImage(ho_ImageReducedObj, &ho_ImageMedian, "circle", 30, "mirrored");
				DynThreshold(ho_ImageReducedObj, ho_ImageMedian, &ho_RegionDynThresh, 20, "light");
				...
				SelectShape(ho_BlackHoles, &ho_BlackHolesSelect, (HTuple("area").Append("circularity")),
					"and", (hv_MinDefectArea * 3).TupleConcat(0.7), hv_AreaMax.TupleConcat(1));
				//利用已获得一个圆的亮度，对另一个圆做分割
				Union1(ho_BlackHolesSelect, &ho_BlackHolesUnion);
				Intensity(ho_BlackHolesUnion, ho_ImageReducedObj, &hv_BlackThresh, &hv_Deviation1);
				ReduceDomain(ho_ImageNew, ho_BlackHolesUnion, &ho_ImageReducedInsides);
				Threshold(ho_ImageReducedInsides, &ho_BlackInsides2, 0, hv_BlackThresh + (hv_Deviation1 * 2));
				Connection(ho_BlackInsides2, &ho_BlackHoles2);
				FillUp(ho_BlackHoles2, &ho_BlackHoles2);
				SelectShape(ho_BlackHoles2, &ho_BlackHoles2, (HTuple("area").Append("circularity")),
					"and", (hv_MinDefectArea * 3).TupleConcat(0.8), hv_AreaMax.TupleConcat(1));
				...
				SelectShape(ho_CircleDefectRegion, &ho_ObjDiff, (HTuple("area").Append("inner_radius")),
					"and", hv_MinDefectArea.TupleConcat(hv_MinDefectInnerRadianScaled), hv_AreaMax.TupleConcat(hv_Height));
			...
			{
				{
					...
					for (hv_j = 1; hv_j.Continue(end_val271, step_val271); hv_j += step_val271)
					{
						//匹配过的不再对比
						...
						if (0 != (int(hv_Indices<0)))
						{
							...
							AreaCenter(ho_RegionIntersection, &hv_Area, &hv_Row4, &hv_Column4);
							if (0 != (int((hv_Area.TupleSum())>0)))
							{
								...
								if (0 != hv_use_details1)
								{
									...
									SelectShape(ho_ObjModelDiff, &ho_ObjModelDiff, (HTuple("area").Append("width")),
										"and", HTuple(50).TupleConcat(hv_ObjWidth*0.5), hv_AreaMax.TupleConcat(hv_ObjWidth*1.3));
									Union1(ho_ObjModelDiff, &ho_ObjModelDiff);
									CountObj(ho_ObjModelDiff, &hv_ObjType1);
									if (0 != (int(hv_ObjType1>0)))
									{
										SmallestRectangle2(ho_ObjModelDiff, &hv_Row6, &hv_Column6, &hv_Phi2,
											&hv_DiffLength11, &hv_DiffLength21);
										TupleMedian(hv_DiffLength11, &hv_DiffLength1);
										//disp_rectangle2 (3600, Row6, Column6, Phi2, DiffLength11, DiffLength21)
									}

									ClosingCircle(ho_ObjSelect, &ho_ObjSelectClosing, 5);
									ErosionCircle(ho_ObjSelectClosing, &ho_ObjSelectClosing, 5);
									ReduceDomain(ho_ImageReducedObj, ho_ObjSelectClosing, &ho_ImageReduced1
										);
									Threshold(ho_ImageReduced1, &ho_ObjSelectDiff, hv_Threshold, 255);
									Connection(ho_ObjSelectDiff, &ho_SelectedRegions);
									SelectShapeStd(ho_SelectedRegions, &ho_SelectedRegions, "max_area",
										70);
									SmallestRectangle2(ho_SelectedRegions, &hv_Row6, &hv_Column6, &hv_Phi2,
										&hv_Length11, &hv_Length21);
									//disp_rectangle2 (3600, Row6, Column6, Phi2, Length11, Length21)
									//转到水平方向做闭运算
									VectorAngleToRigid(hv_Row6, hv_Column6, hv_Phi2, hv_Row6, hv_Column6,
										0, &hv_MovementOfObjectDiff);
									AffineTransRegion(ho_ObjSelectDiff, &ho_ObjSelectDiff, hv_MovementOfObjectDiff,
										"false");
									ClosingRectangle1(ho_ObjSelectDiff, &ho_ObjDiffAffine, 8, 1);
									Connection(ho_ObjDiffAffine, &ho_ObjDiffAffine);
									SelectShape(ho_ObjDiffAffine, &ho_ObjDiffAffineSelect, (HTuple("area").Append("width")),
										"and", HTuple(40).TupleConcat(hv_ObjWidth*0.2), hv_AreaMax.TupleConcat(hv_ObjWidth*1.3));
									Union1(ho_ObjDiffAffineSelect, &ho_ObjDiffAffineSelect);
									CountObj(ho_ObjDiffAffineSelect, &hv_ObjType2);
									if (0 != (int(hv_ObjType2>0)))
									{
										SmallestRectangle2(ho_ObjDiffAffineSelect, &hv_Row6, &hv_Column6,
											&hv_Phi2, &hv_ObjDiffLength11, &hv_ObjDiffLength21);
										...
										if (0 != (int(((hv_DiffLength1 - hv_DiffLength2).TupleAbs())>((hv_DiffLength1.TupleMin2(hv_DiffLength2))*0.5))))
										{
											ConcatObj(ho_NGRegion, ho_ObjSelect, &ho_NGRegion);
										}
										else
								...
								Union2(ho_RegionDefect, ho_RegionDefectInverse, &ho_RegionDefect);
								if (0 != hv_use_details_rectangle)
								{
									...
									{
										ShapeTrans(ho_RegionIntersection, &ho_RegionTrans, "rectangle2");
										Difference(ho_RegionTrans, ho_RegionIntersection, &ho_RectDiff);
								...
								Intersection(ho_RegionDefect, ho_RegionClosed, &ho_RegionDefectInner);
								SelectShape(ho_RegionDefectInner, &ho_RegionDefectInner, ((HTuple("area").Append("inner_radius")).Append("convexity")),
									"and", ((hv_MinDefectArea * 3).TupleConcat(hv_MinDefectInnerRadianScaled)).TupleConcat(0.1),
									(hv_AreaMax.TupleConcat(hv_Height)).TupleConcat(1));
								SelectShape(ho_RegionDefect, &ho_RegionDefect2, ((HTuple("area").Append("inner_radius")).Append("convexity")),
									"and", (hv_MinDefectArea.TupleConcat(hv_MinDefectInnerRadianScaled)).TupleConcat(0.1),
									(hv_AreaMax.TupleConcat(hv_Height)).TupleConcat(1));
								Union2(ho_RegionDefect2, ho_RegionDefectInner, &ho_RegionDefect);
								CountObj(ho_RegionDefect, &hv_DefectNumber);
								...
								if (0 != (int(hv_DefectNumber>0)))
								{
									...
									AreaCenter(ho_ObjDiff, &hv_AreaObjDiff, &hv_Row5, &hv_Column5);
								...
								{
									Intensity(ho_ObjSelect, ho_ImageReducedObj, &hv_Mean, &hv_Deviation);
									if (0 != (int((hv_ModelGrays.TupleLength())>0)))
									{
										hv_ColorMatch = 0;
										{
											...
											for (hv_Index1 = 0; hv_Index1.Continue(end_val362, step_val362); hv_Index1 += step_val362)
											{
												hv_temp = HTuple(hv_ModelGrays[hv_Index1]);
												if (0 != (int(((hv_Mean - hv_temp).TupleAbs())<110)))
												{
													hv_ColorMatch = 1;
													break;
												}
									...
									AreaCenter(ho_RegionIntersection, &hv_AreaIntersect, &hv_Row1, &hv_Column1);
									AreaCenter(ho_ObjModel, &hv_AreaModel, &hv_ObjRow, &hv_ObjColumn);
									...
									if (0 != (HTuple(int(((hv_AreaIntersect - hv_AreaModel).TupleAbs())<hv_ratio)).TupleOr(int(hv_AreaDiff<200))))
									{
										//重合度大于0.9认为是合格料。
										hv_matched_no = hv_matched_no.TupleConcat(hv_j);
										if (0 != (int(((hv_ObjArea - hv_AreaModel).TupleAbs())<(hv_MinModelArea*0.7))))
										{
											//面积差异控制
											hv_tempModelWidth = HTuple(hv_ModelWidthAll[HTuple((*hv_Model)[hv_j - 1])]);
											hv_tempModelHeight = HTuple(hv_ModelHeightAll[HTuple((*hv_Model)[hv_j - 1])]);
											//tempModelWidth := ModelWidthAll[Model[j-1]]*Scale[j-1]
											//tempModelHeight := ModelHeightAll[Model[j-1]]*Scale[j-1]
											if (0 != (HTuple(HTuple(int(((hv_ObjArea - hv_AreaModel).TupleAbs())<(hv_AreaModel*0.35))).TupleAnd(int((((hv_ObjWidth * 2) - hv_tempModelWidth).TupleAbs())<(hv_tmp_WdiffValue*3.3)))).TupleAnd(int((((hv_ObjHeight * 2) - hv_tempModelHeight).TupleAbs())<(hv_tmp_WdiffValue * 3)))))
											{
												ConcatObj(ho_OKRegion, ho_ObjSelect, &ho_OKRegion);
											}
											else
			...
			SelectShape(ho_ObjDiff, &ho_ObjDiff, (HTuple("area").Append("inner_radius")),
				"and", hv_MinDefectArea.TupleConcat(hv_MinDefectInnerRadianScaled / 2), hv_AreaMax.TupleConcat(hv_Height));
			CountObj(ho_ObjDiff, &hv_DiffNumber);
			if (0 != (int(hv_DiffNumber>0)))
			{
				SmallestRectangle1(ho_ObjDiff, &hv_Rect1RowL, &hv_Rect1ColL, &hv_Rect1RowR,
					&hv_Rect1ColR);
				if (0 != (HTuple(int(hv_Rect1ColL<(hv_ROILeftC + hv_m_detectLineC))).TupleOr(int(hv_Rect1ColR>(hv_ROIRightC - 5)))))
				{
				...
				else
				{
					...
					//smallest_rectangle2 (ObjDiff, ObjRow, ObjColumn, Phi, ObjWidth, ObjHeight)
					//for j := 1 to ModelNumber by 1
					//如果物料的长宽其中任意一项超过模版标准尺寸±10%直接视为杂料
					//if ((abs(ObjWidth*2 - ModelWidthAll[j-1]) < tmp_WdiffValue) and (abs(ObjHeight*2 - ModelHeightAll[j-1]) < tmp_WdiffValue))
					//area_center (ObjDiff, AreaDiff, Row2, Column2)
					//tempArea := ModelAreaAll[j-1] + ((ObjHeight*2 - ModelHeightAll[j-1] - 1) / 2 * ModelWidthAll[j-1])	+ ((ObjWidth*2 - ModelWidthAll[j-1] - 1) / 2 * ModelHeightAll[j-1])
					//if (abs(AreaDiff - tempArea) < tempArea * ADiffValue)
					//tmp_ResultFlag := 0
					//concat_obj (OKRegion, ObjDiff, OKRegion)
					//匹配上则调出循环
					//break
					//endif
					//endif
	...
	//统计合格的料
	{
		...
		for (hv_i = 1; hv_i.Continue(end_val478, step_val478); hv_i += step_val478)
		{
			SelectObj(ho_OKRegion, &ho_ObjSelect, hv_i);
			SmallestRectangle2(ho_ObjSelect, &hv_ObjRow, &hv_ObjColumn, &hv_Phi, &hv_ObjWidth,
				&hv_ObjHeight);
			//disp_rectangle2 (WindowHandle, ObjRow, ObjColumn, Phi, ObjWidth, ObjHeight)
			SmallestRectangle1(ho_ObjSelect, &hv_Rect1RowL, &hv_Rect1ColL, &hv_Rect1RowR,
				&hv_Rect1ColR);
	...
	//统计杂料
	{
		...
		for (hv_i = 1; hv_i.Continue(end_val500, step_val500); hv_i += step_val500)
		{
			...
			if (0 != (HTuple(int((hv_ObjWidth * 2)>(hv_MaxModelWidth + 12))).TupleOr(int((hv_ObjHeight * 2)>(hv_MaxModelHeight + 12)))))
			{
	...
	//统计进出视野的料
	{
		...
		for (hv_i = 1; hv_i.Continue(end_val524, step_val524); hv_i += step_val524)
		{
			...
			if (0 != (HTuple(HTuple(HTuple(int(hv_Rect1ColL >= (hv_ROILeftC + 12))).TupleAnd(int(hv_MaxModelWidth>0))).TupleAnd(int(hv_MaxModelHeight>0))).TupleAnd(HTuple(int((hv_ObjWidth * 2)>(hv_MaxModelWidth + 12))).TupleOr(int((hv_ObjHeight * 2)>(hv_MaxModelHeight + 12))))))
			{
				ConcatObj(ho_OverSizeRegion, ho_ObjSelect, &ho_OverSizeRegion);
				hv_tmp_Flag = 3;
			}
...

	{
		...
		{
			disp_message_1(hv_WindowHandle, "Score: " + (HTuple(hv_Score[hv_I]).TupleString(".3")),
				"window", HTuple(hv_RowCheck[hv_I]), HTuple(hv_ColumnCheck[hv_I]), "red",
				"false");
		}
	}
...
void clean_up_output(HTuple hv_OutputDir)
{
	...
	hv_FilesToBeRemoved[2] = "intermediate_dl_anomaly_global_context_info.hdict";
	{
		...
		HTuple step_val14 = 1;
		for (hv_FileIndex = 0; hv_FileIndex.Continue(end_val14, step_val14); hv_FileIndex += step_val14)
		{
...
void get_random_test_anomaly_image_paths(HTuple hv_ImageDir, HTuple hv_MaxNumTestImages,
	HTuple *hv_ImagePaths)
{
	...
	hv_MaxNumTestImages = hv_MaxNumTestImages.TupleMin2(hv_ImageFiles.TupleLength());
	(*hv_ImagePaths) = hv_ImageFiles.TupleSelectRange(0, hv_MaxNumTestImages - 1);
	return;
}

void get_random_test_image_paths(HTuple hv_DLDataset, HTuple hv_MaxNumTestImages,
	HTuple *hv_ImagePaths)
{
	...
	find_dl_samples(hv_DLSamples, "split", "test", "match", &hv_TestSampleIndices);
	hv_NumTestImages = HTuple(hv_TestSampleIndices.TupleLength()).TupleMin2(hv_MaxNumTestImages);
	//
	...
	(*hv_ImagePaths) = HTuple(hv_NumTestImages, -1);
	{
		...
		for (hv_Index = 0; hv_Index.Continue(end_val10, step_val10); hv_Index += step_val10)
		{
			...
			hv_ImagePath = ((hv_DLDataset.TupleGetDictTuple("image_dir")) + "/") + (hv_DLSample.TupleGetDictTuple("image_file_name"));
...
void action()
{
	...
	SetDictTuple(hv_DLDatasetInfo, "class_names", (HTuple(//'ۏٱ'
		"\272\317\270\361").Append(//'ȱݺ'
		"\310\261\275\272")));
	SetDictTuple(hv_DLDatasetInfo, "class_ids", (HTuple(1).Append(2)));
	hv_MaxNumInferenceImages = 120;
	get_random_test_anomaly_image_paths(hv_ImageDir + "/ok", hv_MaxNumInferenceImages,
	...
	CreateDict(&hv_WindowDict);
	{
		...
		for (hv_IndexInference = 0; hv_IndexInference.Continue(end_val137, step_val137); hv_IndexInference += step_val137)
		{
			//
			ReadImage(&ho_Image, HTuple(hv_AnomalyInferenceImagePaths[hv_IndexInference]));
			...
			if (HDevWindowStack::IsOpen())
				DispObj(ho_Image, HDevWindowStack::GetActive());
			//todo:
			//search_shape_model_gray(ho_Image, ho_RegionROI, &ho_ObjRegions, hv_WindowHandle,
			//	hv_Threshold, &hv_NewRow, &hv_NewColumn, &hv_NewWidth, &hv_NewArea, &hv_Result);
			...
			if (0 != (int((hv_Result.TupleLength())>0)))
			{
				...
				ApplyDlModel(hv_DLModelHandle, hv_DLSample, "anomaly_image_local", &hv_DLResult);
				//
				...
				dev_display_dl_data(hv_DLSample, hv_DLResult, hv_DLDatasetInfo, (HTuple("image").Append("anomaly_image_local")),
				...
				TupleGenConst(hv_Result.TupleLength(), hv_DLResult.TupleGetDictTuple("anomaly_class_id_local"),
					&hv_OutputResult);
			}
			if (HDevWindowStack::IsOpen())
				DispText(HDevWindowStack::GetActive(), HTuple(hv_AnomalyInferenceImagePaths[hv_IndexInference]),
				"window", "center", "center", "black", HTuple(), HTuple());
			if (HDevWindowStack::IsOpen())
				DispText(HDevWindowStack::GetActive(), hv_Result, "window", "bottom", "center",
				"black", HTuple(), HTuple());
			// stop(...); only in hdevelop
		}
	}
