...
bool HPDetectionPartsCounter::updateDLMode(std::wstring strPath)
{
	...
	try{
		...
		//set key parameters.
		// 方法三：使用宽字符版本
		//boost::locale::conv::utf_to_utf<wchar_t>(str.c_str(), str.c_str() + str.size())
		//std::wstring widePath = L"./config/model_训练-250616-183342_opt.hdl";
		//std::wstring widePath = string_to_wstring(strPath);
		...
		QueryAvailableDlDevices(((HTuple("runtime").Append("runtime")).Append("id")), ((HTuple("gpu").Append("cpu")).Append(0)),
			&hv_DLDeviceHandles);
		...
		if (m_halconDLModel == DL_ANOMALY_DETECTION)
		{
			...
			SetDictTuple(hv_DLDatasetInfo, "class_names", (HTuple(//合格
				"ok").Append(//不合格
				"ng")));
...
bool HPDetectionPartsCounter::updateProject(std::string strProjectPath, int trainID)
{
	...
	try
	{
		...
		for (auto& regionPath : regionFiles)
		{
			...
			ConcatObj(m_pData->hModelRegionsOrigin, _tmphRegion, &m_pData->hModelRegionsOrigin);
		...
		for (auto& modelPath : modelFiles)
		{
			...
			ReadShapeModel(m_pData->hModelPath, &_tmpModelID);       // todo:改为引入多个模型
	...
	try
	{
		...
		if(m_halconDLModel == DL_MODE_CLOSED)
		{
			if (m_pData->hModelID.Length() == 0)
				//if ((m_pData->hModelRegionsOrigin.CountObj() == 0) || (m_pData->hModelID.Length() == 0))
			{
				m_pData->strError = "Failed ! Can not find 'ModelRegionsOrigin' or 'ModelID' !";
				return false;
			}
			...
			try
			{
				...
				//pDevProcedureCall->SetInputCtrlParamTuple("RegionPath", hRegionlPathList);
		...
		{
			//dev_display_dl_data(hv_DLSample, hv_DLResult, hv_DLDatasetInfo, "bbox_result",
			...
			rows = (hv_DLResult.TupleGetDictTuple("bbox_row1") + hv_DLResult.TupleGetDictTuple("bbox_row2")) * 0.5;
			widths = hv_DLResult.TupleGetDictTuple("bbox_col2") - hv_DLResult.TupleGetDictTuple("bbox_col1");
			areas = (hv_DLResult.TupleGetDictTuple("bbox_row2") - hv_DLResult.TupleGetDictTuple("bbox_row1"))*widths;
			if (results.Length() > 0)
			{
				int dd = results[0].D();
		...
		else if (m_halconDLModel == DL_OBJECT_DETECTION)
		{
			...
			areas = hv_DLResult.TupleGetDictTuple("bbox_length1") * hv_DLResult.TupleGetDictTuple("bbox_length2");
			if (results.Length() > 0)
			{
				int dd = results[0].D();
				double score = hv_score.D();
		...
		{
			ApplyDlModel(hv_DLModelHandle, hv_DLSample, "anomaly_image_global", &hv_DLResult);
			...
			//hv_ClassResult = HTuple((hv_DLDatasetInfo.TupleGetDictTuple("class_names"))[hv_class]);

			hv_score = hv_DLResult.TupleGetDictTuple("anomaly_score_global");
			hv_class = hv_DLResult.TupleGetDictTuple("anomaly_class_global");
			
			hv_classThresh = hv_DLResult.TupleGetDictTuple("anomaly_classification_threshold");
			hv_segmentThresh = hv_DLResult.TupleGetDictTuple("anomaly_segmentation_threshold");
			if (results.Length() > 0)
			{
				int dd = results[0].D();
				double score = hv_score.D();
		...
		DumpWindowImage(&hImage, m_pData->WindowHandle);               // 把算子输出图像保存下来，转成 outImage
		...
		for (int i = 0; i < iMin; ++i)
		{
			...
			if ((m_halconDLModel == DL_OBJECT_DETECTION_HOR) || (m_halconDLModel == DL_OBJECT_DETECTION) || (m_halconDLModel == DL_MODE_CLOSED)){
