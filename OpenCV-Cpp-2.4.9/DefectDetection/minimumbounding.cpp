...
MinimumBounding::MinimumBounding(QObject *parent) : QObject{parent} {}
...
cv::Mat MinimumBounding::findAndCropObject(const cv::Mat &inputImage) {
    ...
    cv::warpAffine(inputImage, rotated, rotationMatrix, inputImage.size(), cv::INTER_CUBIC, cv::BORDER_CONSTANT, cv::Scalar(255, 255, 255));

    // 9. 从扭正后的图像中截取感兴趣区域(ROI) [1](@ref)
    cv::Mat cropped;
    cv::getRectSubPix(rotated, cv::Size(static_cast<int>(rectSize.width), static_cast<int>(rectSize.height)), minRect.center, cropped);
...
cv::Mat MinimumBounding::findAndCropObjectOnNan(const cv::Mat &inputImage) {
    // 0. 输入验证
    if (inputImage.empty()) {
        qWarning() << "输入图像为空！";
        return cv::Mat();
    ...
    // 如果图像不是浮点类型，需要转换
    cv::Mat croppedClone = cropped.clone();
    cv::Mat imageFloat;
    ...
    if (cropped.channels() == 3) {
        ...
        // 将前景物体复制到黑色背景上
        cropped.copyTo(nanBackground, foregroundMask);
    } else {
        // 对于灰度图像，直接使用阈值处理
        cv::Mat foregroundMask;
        cv::threshold(cropped, foregroundMask, 250, 255, cv::THRESH_BINARY);
    ...
    // 12. 最终验证：确保输出图像不为空
    if (nanBackground.empty()) {
        qWarning() << "处理后的图像为空！";
        return cv::Mat();
    }

    return nanBackground;
}
