...
void QtSpdlogIntegration::install() {
    try {
        ...
        QtConcurrent::run([]() { qDebug() << "QtConcurrent::run test thread id"; });
...
void QtSpdlogIntegration::qtMessageHandler(QtMsgType type, const QMessageLogContext &context, const QString &msg) {
    ...
    auto message = QString("[File:%1:Line %2][Thread %3]: %5").arg(QString::fromLocal8Bit(context.file)).arg(QString::number(context.line)).arg(reinterpret_cast<qint64>(QThread::currentThreadId())).arg(msg).toStdString();
